...
namespace PX.Data
{
    ...
    /// <summary>PXEntryStatus enumeration specifies a data record status. A data record status changes as a result of manipulations with the data record: inserting, updating,
    /// or deleting.</summary>
    /// <remarks>
		...
		where T : class
	{
		...
		public T PlaceNotChanged(T value, out bool wasUpdated)
		{
			...
			for (int i = this.buckets[hash % this.buckets.Length]; i >= 0; i = this.entries[i].next)
			{
				if ((this.entries[i].hashCode == hash) && _Cache.ObjectsEqual(this.entries[i].value, value))
				{
					if (this.entries[i].status == PXEntryStatus.Deleted || this.entries[i].status == PXEntryStatus.InsertedDeleted)
					{
						return (T)null;
					}
