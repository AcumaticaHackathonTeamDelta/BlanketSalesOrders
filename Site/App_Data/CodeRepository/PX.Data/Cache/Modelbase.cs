...
namespace PX.Data
{
	...
	public abstract partial class PXCache
	{
		...
		//static PXCache()
		//{
		//	var ret = FindExtensionTypes();
		//	_AvailableExtensions = ret;
		//}

		internal static Type[] FindExtensionTypes(List<Exception> errorLog = null)
		{
			Mapping mapping;
			return FindExtensionTypes(out mapping, errorLog);
		}

		internal static Type[] FindExtensionTypes(out Mapping mapping, List<Exception> errorLog = null)
	    {
            ...
            foreach (System.Reflection.Assembly a in AppDomain.CurrentDomain.GetAssemblies())
		    {
			    ...
			    try
			    {
				    ...
				    foreach (Type t in types)
				    {
						...
						{
							Type bt = PXExtensionManager.GetDeepestGenericBase(t);
							if (bt != t.BaseType)
							{
								foreach (MethodInfo mi in t.GetMethods(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic))
								{
									if (typeof(IBqlMapping).IsAssignableFrom(mi.ReturnType))
									{
										try
										{
		...
		ILookup<Type, PXEventSubscriberAttribute> _AttributesByType;
		ILookup<string, PXEventSubscriberAttribute> _AttributesByName;
		...
		/// and stores these clones to internal collection that contains row specific attributes
		...
		/// <summary>
		/// Check if a subscriber attribute has an item related dirty clone
		/// </summary>
	    ...
	    /// <summary>
	    /// Cleanup unmodified attributes from the item state
	    /// </summary>
	    /// <param name="buffer"></param>
		...
		internal protected int _TranslationsCount;
		...
		{
			return _KeyValueStoredNames != null;
		}
        ...
        internal class Mapping
        {
            ...
            {
                while (mappedCacheExtension.BaseType != typeof(PXMappedCacheExtension))
                    mappedCacheExtension = mappedCacheExtension.BaseType;
                return Tuple.Create(CustomizedTypeManager.GetTypeNotCustomized(extendedGraph), mappedCacheExtension);
            }
            ...
            private List<IBqlMapping> FindMaps(Type extendedGraph, Type extendingGraph, string mappedCacheExtensionName)
            {
                ...
                var maps = _maps.Where(_ => _.Key.Item1 == extendedGraph &&
                                     _.Value.Item1 == extendingGraph &&
                                     String.Compare(_.Key.Item2.Name, mappedCacheExtensionName, StringComparison.OrdinalIgnoreCase) == 0).
		...
		static readonly Type[] _AvailableExtensions = FindExtensionTypes(out _mapping);
		...
		static void ClearCaches()
		{
			...
			PXExtensionManager.ForEachCompany(() => PXDatabase.ResetSlot<CacheStaticInfoDictionary>("CacheStaticInfo", typeof(PXGraph.FeaturesSet)));
				...
				foreach (Type t in GetAllExtensions())
				{
					...
					if (checkActive)
					{
						try
						{
							...
							{
								Type[] args1;
								if (typeof(PXCacheExtension).IsAssignableFrom(t) && t.BaseType.IsGenericType
									&& (args1 = t.BaseType.GetGenericArguments()).Length > 0 && (args1[args1.Length - 1] == extendedTable))
								{
					...
					Type[] args;
					if (typeof(PXCacheExtension).IsAssignableFrom(t) && t.BaseType.IsGenericType
						&& (args = t.BaseType.GetGenericArguments()).Length > 0 && (args[args.Length - 1] == extendedTable))
					{
						ext.Add(t);
					}
				}
        ...
        /// When this property is assigned, the OnRowSelected event is raised.
        /// </summary>
        ...
        /// The item is located by the values of the key fields.
        /// </summary>
        ...
        protected internal abstract void UpdateSessionUnmodified(object item, object unmodified);
        /// <summary>
		...
		internal Tuple<string, int?, int?> _GetOriginalCounts(object item)
		{
			lock (((ICollection)_Originals).SyncRoot)
			{
				...
				{
					return new Tuple<string, int?, int?>(orig.NoteText, orig.FilesCount, orig.ActivitiesCount);
				}
				return new Tuple<string, int?, int?>(null, null, null);
			}
		}
		internal void _SetTranslations(object item, int idx, string[] translations, bool isOriginal = false)
		{
			if (item is IBqlTable)
			{
				lock (((ICollection)_Originals).SyncRoot)
				{
					...
					{
						orig.Translations = new List<string[]>(_TranslationsCount);
						orig.OriginalTranslations = new List<string[]>(_TranslationsCount);
					}
					for (int i = orig.Translations.Count; i <= idx; i++)
					{
						orig.Translations.Add(null);
						orig.OriginalTranslations.Add(null);
					}
					orig.Translations[idx] = translations;
					if (isOriginal)
					{
						orig.OriginalTranslations[idx] = (string[])translations.Clone();
					}
				}
			}
		}
		internal string[] _GetTranslations(object item, int idx, bool isOriginal = false)
		{
			if (item is IBqlTable)
			{
				lock (((ICollection)_Originals).SyncRoot)
				{
					...
					if (_Originals.TryGetValue((IBqlTable)item, out orig))
					{
						if (!isOriginal)
						{
							if (orig.Translations != null && idx < orig.Translations.Count)
							{
								return orig.Translations[idx];
							}
						}
						else
						{
							if (orig.OriginalTranslations != null && idx < orig.OriginalTranslations.Count)
							{
								return orig.OriginalTranslations[idx];
							}
						}
					}
				}
		...
		internal void _SetOriginalCounts(object item, string noteText, int? filesCount, int? activitiesCount)
		{
			if (item is IBqlTable)
			{
				lock (((ICollection)_Originals).SyncRoot)
...
#pragma warning restore 618
		...
		internal protected static PropertyInfo[] _GetProperties(Type type, List<Type> extensions, out Dictionary<string, List<PropertyInfo>> nameToFieldPropertyInfo, out int origLength)
		{
			...
			if (extensions.Count > 0)
			{
                ...
                //optimize
                foreach (var map in _mapping.GetAllMaps().Where(m => m.Table == type))
                {
                    foreach (FieldInfo fi in map.GetType().GetFields(BindingFlags.Instance | BindingFlags.Public))
                    {
                            ...
                            nameToFieldPropertyInfo.TryGetValue(fi.Name, out propertyInfoList))
                        {
                            ...
                            PropertyInfo extensionPI = propertyInfoList.FirstOrDefault(p => p.DeclaringType == map.Extension);
