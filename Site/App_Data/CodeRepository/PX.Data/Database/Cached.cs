...
namespace PX.Data
{
	...
	public class PXView
	{
		...
		protected bool IsNonStandardView
		{
			get
			{
 				return (this.GetType() != typeof(PXView) || _Delegate != null);
			}
		}
		...
		protected PXSearchColumn[] prepareSorts(string[] sortcolumns, bool[] descendings, object[] searches, int topCount, out bool needOverrideSort, out bool anySearch, ref bool resetTopCount)
		{
			object row = null;
		...
		private void RaiseCommandPreparingForSearchColumn(PXCache cache, PXSearchColumn searchColumn, int topCount, string columnName, object row, out bool resetTopCount)
		{
			...
			if (descr != null && !String.IsNullOrEmpty(descr.FieldName) && descr.FieldName != BqlCommand.Null)
			{
				...
				if (descr.FieldName.StartsWith(BqlCommand.SubSelect)
					|| searchColumn.SearchValue == null 
					&& descr.FieldName.StartsWith(new string(Cache.Graph.SqlDialect.functions2sql("switch", new string[0]).TakeWhile(_ => _ != ' ').Union(new char[] { ' ' }).ToArray()))
					)
				{
					searchColumn.UseExt = true;
        ...
        private bool CorrectCacheAndField(string originField, out PXCache cache, out string field)
        {
            ...
            if (idx > -1)
            {
                ...
                PXCache joinedTableCache = tables
                                           ...
                                           .Select(t => _Graph.Caches[t]).FirstOrDefault();
        ...
        {
            PXCache correctedCache;
            string correctedField;
            CorrectCacheAndField(dataField, out correctedCache, out correctedField);
		...
		protected internal bool prepareFilters(ref PXFilterRow[] filters, string forceView = null)
		{
			...
			if (filters != null)
			{
				...
				{
                    var state = GetStateFromCorrectCache(f.DataField) as PXStringState;
					if (state != null && state.MultiSelect)
					{
						var values = f.Value.ToString().Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
						if (f.Condition == PXCondition.LIKE || f.Condition == PXCondition.NOTLIKE)
						{
							...
							foreach (var v in values)
							{
								...
								newFilter.Value = string.Format(",{0},", v);
								newFilter.OrOperator = false;
						...
						{
							f.Value = string.Format(",{0},", string.Join(",", values));
						}
					}
				}
				if (filtersChanged)
				...
				foreach (PXFilterRow fr in filters)
				{
					...
					{
						cache = forceView == null ? Cache : Graph.Views[forceView].Cache;
					}

                    string fieldName = fr.DataField;
                    PXCache correctedCache;
                    string correctedField;
                    bool corrected = CorrectCacheAndField(fr.DataField, out correctedCache, out correctedField);
					...
					switch (fr.Condition)
					{
						case PXCondition.NestedSelector:
							...
							prepareFilters(ref nestedFilters, (string)fr.Value);
							break;
						case PXCondition.IN:
						case PXCondition.NI:
							if (fr.Value is Type)
						...
						case PXCondition.NE:
                            ...
                            if (fr.Variable == FilterVariableType.CurrentUser && !corrected)
							{
								...
								if (fdescr == null || String.IsNullOrEmpty(fdescr.FieldName) || fdescr.DataType != PXDbType.UniqueIdentifier)
									continue;
								...
								if (fr.Value != null)
								{
									try
									{
										...
										else
										{
											...
											{
												anyFailed = true;
												fr.UseExt = fr.UseExt || (fieldName).IndexOf("_", StringComparison.Ordinal) != -1 || Cache.GetBqlField(fr.DataField) == null;
											}
										}
									}
									catch
						...
						case PXCondition.ISNOTNULL:
							try
							{
								...
								else
								{
									...
									if (descr != null && !String.IsNullOrEmpty(descr.FieldName) && descr.FieldName != BqlCommand.Null
										&& descr.FieldName.StartsWith(BqlCommand.SubSelect))
									{
										fr.Description = descr;
										fr.UseExt = true;
									}
						...
						case PXCondition.BETWEEN:
							...
							if (fr.Value != null && fr.Value2 != null)
							{
								...
								try
								{
									...
									if (descr2 != null && !String.IsNullOrEmpty(descr2.FieldName) && descr2.FieldName != BqlCommand.Null)
									{
										cache.RaiseCommandPreparing(fieldName, null, fr.Value, PXDBOperation.Select | PXDBOperation.External, cache.GetItemType(), out descr);
										if (descr != null && !String.IsNullOrEmpty(descr.FieldName) && descr.FieldName != BqlCommand.Null)
										{
											...
											fr.UseExt = fr.UseExt | descr.FieldName.StartsWith(BqlCommand.SubSelect);
										}
										else
										{
											anyFailed = true;
	            ...
	            for (int i = 0; i < filters.Length; i++)
	            {
	                ...
	                if (field != null && !conditionViolation)
	                {
	                    ...
	                    if (field == typeof (FieldNameParam) && expressions[i] != null)
	                    {
	                        ...
	                        int idxdot, idxscore;
	                        if ((idxdot = fieldName.IndexOf('.')) != -1 && fieldName.IndexOf('(') == -1 &&
...
//			if (SelectQueries.IsCommandMutable && queryKey.CommandText == null && queryKey.BadParamsQueryNotExecuted) 
			...
			if (SelectQueries.TryGetValue(queryKey, out stored)
				&& !(PX.Common.WebConfig.IsClusterEnabled && stored.IsExpired))
			{
				list = stored.Items;
				if (!_IsReadOnly)
				{
					...
					if (useExt && !cache.Fields.Contains(fieldName)) // Compare method won't read the value if the field belongs to a diffirent dac
					{
						...
						for (int iExt = 1; iExt < pxres.Items.Length; iExt++)
							if (found = (cache = _Graph.Caches[tableType = pxres.GetItemType(iExt)]).Fields.Contains(fieldName))
								break;
						if (!found) { // no luck finding the item, revert
							cache = Cache;
							tableType = pxres.GetItemType(0);
						}
					}
		...
		static readonly ConcurrentDictionary<Type, IBqlParameter[]> CommandParamsCache = new ConcurrentDictionary<Type, IBqlParameter[]>();
		Type _NewOrder;
...
//			IBqlParameter[] selpars = _Select.GetParameters();
			...
			IBqlParameter[] selpars =  CommandParamsCache.GetOrAdd( _Select.GetSelectType(), type => _Select.GetParameters() );
			...
			try
			{
				if (_Executing != null)
				{
					_Executing.Push(new Context(this, sorts, filters, currents, parameters, reverseOrder, windowed ? startRow : 0, windowed ? maximumRows : 0, this.RestrictedFields));
				}
			...
			if (list == null)
			{
				...
				if (!((BqlSelect.Context != null && BqlSelect.Context.BadParametersSkipMergeCache) || (selectQueryResult != null && selectQueryResult.BadParamsSkipMergeCache)))
			...
			if (needFilterResults && !FiltersResetRequired)
			{
			...
			bool searchNotFound = !reverseOrder && (realTotalRows == -2 || realTopCount == 1 || realTopCount == 0 && maximumRows == 1) && anySearch && !searchFound && list.Count > 0;
			if (searchNotFound)
			{
				list.Clear();
			}
        ...
        /// cref="MessageButtons">MessageButtons</see> enumeration
