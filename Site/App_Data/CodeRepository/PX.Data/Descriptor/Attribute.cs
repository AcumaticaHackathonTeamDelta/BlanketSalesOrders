...
namespace PX.Data
{
	...
	public class PXDefaultAttribute : PXEventSubscriberAttribute, IPXFieldDefaultingSubscriber, IPXRowPersistingSubscriber, IPXFieldSelectingSubscriber
	{
        ...
        /// cref="PXPersistingCheck">PXPersistingCheck</see> value
		...
		public PXDefaultAttribute(Type sourceType)
		{
			...
			{
				_Formula = PXFormulaAttribute.InitFormula(sourceType);
			}
			else
			{
		...
		protected static Tuple<object, PXCache> SelectRow(PXCache cache, PXDefaultAttribute attr, object data)
		{
			...
			if (attr._CacheGlobal && attr._ReferencedField != null && (key = cache.GetValue(data, attr._ReferencedField.Name)) != null)
			{
				...
				var dict = PXSelectorAttribute.GlobalDictionary.GetOrCreate(type, cache.Graph.Caches[type].BqlTable);
				lock ((dict).SyncRoot)
				{
				...
				view = cache.Graph.TypedViews.GetView(attr._NeutralSelect, true);
				if (row == null)
				{
					...
					{
						PXSelectorAttribute.CheckIntegrityAndPutGlobal(cache, view.Cache, attr._ReferencedField.Name, dict, row, false, key, attr._FieldName);
					}
				}
    ...
    /// 	<code title="Example3" description="Obtaining the parent data record at run time:" groupname="Example2" lang="CS">
    /// CR.Location child = (CR.Location)e.Row;
    /// BAccount parent =
    ///     (BAccount)PXParentAttribute.SelectParent(sender, child, typeof(BAccount));</code>
    /// 	<code title="Example4" description="Setting the parent data record at run time:" groupname="Example3" lang="CS">
	...
	public class PXParentAttribute : PXForeignReferenceAttribute, IPXRowInsertedSubscriber, IPXRowPersistedSubscriber
	{
		...
		private static object SelectParent(PXCache cache, object row, object newRow, Type ParentType, PXParentAttribute attr)
		{
			...
			if (!(ParentType == null || parentview.Cache.GetItemType() == ParentType || ParentType.IsAssignableFrom(parentview.Cache.GetItemType())))
			{
				return null;
			}
			...
			if (ParentType == null)
			{
				ParentType = parentview.Cache.GetItemType();
			}
	...
	public class PXUIFieldAttribute : PXEventSubscriberAttribute, IPXInterfaceField, IPXExceptionHandlingSubscriber, IPXCommandPreparingSubscriber, IPXFieldSelectingSubscriber, IPXFieldVerifyingSubscriber
	{
        ...
        internal PXErrorLevel ErrorLevel
        {
            get
            {
                return _ErrorLevel;
		...
		private static void SetError(PXCache cache, object data, string name, string error, string errorvalue, bool resetErrorValue, PXErrorLevel level)
		{
			if (data == null)
			{
    ...
    /// public virtual string TitleOfCourtesy { get; set; }</code>
    /// 	<code title="Example4" description="The example below demonstrates an invocation of a PXStringListAttribute static method. The method called in the example will set the new lists of values and labels for all data records in the cache object the Schedule.Cache variable references. The method will assign the lists to the PXStringList attribute instances attached to the ActionName field." groupname="Example3" lang="CS">
	...
	public class PXStringListAttribute : PXEventSubscriberAttribute, IPXFieldSelectingSubscriber, IPXLocalizableList
	{
		...
		/// pairs of allowed values and corresponding labels. When a user selects a label
		/// in the user interface, the corresponding value is assigned to the
		/// field marked by the instance.</summary>
		...
		protected PXStringListAttribute(Tuple<string, string>[] valuesToLabels)
		    : this(
        ...
        /// <tt>null</tt>, the method is applied to all data records kept in the
        /// cache object.</param>
		/// <param name="valuesToLabels">The new list of pairs where 
		/// first item is a value assigned to the field when a user selects the corresponding labels, and
		/// second item is a label displayed in the user interface when a user expands the control.</param>
		public static void SetList<Field>(PXCache cache, object data, Tuple<string, string>[] valuesToLabels)
			where Field : IBqlField
		{
		...
		/// first item is a value assigned to the field when a user selects the corresponding labels, and
		/// second item is a label displayed in the user interface when a user expands the control.</param>
		public static void SetList(PXCache cache, object data, string field, Tuple<string, string>[] valuesToLabels)
	    {
				...
				try
				{
					...
					if (descr != null && !String.IsNullOrEmpty(descr.FieldName))
					{
						...
						for (int i = 0; i < allowedValues.Length; i++)
						{
							...
							if (value == null)
								kvList.Add(String.Format("{0} IS NULL", descr.FieldName));
							else
								kvList.Add(String.Format("{0} = {1}", descr.FieldName, sqlDialect.quoteLiteral(value)));
							kvList.Add(sqlDialect.quoteLiteral(_AllowedLabels[i]));
						}
						e.FieldName = sqlDialect.functions2sql("switch", kvList.ToArray());
	...
	public class PXIntListAttribute : PXEventSubscriberAttribute, IPXFieldSelectingSubscriber, IPXLocalizableList
	{
		...
		/// first item is a value assigned to the field when a user selects the corresponding labels, and
		/// second item is a label displayed in the user interface when a user expands the control.</param>
		protected PXIntListAttribute(Tuple<int, string>[] valuesToLabels)
			: this(
				valuesToLabels.Select(t => t.Item1).ToArray(),
				valuesToLabels.Select(t => t.Item2).ToArray()) {}
			...
			if (!e.Cancel && e.NewValue != null)
			{
					...
					lock ((dict).SyncRoot)
					{
						...
						if (dict.TryGetValue(e.NewValue, out cacheValue))
						{
							if (cacheValue.IsDeleted)
							{
								...
								object ret = select.Cache.GetValue(cacheValue.Item, ((IBqlSearch)_Select).GetField().Name);
								e.NewValue = ret;
								return;
							}
				...
				if (item == null)
				{
					...
					if (item != null)
					{
						...
						{
							CheckIntegrityAndPutGlobal(sender, select.Cache, _SubstituteKey.Name, dict, item, false, e.NewValue, _FieldName);
							CheckIntegrityAndPutGlobal(sender, select.Cache, ((IBqlSearch)_Select).GetField().Name, dict, item, false, ret, _FieldName);
						}
						e.NewValue = ret;
					}
					else
						...
						if (PXDatabase.IsReadDeletedSupported(_BqlType))
						{
							using (new PXReadDeletedScope())
							{
								...
								if (item != null)
								{
									...
									if (dict != null && select.Cache.GetStatus(item) == PXEntryStatus.Notchanged && select.Cache.Keys.Count <= 1)
									{
										CheckIntegrityAndPutGlobal(sender, select.Cache, _SubstituteKey.Name, dict, item, true, e.NewValue, _FieldName);
										CheckIntegrityAndPutGlobal(sender, select.Cache, ((IBqlSearch)_Select).GetField().Name, dict, item, true, ret, _FieldName);
									}
									e.NewValue = ret;
									found = true;
								}
	...
	/// Notice that the error message contains <i>[ScreenName]</i> and
	/// <i>[ScreenID]</i>, which will be replaced with the current screen
	/// values. In addition, message includes<i>{0}</i>, which will be replaced
	...
	public class PXRestrictorAttribute : PXEventSubscriberAttribute, IPXFieldVerifyingSubscriber
	{
		...
		public PXRestrictorAttribute(Type where, string message, params Type[] pars)
		{
			...
			{
				_TablesCount = new TablesCount();
			}
		}

		private class TablesCount
		{
			public bool IsSet { get; set; }
			public int Count { get; set; }
		}
		private TablesCount _TablesCount = null;
		...
		public override void CacheAttached(PXCache sender)
		{
			...
			foreach (PXSelectorAttribute selattr in sender.GetAttributesReadonly(_FieldName).Where(attr => attr is PXSelectorAttribute).Cast<PXSelectorAttribute>())
			{
				...
				{
					if (!this._TablesCount.IsSet)
					{
						this._TablesCount.Count = _OriginalCmd.GetTables().Length;
						this._TablesCount.IsSet = true;
					}
					tablesCount = this._TablesCount.Count;
				}
				else
				{
        ...
        protected virtual BqlCommand WhereAnd(PXCache sender, PXSelectorAttribute selattr, Type Where)
        {
			...
			if (WebConfig.EnablePageOpenOptimizations)
			{
				...
				var types = new Tuple<Type, Type, Type>(_OriginalCmd.GetType(), Where, selattr.ValueField);
		...
		protected virtual void AlterCommand(PXCache sender)
		{
			...
			for (int i = 0; i < bql.Length; i++)
			{
                ...
                {
                    bool hasOrderBy = bql.Skip(i).Any(x=>x == typeof(OrderBy<>));
                    Type[] dummy;
                    if (hasOrderBy)
		...
		public static object GetItem(PXCache cache, PXRestrictorAttribute attr, object data, object key)
		{
			...
			{
				dict = PXSelectorAttribute.GlobalDictionary.GetOrCreate(attr._Type, cache.Graph.Caches[attr._Type].BqlTable);
				lock ((dict).SyncRoot)
				{
					PXSelectorAttribute.GlobalDictionary.CacheValue cacheValue;
					if (dict.TryGetValue(key, out cacheValue) && !cacheValue.IsDeleted && !(cacheValue.Item is IDictionary))
			...
			if (row == null && (key == null || key.GetType() == cache.GetFieldType(attr._FieldName)))
			{
				...
				if (attr._CacheGlobal && key != null && select.Cache.GetStatus(item) == PXEntryStatus.Notchanged && !PXDatabase.ReadDeleted && select.Cache.Keys.Count <= 1)
				{
					PXSelectorAttribute.CheckIntegrityAndPutGlobal(cache, select.Cache, ((IBqlSearch)attr._OriginalCmd).GetField().Name, dict, item, false, key, attr._FieldName);
				}
			}
			return row;
		}
		...
		public virtual void FieldSelecting(PXCache sender, PXFieldSelectingEventArgs e)
		{
			...
			try
			{
				Verify(sender, args, false);
			}
			catch (PXSetPropertyException ex)
			{
				message = ex.MessageNoPrefix;
			}
		...
		public virtual void Verify(PXCache sender, PXFieldVerifyingEventArgs e, bool IsErrorValueRequired)
		{
			...
			object item;
			object itemres = item = GetItem(sender, this, e.Row, e.NewValue);
	...
	public class PXFormulaAttribute : PXEventSubscriberAttribute, IPXRowUpdatedSubscriber, IPXRowInsertedSubscriber, IPXRowDeletedSubscriber, IPXDependsOnFields
	{
		...
		public virtual void RowInserted(PXCache sender, PXRowInsertedEventArgs e)
		{
			...
			if (_Formula != null && _CancelDefaulting != true && !CancelCalculation && sender.GetValue(e.Row, _FieldOrdinal) == null)
			{
				BqlFormula.Verify(sender, e.Row, _Formula, ref result, ref value);
		...
		public override void CacheAttached(PXCache sender)
		{
            ...
            if (!sender.GetAttributesReadonly(_FieldName).Any(attr => attr is PXDBFieldAttribute)  && _Formula as IBqlTrigger == null)
			{
	...
	public class PXZipValidationAttribute : PXAggregateAttribute, IPXFieldVerifyingSubscriber, IPXFieldSelectingSubscriber
	{
		#region State
                ...
                !_recursiveCall)
			{
				...
				(e.Operation & PXDBOperation.Option) == PXDBOperation.Internal))
				{
					...
					try
					{
                        ...
                        _recursiveCall = true;
                        _Search.Parse(sender.Graph, new List<IBqlParameter>(), tables, null, null, text, selection);
					...
					else
					{
						...
						if (_Search is IBqlAggregate &&
							...
							fieldtext = selection.Columns.Last();
						else
							fieldtext = BqlCommand.GetSingleField(field, sender.Graph, new List<Type>(), selection, BqlCommand.FieldPlace.Select);
						e.FieldName = BqlCommand.SubSelect + fieldtext + text + ")";
					}
	...
	public class PXDBDataLengthAttribute : PXEventSubscriberAttribute, IPXCommandPreparingSubscriber, IPXRowSelectingSubscriber
	{
		...
		public virtual void CommandPreparing(PXCache sender, PXCommandPreparingEventArgs e)
		{
			...
			{
				e.BqlTable = _BqlTable;
				e.FieldName = sender.Graph.SqlDialect.BinaryLength + "(" + tableName + "." + this._TargetFieldName + ")";
			}
		}
	...
	public class PXEMailAccountIDSelectorAttribute : PXSelectorAttribute
	{
		...
		private static Type GetCommand(Type additional)
		{
			...
			Type where = typeof(Where<EMailAccount.emailAccountType, NotEqual<EmailAccountTypesAttribute.exchange>, Or<EMailSyncServer.isActive, Equal<True>>>);
	...
	public sealed class PXDependsOnFieldsAttribute : Attribute
	{
		...
		static void AddDependsRecursive(PXCache table, string field, HashSet<string> result)
		{
			...
			foreach (Type fType in new HashSet<Type>(fields))
			{
			...
			if (_Segments == null)
			{
				...
				for (int i = 0; i < sender.Keys.Count; i++)
				{
					...
					if (state != null)
					{
						...
						segments[i + offset] = new PXSegment(mask, ' ', (short)state.Length, true, caseconvert, 0, '-', !state.Enabled, string.Empty);
					}
					else
					{
						...
						if (fs != null && !String.IsNullOrEmpty(fs.ViewName))
						{
							...
							if (!object.ReferenceEquals(sender, view.Cache))
							{
								...
								if (comp != null)
								{
									...
									{
										segments[i + offset + j] = new PXSegment(comp.Segments[j].EditMask, ' ', comp.Segments[j].Length, true, comp.Segments[j].CaseConvert, 0, comp.Segments[j].Separator, comp.Segments[j].ReadOnly, string.Empty);
									}
						...
						segments[i + offset] = new PXSegment('C', ' ', 0, true, (short)0, 0, '-', false, string.Empty);
					}
				}
				_Segments = segments;
			}
		...
		public virtual void CompositeKeyFieldSelecting(PXCache sender, PXFieldSelectingEventArgs e)
		{
			...
			if (_AttributeLevel == PXAttributeLevel.Item || e.IsAltered)
			{
				e.ReturnState = PXSegmentedState.CreateInstance(
					...
					"CompositeKey", _Segments, "_" + sender.GetItemType().Name + "_Composite_", DimensionLookupMode.BySegmentedKeys, false, null);
	...
	public class PXUniqueAttribute : PXEventSubscriberAttribute, IPXRowPersistingSubscriber
	{
		...
		//private PXView _viewSkipCurrent;
		...
		public override void CacheAttached(PXCache sender)
			...
			_fieldType = this.BqlTable.GetNestedType(typeName);
			if (_fieldType == null)
				throw new PXException(MsgNotLocalizable.NoNestedClassInDacForThisField);
			if (!typeof(IBqlField).IsAssignableFrom(_fieldType))
			...
			BqlCommand command = BqlCommand.CreateInstance(
				BqlCommand.Compose(typeof(Select<,>), this.BqlTable,
					typeof(Where<,>), _fieldType, typeof(Equal<>), typeof(Required<>), _fieldType));

			if (WhereCondition != null)
				{
				if (!typeof(WhereBase<,,>).GenericIsAssignableFrom(WhereCondition))
					throw new PXException(String.Format(MsgNotLocalizable.TypeNotBqlWhereCondition, WhereCondition.FullName));
				command = command.WhereAnd(WhereCondition);
	}

			_view = new PXView(sender.Graph, false, command);
		}
