...
namespace PX.Data
{
	...
	public class PXTableAttribute : PXDBInterceptorAttribute
	{
				...
				foreach (string field in keys)
				{
					...
					if (description != null && !String.IsNullOrEmpty(description.FieldName))
					{
						...
						&& origval != null)
						{
							...
							sender.RaiseCommandPreparing(field, row, origval, PXDBOperation.Select, tables[j], out origdescription);
							if (origdescription != null && !String.IsNullOrEmpty(origdescription.FieldName))
							{ PXDataFieldAssign assign = new PXDataFieldAssign(description.FieldName, description.DataType, description.DataLength, description.DataValue, sender.ValueToString(field, val, description.DataValue)); pars[j].Add(assign); pars[j].Add(new PXDataFieldRestrict(origdescription.FieldName, origdescription.DataType, origdescription.DataLength, origdescription.DataValue)); }
							else
                    ...
                    for (int j = 0; j < tables.Length; j++)
                    {
                        if (tableMeet(description, tables[j], dialect))
                        {
                            ...
                            if (description.IsRestriction)
                            {
                                ...
                                {
                                    PXCommandPreparingEventArgs.FieldDescription origdescription;
                                    sender.RaiseCommandPreparing(field, row, origval, PXDBOperation.Update, null,
                                        out origdescription);
                                    if (origdescription != null && !String.IsNullOrEmpty(origdescription.FieldName))
                                    {
                                        PXDataFieldAssign assign = new PXDataFieldAssign(description.FieldName,
                                        ...
                                        pars[j].Add(assign);
                                        pars[j].Add(new PXDataFieldRestrict(origdescription.FieldName,
                                            origdescription.DataType,
                                            origdescription.DataLength, origdescription.DataValue));
                                    }
                                    else
                                    {
                                        pars[j].Add(new PXDataFieldRestrict(description.FieldName, description.DataType,
							...
							else
                            {
                                if (description.IsExcludedFromUpdate)
                                {
                                    if (unchanged != null)
                                    {
                                        var param = new PXDummyDataFieldRestrict(description.FieldName,
		...
		public override bool PersistDeleted(PXCache sender, object row)
		{
			...
			PrepareRestrictionsForDelete(sender, row, tables, dialect, pars);
            for (int j = 0; j < tables.Length - 1; j++)
			{
				foreach (string field in keys)
				{
        ...
        private void PrepareRestrictionsForDelete(PXCache sender, object row, Type[] tables, ISqlDialect dialect, List<PXDataFieldRestrict>[] pars)
        {
            var unchanged = sender.GetOriginal(row);
            bool noteIDRequred = sender._HasKeyValueStored();
            foreach (string field in sender.Fields)
        ...
        private sealed class BqlRowSelection : BqlCommand, IPXExtensibleTableAttribute
		{
			...
			public override void Parse(PXGraph graph, List<IBqlParameter> pars, List<Type> tables, List<Type> fields, List<IBqlSortColumn> sortColumns, StringBuilder text, Selection selection)
			{
				...
				for (int i = 0; i < this.tables.Count; i++)
				{
					...
					text.Append(sqlDialect.quoteDbIdentifier(this.tables[i].Name));
					text.Append(" ");
					text.Append(sqlDialect.quoteDbIdentifier(this.tables[i].Name));
				...
				if (extensions != null)
				{
					foreach (Type ext in extensions)
					{
					    text.Append(!optional.Contains(ext) ? " CROSS JOIN " : " LEFT JOIN ")
					        .Append(sqlDialect.quoteDbIdentifier(ext.Name))
					        .Append(" ")
					        .Append(sqlDialect.quoteDbIdentifier(ext.Name));
				...
				if (single)
				{
				        ...
				        for (int i = 0; i < wherefields.Length; i++)
					{
						string expressionThere;
				            if (selection.preparedColumnsByAlias.TryGetValue(wherefields[i], out expressionThere))
					...
					int j = 0;
				        text.AppendJoined(wherefields, " WHERE ", " AND ",
				            k =>
				                sqlDialect.quoteTableAndColumn(mainTable.Name, wherefields[j]) + " = " +
				                PXDatabase.CreateParameterName(j++));
						...
						if (pdescription != null && !String.IsNullOrEmpty(pdescription.FieldName))
						{
							...
							if (k < bound.Count)
							{
								...
								if (description != null && !String.IsNullOrEmpty(description.FieldName))
								{
								    ...
								    sb.Append(pdescription.FieldName);
								    sb.Append(" = ");
								    sb.Append(description.FieldName);
                                    TableChangingScope.ProcessOns(cache.Graph, new List<Type> {bound[j], bound[k]}, sb, new Selection(), true);
            ...
            for (int i = 0; i < length; i++)
            {
                if (pars[i] != null && pars[i].Count > 0 && !pars[i].Any(_ => fieldMeet(_.FieldName, "NoteID", dialect)))
                {
                    ...
                    if (tc.Fields.Contains("NoteID"))
                    {
                        ...
                        if (!string.IsNullOrEmpty(description?.FieldName) && !description.IsExcludedFromUpdate && !description.IsRestriction)
                        {
                            if (tableMeet(description, tables[i], dialect))
                            {
                                PXDataFieldAssign assign = new PXDataFieldAssign(description.FieldName, description.DataType, description.DataLength, description.DataValue, null);
				...
				if (description != null && !String.IsNullOrEmpty(description.FieldName) )
				{
					for (int j = 0; j < tables.Length; j++)
					{
						if (tableMeet(description, tables[j], dialect))
						{
						    ...
						    {
                                var origval = unchanged.With(c=>sender.GetValue(c, field));
                                var assign = new PXDummyDataFieldRestrict(description.FieldName, description.DataType, description.DataLength, unchanged!=null?origval:description.DataValue);
                                pars[j].Add(assign);
                            }
                            else if (pars[j] != null)
						    {
