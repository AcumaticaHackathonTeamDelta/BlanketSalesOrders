...
using PX.Data.SqlConstruct;
using PX.Data.SqlConstruct.AggregateTree;
using PX.Data.SqlConstruct.CommandTree;
using PX.Data.SqlConstruct.CommandTree.FunctionNodes;
...
namespace PX.Data
{
    ...
    /// 	<code title="Example2" description="The attribute below defines a bound field taking as a value strings of any 8 characters. In the user interface, the input control will show the mask that splits the value into four groups separated by dots." groupname="Example" lang="CS">
    /// [PXDBString(8, InputMask = "CC.CC.CC.CC")]
    /// public virtual string ReportID { get; set; }</code>
	...
	public class PXDBLocalizableStringAttribute : PXDBStringAttribute, IPXRowPersistedSubscriber
	{
			...
			where TField : IBqlField
		{
			...
			for (int i = 0; i < languages.Length; i++)
			{
				...
				{
					translations[i] = PXLocalizer.Localize(message);
				}
			}
			foreach (PXDBLocalizableStringAttribute attribute in
		...
		public static PXDataField GetValueSelect(string tableName, string fieldName, bool isLong)
		{
			...
			for (int i = 0; i < ordered.Count; i++)
			{
				...
				kvList.Add(String.Format("{0} = {1}", sqlDialect.quoteTableAndColumn(extTable, "FieldName"), sqlDialect.quoteLiteral(fieldName + value)));
				kvList.Add(i.ToString());
			}
			string restriction = "";
			...
			if (provider != null)
			{
				...
				{
					restriction = String.Format("({2}.CompanyID IN ({0}, {1}))", listCompanies, cid, sqlDialect.quoteDbIdentifier(extTable));
					multicompany = true;
				}
				else
				{
					restriction = provider.getRestriction(extTable, extTable, true);
				}
			...
			if (isLong)
			{
				bld.Append(sqlDialect.quoteTableAndColumn(extTable, "ValueText"));
			}
			else
			{
				bld.Append(sqlDialect.quoteTableAndColumn(extTable, "ValueString"));
			}
			bld.Append(" FROM ");
			bld.Append(sqlDialect.quoteDbIdentifier(extTable));
			bld.Append(" WHERE ");
			bld.Append(sqlDialect.quoteTableAndColumn(extTable, "RecordID")).Append(" = ").Append(sqlDialect.quoteTableAndColumn(tableName, "NoteID"));
			bld.Append(" AND ");
			bld.Append(sqlDialect.quoteTableAndColumn(extTable, "FieldName")).Append(" IN (");
			for (int i = 0; i < ordered.Count; i++)
			{
				bld.Append(sqlDialect.enquoteValue(fieldName + ordered[i].ToUpper()));
			...
			bld.Append(")");
			bld.Append(restriction);
			bld.Append(" ORDER BY ");
			bld.Append(sqlDialect.functions2sql("switch", kvList.ToArray()));
			if (multicompany)
			{
				bld.Append(", ");
				bld.Append(sqlDialect.quoteDbIdentifier(extTable));
				bld.Append('.');
				bld.Append("CompanyID DESC");
			}

			return new PXDataField("(" + sqlDialect.limitRowsBeingSelected(bld.ToString(), 1) + ")");
		}
		...
		public override void CommandPreparing(PXCache sender, PXCommandPreparingEventArgs e)
		{
			...
			if (_Definition != null)
			{
				if ((e.Operation & PXDBOperation.Command) == PXDBOperation.Select)
				{
					...
					if (description != null)
					{
						...
						if ((e.Operation & PXDBOperation.Option) == PXDBOperation.External)
						{
							...
							string extTable = description.BqlTable.Name + SqlDialectBase.KvExtSuffix;
							List<String> kvList = new List<string>();
							...
							{
								var value = ordered[i].ToUpper();
								kvList.Add(String.Format("{0} = {1}", sqlDialect.quoteTableAndColumn(extTable, "FieldName"), sqlDialect.quoteLiteral((IsProjection ? _DatabaseFieldName : _FieldName) + value)));
								kvList.Add(i.ToString());
							}
							StringBuilder bld = new StringBuilder(BqlCommand.SubSelect);
							...
							bld.Append(sqlDialect.quoteDbIdentifier(extTable));
							bld.Append(" WHERE ");
							bld.Append(sqlDialect.quoteTableAndColumn(extTable, "RecordID")).Append(" = ").Append(sqlDialect.quoteTableAndColumn((e.Table ?? description.BqlTable).Name, sender._NoteIDName));
							bld.Append(" AND ");
							bld.Append(sqlDialect.quoteTableAndColumn(extTable, "FieldName")).Append(" IN (");
							for (int i = 0; i < ordered.Count; i++)
							{
								bld.Append(sqlDialect.enquoteValue((IsProjection ? _DatabaseFieldName : _FieldName) + ordered[i].ToUpper()));
								if (i < ordered.Count - 1)
								{
									bld.Append(", ");
								}
						...
						{
							if (description.BqlTable.IsAssignableFrom(BqlTable))
							{
								StringBuilder bld = new StringBuilder();
								sqlDialect.prepareNoteAttributesJoined(bld, null, description.BqlTable, e.Table);
								if ((e.Operation & PXDBOperation.Option) == PXDBOperation.GroupBy)
								{
									var noteIDColumn = sqlDialect.quoteTableAndColumn((e.Table ?? description.BqlTable).Name, sender._NoteIDName);
									bld.Replace(noteIDColumn, "MAX(" + noteIDColumn + ")");
								}
								e.FieldName = bld.ToString();
							}
						}
				...
				{
					string[] translation = sender._GetTranslations(e.Row, _PositionInTranslations);
					string value;
		...
		public override void RowSelecting(PXCache sender, PXRowSelectingEventArgs e)
		{
			...
			if (_Definition != null)
			{
				...
				string[] translation;
				if (sqlDialect.tryExtractAttributes(firstColumn, _FieldIndexes, out translation))
				{
					string value = null;
					sender._SetTranslations(e.Row, _PositionInTranslations, translation, true);
					string currentLanguage = Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
					if (!String.IsNullOrEmpty(currentLanguage) && _Indexes.ContainsKey(currentLanguage))
					{
        ...
        /// <exclude/>
		protected class OnDemandCommand : BqlCommand
		{
			protected readonly string _Text;
			public OnDemandCommand(string text)
			{
				_Text = text;
			}
			...
			public override BqlCommand WhereAnd<TWhere>()
			{
				return this;
			}
			...
			public override BqlCommand WhereOr<TWhere>()
			{
				return this;
			}
		}
		...
		{
			string[] alternatives = sender._GetTranslations(row, _PositionInTranslations);
			if (alternatives == null)
			{
				if (sender.GetStatus(row) != PXEntryStatus.Inserted)
				{
					if (sender.IsKeysFilled(row) && 
						sender.Keys.All(key => 
						{
							var val = sender.GetValue(row, key) as int?;
							return val == null ? true : val > 0;
						}))
					{
						StringBuilder bld = new StringBuilder();
						ISqlDialect sqlDialect = sender.Graph.SqlDialect;
						sqlDialect.prepareNoteAttributesJoined(bld, null, _BqlTable, null);
						bld.Append(" FROM ");
						bld.Append(sqlDialect.quoteDbIdentifier(_BqlTable.Name));
						List<PXDataValue> pars = new List<PXDataValue>();
						foreach (string descr in sender.Keys)
						{
							PXCommandPreparingEventArgs.FieldDescription description;
							object val = sender.GetValue(row, descr);
							sender.RaiseCommandPreparing(descr, row, val, PXDBOperation.Update, _BqlTable, out description);
							if (description != null && !String.IsNullOrEmpty(description.FieldName) && description.IsRestriction)
							{
								...
								bld.Append(" = ");
								bld.Append(PXDatabase.CreateParameterName(pars.Count));
								pars.Add(new PXDataValue(description.DataType, description.DataLength, description.DataValue));
							}
						}
						OnDemandCommand cmd = new OnDemandCommand(bld.ToString());
						try
						{
							foreach (PXDataRecord record in sender.Graph.ProviderSelect(cmd, 1, pars.ToArray()))
							{
								string firstColumn = record.GetString(0);
								if (sqlDialect.tryExtractAttributes(firstColumn, _FieldIndexes, out alternatives))
								{
									sender._SetTranslations(row, _PositionInTranslations, alternatives, true);
								}
								break;
							}
						}
		...
		{
			sender._SetTranslations(row, _PositionInTranslations, translations);
		}
		...
		protected virtual void Translations_FieldUpdating(PXCache sender, PXFieldUpdatingEventArgs e)
		{
			if ( _Definition != null && e.NewValue is string[])
			{
				if (e.Row != null)
				{
					...
					if (updated)
					{
						sender._SetTranslations(e.Row, _PositionInTranslations, translation);
		...
		public virtual void RowPersisted(PXCache sender, PXRowPersistedEventArgs e)
		{
			if (e.Row != null && _Definition != null)
			{
				if (e.TranStatus == PXTranStatus.Open)
				{
					...
					{
						string[] translation = GetTranslations(sender, e.Row);
						string[] originals = sender._GetTranslations(e.Row, _PositionInTranslations, true);
						if (translation != null)
						{
							string currentLanguage = Thread.CurrentThread.CurrentCulture.TwoLetterISOLanguageName;
							if (String.IsNullOrEmpty(currentLanguage) || !_Indexes.ContainsKey(currentLanguage))
							...
							foreach (KeyValuePair<string, int> pair in _FieldIndexes)
							{
										...
										&& !String.IsNullOrEmpty(val))
								{
									...
									if (description != null && !String.IsNullOrEmpty(description.FieldName) && description.DataValue != null)
									{
										assign = new PXDataFieldAssign(description.FieldName, description.DataType, description.DataLength, description.DataValue);
		...
		public override void FieldUpdating(PXCache sender, PXFieldUpdatingEventArgs e)
		{
			if (_Definition != null && e.Row != null)
			{
				if (e.NewValue is string && ((string)e.NewValue).StartsWith("[{") && ((string)e.NewValue).EndsWith("}]"))
				{
					...
					sender._SetTranslations(e.Row, _PositionInTranslations, incoming);
					sender.SetValue(e.Row, _FieldOrdinal, incoming.FirstOrDefault(_ => _ != null));
					if (sender.GetStatus(e.Row) == PXEntryStatus.Notchanged)
					{
						sender.SetStatus(e.Row, PXEntryStatus.Modified);
		...
		public override void CacheAttached(PXCache sender)
		{
			...
			if (_Definition != null)
			{
				...
				_PositionInTranslations = sender._TranslationsCount;
				sender._TranslationsCount++;
			}
		}
		#endregion
	}
	...
	public class PXDBLocalStringAttribute : PXDBStringAttribute
	{
		...
		protected override void PrepareCommandImpl(string dbFieldName, PXCommandPreparingEventArgs e)
		{
			...
			{
				string tableName = (e.Table == null ? _BqlTable.Name : e.Table.Name);
				e.FieldName = "COALESCE(" + e.SqlDialect.quoteTableAndColumn(tableName, dbFieldName) + ", " 
					+ e.SqlDialect.quoteTableAndColumn(tableName, _DatabaseFieldName) + ")";
			}
		}
		#endregion
	}
    ...
    /// name.</remarks>
    /// <example>
    /// 	<code title="Example" description="The attribute below binds the field to the unique indentifier column and assigns a default value to the field." lang="CS">
    /// [PXDBGuid(true)]
    /// public virtual Guid? SetupID { get; set; }</code>
    /// 	<code title="Example1" description="The attribute below binds the field to the unique indentifier column. The field becomes a key field." groupname="Example" lang="CS">
    /// [PXDBGuid(IsKey = true)]
    /// public virtual Guid? SetupID { get; set; }</code>
    /// </example>
	[AttributeUsage(AttributeTargets.Property | AttributeTargets.Parameter | AttributeTargets.Class | AttributeTargets.Method)]
    ...
    /// name.</remarks>
    /// <example>
    /// 	<code title="Example" description="The attribute below maps a field to the database column and sets the properties inherited from the PXDBField attribute." lang="CS">
	...
	public class PXDBDateAttribute : PXDBFieldAttribute, IPXRowSelectingSubscriber, IPXCommandPreparingSubscriber, IPXFieldUpdatingSubscriber, IPXFieldSelectingSubscriber
	{
        ...
        /// <summary>
        /// 	<para>Gets or sets the value that indicates the database column data type. The following table shows the difference in using the property for MS SQL and
        /// MySQL.</para>
        /// 	<table>
        /// 		<tbody>
        /// 			<tr>
		...
		static SqlCommandsFactory.DatePartFunctionsEnum[] dateParts = new[] { SqlCommandsFactory.DatePartFunctionsEnum.Month,
				SqlCommandsFactory.DatePartFunctionsEnum.Day,
				SqlCommandsFactory.DatePartFunctionsEnum.Hour,
				SqlCommandsFactory.DatePartFunctionsEnum.Quarter };
	    ...
	    private void AddFieldForDatePart(PXCache cache, SqlCommandsFactory.DatePartFunctionsEnum datePart, string datePartName)
	    {
			...
			cache.CommandPreparingEvents.Add(partFieldName, (sender, e) =>
			{
				if (e.IsSelect() && (e.Operation & PXDBOperation.Option) == PXDBOperation.External)
				{
					...
					e.FieldName = new SqlCommandsFactory(e.SqlDialect).DatePartByTimeZone(datePart, fieldName, isNeedConvertTime ? GetTimeZone() : null);
				}
			});
		}
    ...
    /// </remarks>
    /// <example>
    /// 	<code title="Example" description="The code below binds the SunStartTime DAC field to the database column with the same name and sets the default value for the field. Notice the setting of the DisplayMask property inherited from the %PXDBDate:PXDateAttribute% attribute." lang="CS">
			...
			if ((e.Operation & PXDBOperation.Command) == PXDBOperation.Select && (!sender.BypassCalced || sender.BqlSelect != null))
			{
				...
				if (!typeof(IBqlCreator).IsAssignableFrom(_OperandType)) {
					if (String.Compare(_OperandType.Name, _FieldName, StringComparison.OrdinalIgnoreCase) != 0) {
						text.Append(" ").Append(BqlCommand.GetSingleField(_OperandType, sender.Graph, tables, null, BqlCommand.FieldPlace.Select));
					}
					else {
						text.Append(sender.Graph.SqlDialect.quoteTableAndColumn(tables[0].Name, _OperandType.Name));
					}
				}
				else {
					if (_Operand == null) {
				...
				e.BqlTable = _BqlTable;
				if (replace) {
					text.Replace(" " + sender.Graph.SqlDialect.quoteDbIdentifier(sender.GetItemType().Name) + ".",
								 " " + sender.Graph.SqlDialect.quoteDbIdentifier(_BqlTable.Name) + ".");
				}
				...
				if( useFieldText )
				{
					e.FieldName = text.Length == 0 ? String.Empty : "(" + text.ToString() + ")";
					if (_Type.Equals(typeof(Decimal)) && CastToPrecision > 0 && !String.IsNullOrEmpty(e.FieldName))
						e.FieldName = sender.Graph.SqlDialect.castAsDecimal(e.FieldName, CastToPrecision, CastToScale);
				}
				else 
					e.FieldName = BqlCommand.Null;
	...
	public class PXDBCreatedByIDAttribute : PXAggregateAttribute, IPXRowInsertingSubscriber, IPXFieldVerifyingSubscriber
	{
		internal class AuditUserSelectorAttribute : PXDisplaySelectorAttribute
		{
			...
			public override void DescriptionFieldCommandPreparing(PXCache sender, PXCommandPreparingEventArgs e)
			{
				...
				int idx;
				if (!String.IsNullOrWhiteSpace(e.FieldName)
		...
		public sealed class Creator : Users
		{
			...
			/// <exclude/>
			public new abstract class displayName : IBqlField { }
	...
	public class PXDBBinaryAttribute : PXDBFieldAttribute
	{
		...
		protected override void PrepareCommandImpl(string dbFieldName, PXCommandPreparingEventArgs e)
		{
			...
			e.DataValue = e.Value ?? new byte[0];
			if (_Length > -1) {
				e.DataLength = _Length;
			}
		}
	...
	public class PXDBBinaryStringAttribute : PXDBBinaryAttribute
	{
		...
		public override void RowSelecting(PXCache sender, PXRowSelectingEventArgs e)
		{
			if (e.Row != null) {
				...
				sender.SetValue(e.Row, _FieldOrdinal, Encoding.UTF8.GetString(buff));
			}
			e.Position++;
		}
		...
		public override void CommandPreparing(PXCache sender, PXCommandPreparingEventArgs e)
		{
			if (e.Value != null)
			{
				e.Value = !string.IsNullOrEmpty(e.Value.ToString())
				...
				if (_Persisted.TryGetValue(e.Row, out parents) && parents != null && parents is List<object>) {
					foreach (object parent in (List<object>)parents) {
						...
						if (id != null && idintval > 0) {
                            ...
                            foreach (string field in cache.Fields) {
								...
								if (description != null && !String.IsNullOrEmpty(description.FieldName))
                                {
                                        ...
                                        description.DataType != PXDbType.Timestamp)
                                    {
                                        ...
                                        {
                                            string fieldName =
                                                sender.Graph.SqlDialect.unquoteColumn(description.FieldName);
                                        ...
                                        pars.Add(new PXDataFieldRestrict(description.FieldName, description.DataType,
							...
							kv.Check(_BqlTable);
                            PXTransactionScope.SendIdentity(_FieldName, _BqlTable.Name, idintval);
                            PXTransactionScope.SetChildIdentity();
