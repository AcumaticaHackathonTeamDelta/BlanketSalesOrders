...
namespace PX.Data
{
		...
		IPXFieldUpdatingSubscriber
	{
			...
			for (int i = 0; i < segs.Length; i++)
			{
				...
				else if (segs[i].Validate)
				{
					...
					else
					{
						...
						{
							match = false;
							foreach (SegmentValue sv in getOuterSegments(sender, ((SegDescr)segs[i]).SegmentID, val, e.Row))
							{
	    ...
	    internal void CommandPreparing(PXCache sender, PXCommandPreparingEventArgs e)
	    {
		    string val = e.Value as string;
		    string wildcard = e.SqlDialect.WildcardAnything;

			if (!String.IsNullOrEmpty(val) && val.EndsWith(wildcard) && _Dimension != null)
			{
				bool fullLike = val.StartsWith(wildcard);
			    PXSegment[] segments = _Definition.Dimensions[_Dimension];
			    int segStart = 0, segEnd = 0;
			    int valLength;
				if (fullLike)
				{
					valLength = val.Length - 2 * wildcard.Length;
					val = val.Substring(0, valLength + wildcard.Length);
				}
				else
				{
					valLength = val.Length - wildcard.Length;
					val = val.Substring(0, valLength);
				}
				StringBuilder sbld = new StringBuilder(val);
			    foreach (PXSegment seg in segments)
			    {
				    segStart = segEnd;
					segEnd = segStart + seg.Length;
				    if (valLength > segStart && valLength < segEnd)
					    sbld.Append(' ', segEnd - valLength);
			    }
			    sbld.Append(wildcard);

			    e.Value = sbld.ToString();
		    }
	    }
	    ...
	    private sealed class ParentSelectHelper : IBqlTable
	    {
		    ...
		    public abstract class intValue : IBqlField { }
		    #endregion
	    }
		...
		public override void CacheAttached(PXCache sender)
		{
	        ...
	        if (_Definition != null)
	        {
		        ...
		        if (_Definition.LookupModes[_Dimension] == DimensionLookupMode.BySegmentsAndChildSegmentValues)
		        {
			        ...
			        sender.CommandPreparingEvents[_FieldName] += CommandPreparing;
		...
		private void AddFieldForSegment(PXCache cache, int segNumber, PXSegment seg, int startPos)
		{
			...
			cache.CommandPreparingEvents.Add(segFieldName, (sender, e) =>
			{
				if (e.IsSelect() && (e.Operation & PXDBOperation.Option) == PXDBOperation.External)
				{
					...
					string substr = e.SqlDialect.substr(fieldName, (startPos + 1).ToString(), seg.Length.ToString());
					e.FieldName = e.SqlDialect.functions2sql("nullif", new[] { substr, e.SqlDialect.quoteLiteral(String.Empty) });
					e.DataValue = e.Value;
				}
			});
		}
	...
	public class PXDimensionSelectorAttribute : PXAggregateAttribute, IPXFieldVerifyingSubscriber, IPXRowPersistingSubscriber, IPXRowPersistedSubscriber, IPXDependsOnFields 
	{
		...
		protected PXSelectorAttribute SelectorAttribute => (PXSelectorAttribute)_Attributes[_Attributes.Count - 1];
		protected PXDimensionAttribute DimensionAttribute => (PXDimensionAttribute)_Attributes[_Attributes.Count - 2];
		...
		public override void CacheAttached(PXCache sender)
		{
	        ...
	        if (DimensionAttribute.DimensionLookupModeState != null &&
                DimensionAttribute.DimensionLookupModeState == DimensionLookupMode.BySegmentsAndChildSegmentValues
				&& _SubstituteKey != null && !String.Equals(_SubstituteKey.Name, _FieldName, StringComparison.OrdinalIgnoreCase))
	        {
		        sender.CommandPreparingEvents[_SubstituteKey.Name] += DimensionAttribute.CommandPreparing;
	        }
    ...
    #region PXIdentitySubscriberAttribute
    public class PXIdentitySubscriberAttribute : PXEventSubscriberAttribute, IPXRowPersistingSubscriber, IPXRowPersistedSubscriber
    {
        #region State
        protected object _KeyToAbort;
        protected Dictionary<object, object> _Persisted;
        ...
        public PXIdentitySubscriberAttribute(Type keyfield)
        {
            _KeyField = keyfield;
        }
        #endregion
		...
		public virtual void RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
        {
            ...
            if (key != null && Convert.ToInt32(key) < 0)
            {
                PXCache cache = sender.Graph.Caches[BqlCommand.GetItemType(_KeyField)];
                object newkey;
                _KeyToAbort = key;
                if (_Persisted.TryGetValue(key, out newkey))
                {
                ...
                else
                {
                    ...
                    foreach (object data in cache.Inserted)
                    {
                        if (object.Equals(key, cache.GetValue(data, _KeyField.Name)))
                        {
                            try
                            {
                                cache.PersistInserted(data);
                            ...
                            newkey = cache.GetValue(data, _KeyField.Name);
                            if (newkey != null && Convert.ToInt32(newkey) > 0)
                            {
                                sender.SetValue(e.Row, _FieldOrdinal, newkey);
                                if (!object.Equals(_KeyToAbort, newkey))
            ...
            if (_KeyToAbort != null && e.TranStatus != PXTranStatus.Open)
            {
                if (e.TranStatus == PXTranStatus.Aborted)
                {
                    ...
                    if (_Persisted.TryGetValue(_KeyToAbort, out newkey))
                    {
                        ...
                        foreach (object data in cache.Inserted)
                        {
                            if (object.Equals(newkey, cache.GetValue(data, _KeyField.Name)))
                            {
                                try
                                {
                                    cache.RaiseRowPersisted(data, PXDBOperation.Insert, PXTranStatus.Aborted, null);
		...
		public virtual void SubstituteRowPersisting(PXCache sender, PXRowPersistingEventArgs e)
        {
            if ((e.Operation & PXDBOperation.Command) == PXDBOperation.Insert)
            {
                _JustPersistedKey = sender.GetValue(e.Row, _KeyField.Name);
            }
        }
        /// <exclude/>
		public virtual void SubstituteRowPersisted(PXCache sender, PXRowPersistedEventArgs e)
        {
            if ((e.Operation & PXDBOperation.Command) == PXDBOperation.Insert && e.TranStatus == PXTranStatus.Open)
            {
                _SubstitutePersisted[_JustPersistedKey] = sender.GetValue(e.Row, _KeyField.Name);
            }
        }
		...
		public override void CacheAttached(PXCache sender)
        {
            ...
            Type foreignType = BqlCommand.GetItemType(_KeyField);
            sender.Graph.RowPersisting.AddHandler(foreignType, SubstituteRowPersisting);
            sender.Graph.RowPersisted.AddHandler(foreignType, SubstituteRowPersisted);
            if (!sender.Graph.Views.Caches.Contains(foreignType) && !sender.Graph.Views.RestorableCaches.Contains(foreignType))
            {
                sender.Graph.Views.RestorableCaches.Add(foreignType);
