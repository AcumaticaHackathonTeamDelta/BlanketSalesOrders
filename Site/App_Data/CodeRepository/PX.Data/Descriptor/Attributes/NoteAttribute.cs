...
namespace PX.Data
{
    ...
    /// 			<description>
    /// 				<i>Association of
    /// activity items with a data record</i>.</description>
    /// 		</item>
    /// 	</list>
    ...
    /// <example>
    /// 	<para></para>
    /// 	<code title="Example" description="The attribute below indicates that the DAC field references a note." lang="CS">
    /// [PXNote(new Type[0])]
    /// public virtual Guid? NoteID { get; set; }</code>
    /// 	<code title="Example2" description="Here, new Type[0] as parameter is used to force creation of the note on saving of a data record even if the used did not create a note manually.
	...
	public class PXNoteAttribute : PXDBGuidAttribute, IPXRowPersistingSubscriber, IPXRowPersistedSubscriber, IPXRowDeletedSubscriber, IPXReportRequiredField
	{
		...
		protected Note GetNote(PXGraph graph, Guid? id)
		{
			...
			{
				return new Note() { NoteID = result.GetGuid(0), NoteText = result.GetString(1), EntityType = result.GetString(2), GraphType = result.GetString(3), ExternalKey = result.GetString(4) };
			}
			return null;
		}
		...
		protected int GetActivityCount(PXGraph graph, Guid? id)
		{
			...
			if (ActivitiesCountByParent)
			{
				...
				int parentID = (int)helper.GetField(entity, null, EntityHelper.GetIDField(entity.GetType()));

				field = new PXDataFieldValue(graph.SqlDialect.quoteDbIdentifier("CRActivity") + "." + graph.SqlDialect.quoteDbIdentifier("BAccountID"), PXDbType.Int, 4, parentID);
			}
			else
			{
				field = new PXDataFieldValue(graph.SqlDialect.quoteDbIdentifier("CRActivity") + "." + graph.SqlDialect.quoteDbIdentifier("RefNoteID"), PXDbType.UniqueIdentifier, 16, id.Value);
			}

			using (var record = PXDatabase.SelectSingle<CRActivity>(
				new PXDataField("COUNT(*)"),
				field,
				...
				new PXDataFieldValue(graph.SqlDialect.quoteDbIdentifier("CRActivity") + "." + graph.SqlDialect.quoteDbIdentifier("UIStatus"), PXDbType.Char, 2, "CL", PXComp.NE),
				new PXDataFieldValue(graph.SqlDialect.quoteDbIdentifier("CRActivity") + "." + graph.SqlDialect.quoteDbIdentifier("UIStatus"), PXDbType.Char, 2, "RL", PXComp.NE)
				))
			{
				if (record != null)
				{
			...
			foreach (string field in sender.Fields)
			{
				...
				if (description != null && description.IsRestriction && description.DataType != PXDbType.Timestamp
					&& (sender.BqlSelect == null
						|| !String.IsNullOrEmpty(description.FieldName)
						&& (description.FieldName.StartsWith(BqlTable.Name + ".")
						|| description.FieldName.StartsWith(sender.Graph.SqlDialect.quoteDbIdentifier(BqlTable.Name) + "."))))
				{
		...
		public virtual void noteTextFieldSelecting(PXCache sender, PXFieldSelectingEventArgs e)
		{
			...
			if (id.HasValue)
			{
				string result = sender._GetOriginalCounts(e.Row as IBqlTable).Item1;
				if (result == null)
				{
					Note note = GetNote(sender.Graph, id);
					if (note != null)
					{
						e.ReturnValue = NormalizeNoteText(note.NoteText ?? string.Empty);
					}
				}
				else
				{
			...
			{
				e.ReturnState = PXNoteState.CreateInstance(e.ReturnState, _NoteTextField, _FieldName, _NoteTextField, _NoteFilesField, _NoteActivityField, _NoteTextExistsField, _NoteFilesCountField, _NoteTextFieldDisplayName);
			}
		}
			...
			if (e.Row != null)
			{
				...
				else
				{
					...
					if (referencedNoteExists)
					{
						...
						if (note != null)
						{
							note.NoteText = (string)(e.NewValue ?? String.Empty);
					...
					if (!referencedNoteExists && !String.IsNullOrEmpty(e.NewValue as string))
					{
						Note note = new Note();
					...
					foreach (NoteDoc doc in GetDocView(sender.Graph).SelectMulti(id))
					{
						if (doc.FileID != null)
						{
							...
							if (file != null && fileRev != null && !string.IsNullOrEmpty(file.Name))
							{
								...
								ret.Add(fileId + "$" + file.Name + "$" + comment + "$" + size );
							}
						}
					}
					if (ret.Count > 0)
			...
			if (docs != null && e.Row != null)
			{
				...
				if (_PassThrough || !sender.AllowUpdate && sender.GetStatus(e.Row) == PXEntryStatus.Notchanged)
				{
					using (PXTransactionScope tran = new PXTransactionScope())
					{
						if (id == null) {
							id = GenerateId();
							InsertNoteRecord(sender, id.Value, String.Empty);
							sender.SetValue(e.Row, _FieldOrdinal, id);
							updateTableWithId(sender, e.Row, id);
						}

						UpdateNoteRecord(sender, id.Value);
				...
				else
				{
					...
					if (id != null)
					{
						...
						if (washere)
						{
                            ...
                            if (status == PXEntryStatus.Notchanged || status == PXEntryStatus.Held)
		...
		public static void InsertNoteRecord(PXCache sender, Guid id, string text = "")
		{
			try {
				PXDatabase.Insert(typeof(Note),
					...
					new PXDataFieldAssign<Note.entityType>(PXDbType.VarChar, sender.GetItemType().FullName),
					new PXDataFieldAssign<Note.graphType>(PXDbType.VarChar, sender.Graph.GetType().FullName),
        ...
        /// <exclude/>
		public static void UpdateNoteRecord(PXCache sender, Guid id, String text = null)
		{
			Note note = new Note { NoteID = id };
			...
			try {
				...
				if (text != null)
					paramz.Insert(0, new PXDataFieldAssign<Note.noteText>(PXDbType.VarChar, text));
		...
		{
			var subqueryKey = GetValueForSubquery(sender, e, _TextRequired, false);
			if (null != subqueryKey)
			{
				var query = new YaqlTableQuery(new YaqlSchemaTable("Note"));
				query.Columns.Add(new YaqlScalarAlilased(new YaqlColumn("NoteText")));
				query.Condition = Yaql.column("NoteId", "Note").eq(subqueryKey);
				e.FieldName = query.ToSql((PXDatabase.Provider as PXDatabaseProviderBase).getScripter());
			}
			else
			{
		...
		protected virtual void noteActivitiesCommandPreparing(PXCache sender, PXCommandPreparingEventArgs e)
		{
			...
			if (tableFound)
			{
				...
				if (null != subqueryKey)
				{
					var query = new YaqlTableQuery(new YaqlSchemaTable("CRActivity"));
					query.Columns.Add(new YaqlScalarAlilased(Yaql.count(Yaql.columnWildCard())));
					query.Condition = Yaql.and(new List<YaqlCondition>()
					{
						ActivitiesCountByParent
							? Yaql.column("BAccountID", "CRActivity").eq(subqueryKey)
							: Yaql.column("RefNoteID", "CRActivity").eq(subqueryKey),
						//Yaql.column("DeletedDatabaseRecord", "CRActivity").eq(0),
						Yaql.column("UIStatus", "CRActivity").ne("CL"),
						Yaql.column("UIStatus", "CRActivity").ne("RL")
					});
					e.FieldName = query.ToSql((PXDatabase.Provider as PXDatabaseProviderBase).getScripter());
				}
				else
				...
				if (!_BqlTable.IsAssignableFrom(sender.BqlTable))
				{
					...
					else
					{
						...
						if (description != null)
						{
							...
							e.BqlTable = _BqlTable;
							return Yaql.raw(description.FieldName);
						}
					}
				}
		...
		protected virtual void noteFilesCommandPreparing(PXCache sender, PXCommandPreparingEventArgs e)
		{
			...
			if (null != subqueryKey)
			{
				var query = new YaqlTableQuery(new YaqlSchemaTable("NoteDoc"));
				query.Columns.Add(new YaqlScalarAlilased(Yaql.count(Yaql.columnWildCard())));
				query.Condition = Yaql.column("NoteId", "NoteDoc").eq(subqueryKey);
				e.FieldName = query.ToSql((PXDatabase.Provider as PXDatabaseProviderBase).getScripter());
			}
			else
			{
		...
		public override void CommandPreparing(PXCache sender, PXCommandPreparingEventArgs e)
		{
			...
			{
				e.BqlTable = _BqlTable;
				e.FieldName = e.FieldName + ", NULL, NULL, NULL";
			}
		}
		...
		public void SetFilesExists(PXCache sender, object row, int? count)
		{
			...
			sender._SetOriginalCounts((IBqlTable)row, null, count, null);
		}
		...
		{
			if (!(row is IBqlTable)) return;
			sender._SetOriginalCounts((IBqlTable)row, null, null, count);
		}
		...
		{
			if (!(row is IBqlTable)) return;
			sender._SetOriginalCounts((IBqlTable)row, text, null, null);
		}
		...
		public override void RowSelecting(PXCache sender, PXRowSelectingEventArgs e)
		{
			...
			if (e.Row != null)
			{
				...
				if (!id.HasValue || (text == null && count == null)) return;
		...
		private void noteTextExistsFieldSelecting(PXCache sender, PXFieldSelectingEventArgs e)
		{
			...
			if (id.HasValue)
			{
				...
				{
					Note note = GetNote(sender.Graph, id);
					string noteText = (note != null ? note.NoteText : String.Empty) ?? String.Empty;
					string[] split = noteText.Split('\0');
					SetNoteTextExists(sender, e.Row, split.Length > 0 ? (split[0] ?? String.Empty) : String.Empty);
					result = sender._GetOriginalCounts(e.Row as IBqlTable).Item1;
				}
