...
namespace PX.Data
{
    ...
    /// <summary>Sets the user-friendly name of the data access class (DAC).</summary>
    /// <remarks>
    ...
    /// </remarks>
    /// <example>
    /// 	<code title="Example" description="The attibute below adds the upload button to the toolbar of the grid that will use the Transactions view as a data provider." lang="CS">
    ...
    ///        Transactions;</code>
    /// 	<code title="Example2" description="In this example, the primary view DAC is INRegister, and it is passed to the attribute as a parameter.
    /// In the following example, the graph implements the PXImportAttribute.IPXPrepareItems interface to control the data import." groupname="Example" lang="CS">
	...
	public class PXLineNbrAttribute : PXEventSubscriberAttribute, IPXFieldDefaultingSubscriber, IPXRowDeletedSubscriber, IPXRowInsertedSubscriber, IPXRowPersistedSubscriber 
	{
		...
		private short _IncrementStep = 1;
		...
		protected class LastDefault
		{
			...
			{
				value._Value = value._owner.Increment(value._Value, value._owner._IncrementStep);
				return value;
			}

			public static LastDefault operator --(LastDefault value)
			{
				value._Value = value._owner.Decrement(value._Value, value._owner._IncrementStep);
				return value;
			}
		...
		{
			get { return this._IncrementStep; }
			set { this._IncrementStep = value; }
		}
		...
		public virtual void RowInserted(PXCache sender, PXRowInsertedEventArgs e)
		{
			if (_sourceField != null)
			{
				...
				{
					object currentVal = cache.GetValue(cache.Current, _sourceField);
		...
		{
			if (DecrementOnDelete && _sourceField != null)
			{
				PXCache cache = sender.Graph.Caches[_sourceType];
				if (cache.Current != null)
				{
		...
		public override void CacheAttached(PXCache sender)
		{
			...
			sender.Graph.OnClear += (graph_, option) => ClearLastDefaultValue();
		}
		...
		public object NewLineNbr(PXCache sender, object Current)
		{
			...
			if (Current != null)
			{
			    ...
			    {
                    PXCache cache = sender.Graph.Caches[_sourceType];
                    currentVal = Increment(cache.GetValue(Current, _sourceField), _IncrementStep);
			    ...
			    else
			    {
                    foreach (object data in PXParentAttribute.SelectChildren(sender, Current, _sourceType))
                    {
                        ...
                        if (((IComparable)lastLineNbr).CompareTo(currentVal) > 0)
                        {
                            currentVal = lastLineNbr;
                        }
                    }
			        currentVal = Increment(currentVal, _IncrementStep);
			    }
            }
			return currentVal;
		}
		...
		public virtual void FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
		{
			...
			else
			{
				if (_sourceField != null)
				{
					...
					if (cache.Current != null)
					{
						_LastDefaultValue.Value = cache.GetValue(cache.Current, _sourceField);
					}
				}
				else 
				{
    ...
    /// <exclude/>
    [AttributeUsage(AttributeTargets.Property, AllowMultiple = false)]
    public sealed class PXRoleRightAttribute : PXIntListAttribute
    {
        ...
        public override void FieldSelecting(PXCache sender, PXFieldSelectingEventArgs e)
        {
			...
			string identifier = ListRoleRight.GetRoleRightIdentifier(row, sender.Graph);
