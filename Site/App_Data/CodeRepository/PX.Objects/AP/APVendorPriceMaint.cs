...
namespace PX.Objects.AP
{
	public class APVendorPriceMaint : PXGraph<APVendorPriceMaint>//, PXImportAttribute.IPXPrepareItems
	{
		...
		/// <param name="date">Date</param>
		/// <returns>Unit Cost</returns>
		public static decimal? CalculateUnitCost(PXCache sender, int? vendorID, int? vendorLocationID, int? inventoryID, int? siteID, CurrencyInfo currencyinfo, string UOM, decimal? quantity, DateTime date, decimal? currentUnitCost, bool alwaysFromBaseCurrency = false)
		...
		public virtual decimal? CalculateUnitCostInt(PXCache sender, int? vendorID, int? vendorLocationID, int? inventoryID, int? siteID, CurrencyInfo currencyinfo, string UOM, decimal? quantity, DateTime date, decimal? currentUnitCost, bool alwaysFromBaseCurrency = false)
		{
			...
			UnitCostItem ucItem = FindUnitCostInt(sender, vendorID, vendorLocationID, inventoryID, siteID, currencyinfo.BaseCuryID, alwaysFromBaseCurrency ? currencyinfo.BaseCuryID : currencyinfo.CuryID, Math.Abs(quantity ?? 0m), UOM, date);
		...
		{
			PXSelectBase<APVendorPrice> unitCost = 
				new PXSelect<APVendorPrice,
				Where<APVendorPrice.inventoryID, Equal<Required<APVendorPrice.inventoryID>>,
					...
					And<APVendorPrice.curyID, Equal<Required<APVendorPrice.curyID>>,
					And2<Where<APVendorPrice.siteID, Equal<Required<APVendorPrice.siteID>>, Or<APVendorPrice.siteID, IsNull>>,
					And<APVendorPrice.uOM, Equal<Required<APVendorPrice.uOM>>,
				...
				OrderBy<Desc<APVendorPrice.isPromotionalPrice,
						...
						Desc<APVendorPrice.breakQty>>>>>>(sender.Graph);

			PXSelectBase<APVendorPrice> unitCostBaseUOM = 
				new PXSelectJoin<APVendorPrice, 
				InnerJoin<InventoryItem, On<InventoryItem.inventoryID, Equal<APVendorPrice.inventoryID>, And<InventoryItem.baseUnit, Equal<APVendorPrice.uOM>>>>,
				Where<APVendorPrice.inventoryID, Equal<Required<APVendorPrice.inventoryID>>,
					And<APVendorPrice.vendorID, Equal<Required<APVendorPrice.vendorID>>,
					And<APVendorPrice.curyID, Equal<Required<APVendorPrice.curyID>>,
					And2<Where<APVendorPrice.siteID, Equal<Required<APVendorPrice.siteID>>, Or<APVendorPrice.siteID, IsNull>>,
			...
			APVendorPrice item = unitCost.SelectWindowed(0, 1, inventoryID, vendorID, curyID, siteID, UOM, quantity, date, date, date, date);
			if (item != null)
				return new UnitCostItem(UOM, item.SalesPrice ?? 0, item.CuryID);
			
			decimal baseUnitQty = INUnitAttribute.ConvertToBase(sender, inventoryID, UOM, (decimal)quantity, INPrecision.QUANTITY);
			item = unitCostBaseUOM.Select(inventoryID, vendorID, curyID, siteID, baseUnitQty, date, date, date, date);
			if (item != null)
				return new UnitCostItem(item.UOM, item.SalesPrice ?? 0, item.CuryID);
