...
namespace PX.Objects.AR
{
	...
	public class ARDocumentRelease : PXGraph<ARDocumentRelease>
	{
		...
		protected virtual IEnumerable ardocumentlist()
		{
			PXSelectBase<BalancedARDocument> selectDocumentsCommand = new PXSelectJoinGroupBy<BalancedARDocument,
				...
				Where2<
					...
					And2<Where<
						...
						And<BalancedARDocument.origModule, Equal<BatchModule.moduleAR>, 
						Or<
							BalancedARDocument.openDoc, Equal<True>, 
							And<ARAdjust.adjdRefNbr, IsNotNull,
							And<ARAdjust.isInitialApplication, NotEqual<True>>>>>>,
					And<ARRegister.isMigratedRecord, Equal<Current<ARSetup.migrationMode>>>>>>>>>,
				Aggregate<
			...
			foreach (PXResult<BalancedARDocument, Customer, ARAdjust, ARInvoice, ARPayment> res in
                selectDocumentsCommand.View.Select(
					null, 
					null, 
					PXView.Searches,
	...
	public class ARReleaseProcess : PXGraph<ARReleaseProcess>
	{
		...
		public ARReleaseProcess()
		{
			...
			PXDBDefaultAttribute.SetDefaultForUpdate<ARAdjust.adjgFinPeriodID>(cacheARAdjust, null, false);
			PXDBDefaultAttribute.SetDefaultForUpdate<ARAdjust.adjgTranPeriodID>(cacheARAdjust, null, false);
			...
			PXDBDefaultAttribute.SetDefaultForUpdate<ARTran.finPeriodID>(cacheARTran, null, false);
			...
			if (IsMigrationMode == true)
			{
				...
				PXDBDefaultAttribute.SetDefaultForInsert<ARAdjust.adjgFinPeriodID>(cacheARAdjust, null, false);
				PXDBDefaultAttribute.SetDefaultForInsert<ARAdjust.adjgTranPeriodID>(cacheARAdjust, null, false);
			}
		}
		...
		private void CreatePayment(PXResult<ARInvoice, CurrencyInfo, Terms, Customer> res, ref List<ARRegister> ret)
		{
			...
			if (invoicesNotCSL.Count() > 0)
			{
				foreach (SOInvoice soinvoice in invoicesNotCSL)
				{
					...
					CCProcTranHelper.UpdateCapturedState<SOInvoice>(soinvoice, cmd.Select(soinvoice.DocType, soinvoice.RefNbr, soinvoice.DocType, soinvoice.RefNbr));
		...
		{
			return ReleaseInvoice(je, ref doc, res, out pmDoc);
		}
			...
			out PMRegister pmDoc)
		{
			...
			if (doc.Released != true)
			{
				...
				foreach (PXResult<ARTran, ARTax, Tax, DRDeferredCode, SO.SOOrderType> r in ARTran_TranType_RefNbr.Select((object)ardoc.DocType, ardoc.RefNbr))
				{
					ARTran n = r;
					...
					if (_IsIntegrityCheck == false && sotype.PostLineDiscSeparately == true && sotype.DiscountAcctID != null && n.CuryDiscAmt > 0.00005m && !object.Equals(prev_n, n))
					{
						...
						new_tran.AccountID = GetDefaultTradeDiscountAccountID(sotype, n, new_tran, ardoc);
				...
				List<PMTran> reverseToNonBillable = new List<PMTran>();
				List<Tuple<PMProformaTransactLine, PMTran>> billLaterNotAllocated = new List<Tuple<PMProformaTransactLine, PMTran>>();
				...
				if (proforma != null)
				{
					...
					var select = new PXSelectJoin<PMTran, 
						...
						And<PMProformaTransactLine.option, NotEqual<PMProformaLine.option.bill>,
						And<PMTran.reversed, NotEqual<True>>>>>>>(this);
					...
					foreach(PXResult<PMTran, PMProformaTransactLine> res2 in select.Select(proforma.RefNbr))
					{
						...
						{
							reverseToNonBillable.Add(pmt);
						}
						else if (line.Option == PMProformaLine.option.HoldRemainder && line.LineTotal < line.BillableAmount && string.IsNullOrEmpty(pmt.AllocationID))
						{
					...
					billLaterNotAllocated.AddRange(billLaterLinesWithFirstTransaction.Values);
				}
				...
				if (doc.DocType == ARDocType.CreditMemo && doc.OrigDocType == ARDocType.Invoice) //credit memo reversal only for Invoice created by PM billing. CreditMemo created by PM Billing should be handled same as INV.
				{
					...
					var selectOriginalBilledTrans = new PXSelect<PMTran, Where<PMTran.aRTranType, Equal<Required<PMTran.aRTranType>>,
						And<PMTran.aRRefNbr, Equal<Required<PMTran.aRRefNbr>>,
						And<PMTran.taskID, IsNotNull>>>,
						OrderBy<Asc<ARTran.lineNbr>>>(this);


					foreach (PMTran pmtran in selectOriginalBilledTrans.Select(doc.OrigDocType, doc.OrigRefNbr))
					{
						string key = string.Format("{0}.{1}", pmtran.TaskID, pmtran.InventoryID.GetValueOrDefault(PMInventorySelectorAttribute.EmptyInventoryID));
				...
				foreach (PXResult<ARTran, ARTax, Tax, DRDeferredCode, SO.SOOrderType> r in ARTran_TranType_RefNbr.Select((object)ardoc.DocType, ardoc.RefNbr))
				{
					...
					{
						GLTran tran = new GLTran();
						tran.SummPost = this.SummPost && n.PMDeltaOption != ARTran.pMDeltaOption.BillLater;
						...
						{
							if (n.PMDeltaOption == ARTran.pMDeltaOption.CompleteLine)
							{
								foreach (PMTran pmtran in linkedTransactions)
								{
									if (pmtran.Reverse != PM.PMReverse.OnInvoice)
										continue;

									if (pmtran.IsNonGL != true && pmtran.OffsetAccountID != pmtran.AccountID)
									{
										tran.PMTranID = null;
							...
							else if (n.PMDeltaOption == ARTran.pMDeltaOption.BillLater)
							{
								//if transaction is allocated there will be only 1 linked transaction.
								if (linkedTransactions.Count == 1 && !string.IsNullOrEmpty(linkedTransactions[0].AllocationID))
								{
									PMTran pmtran = linkedTransactions[0];

									if (pmtran.Reverse == PM.PMReverse.OnInvoice)
									{
										if (pmtran.IsNonGL != true)
										{
											if (pmtran.OffsetAccountID != pmtran.AccountID)
											{
												tran.PMTranID = null;
												tran.OrigPMTranID = pmtran.TranID;
												tran.TranLineNbr = null;
				...
				if (_IsIntegrityCheck == false)
				{
					...
					else if (nonglBillLater.Count > 0 || nonglReverse.Count > 0 || billLaterNotAllocated.Count > 0 || reverseToNonBillable.Count > 0)
					{
						PM.PMAllocator allocator = PXGraph.CreateInstance<PM.PMAllocator>();
						allocator.NonGlBillLaterAndReverse(doc.RefNbr, nonglBillLater, nonglReverse, billLaterNotAllocated, reverseToNonBillable);
						allocator.Actions.PressSave();
						pmDoc = allocator.Document.Current;
					}
				}
		...
		public virtual void ReleasePayment(
			...
			PXResult<ARPayment, CurrencyInfo, Currency, Customer, CashAccount> res)
		{
			ReleasePayment(je, ref doc, res, doc.AdjCntr);
			...
			int? AdjNbr)
		{
			...
			PXResultset<ARAdjust> adjustments = ARAdjustsToRelease ?? ARAdjust_AdjgDocType_RefNbr_CustomerID.Select(doc.DocType, doc.RefNbr, AdjNbr);
			Tuple<ARAdjust, CurrencyInfo> lastAdjustment = ProcessAdjustments(je, adjustments, doc, ardoc, vend, new_info, paycury);

			ARAdjust prev_adj = lastAdjustment.Item1;
			CurrencyInfo prev_info = lastAdjustment.Item2;

			if (_IsIntegrityCheck == false && ardoc.VoidAppl != true && doc.CuryDocBal < 0m)
			{
				throw new PXException(Messages.DocumentBalanceNegative);
			}
			...
			Currency paycury)
		{
			...
			foreach (PXResult<ARAdjust, CurrencyInfo, Currency, ARRegister, ARPayment> adjres in 
				adjustments.Where(res => ((ARAdjust)res).IsInitialApplication != true))
			{
				ARAdjust adj = adjres;
		...
		protected virtual void ProcessAdjustmentsOnlyAdjusted(JournalEntry je, PXResultset<ARAdjust> adjustments)
		{
			...
			// for migrated document - should be excluded from the processing
			//
			foreach (PXResult<ARAdjust, CurrencyInfo, Currency, ARInvoice, ARPayment> adjres in
				adjustments.Where(res => ((ARAdjust)res).IsInitialApplication != true))
			{
				ARAdjust adj = adjres;
				CurrencyInfo vouch_info = adjres;
				...
				CurrencyInfo payment_info = PXSelect<CurrencyInfo, 
					Where<CurrencyInfo.curyInfoID, Equal<Required<CurrencyInfo.curyInfoID>>>>.Select(this, adjgdoc.CuryInfoID);
		...
		public virtual List<ARRegister> ReleaseDocProc(JournalEntry je, ARRegister ardoc, List<Batch> pmBatchList, ARDocumentRelease.ARMassProcessReleaseTransactionScopeDelegate onreleasecomplete)
		{
			...
			ARRegister doc = PXCache<ARRegister>.CreateCopy(ardoc);
				...
				foreach (PXResult<ARInvoice, CurrencyInfo, Terms, Customer, Account> res in ARInvoice_DocType_RefNbr.Select(doc.DocType, doc.RefNbr))
					...
					// Must check for CM application in different period
						...
						invoice.Released == false)
						{
							...
							new PXResult<ARInvoice, CurrencyInfo, Terms, Customer, Account>(ie.Document.Current, currencyinfo, (Terms)res, customer, (Account)res), out pmDoc);
							}
						}
						else
						{
						ret = ReleaseInvoice(je, ref doc, res, out pmDoc);
						}
				...
				foreach (PXResult<ARPayment, CurrencyInfo, Currency, Customer, CashAccount> res in ARPayment_DocType_RefNbr.Select(doc.DocType, doc.RefNbr, doc.CustomerID))
					...
					if ((doc.DocType == ARDocType.Payment || doc.DocType == ARDocType.VoidPayment || doc.DocType == ARDocType.Refund) && 
						doc.Released != true)
						{
						SegregateBatch(je, doc.BranchID, doc.CuryID, ((ARPayment)res).DocDate, ((ARPayment)res).FinPeriodID, doc.DocDesc, info);

						if (_IsIntegrityCheck == false && customer.AutoApplyPayments == true && payment.DocType == ARDocType.Payment && payment.Released != true)
							{
								pe.Clear();
								bool anyAdj = false;
							...
							ReleasePayment(je, ref doc, 
							new PXResult<ARPayment, CurrencyInfo, Currency, Customer, CashAccount>(pe.Document.Current, info, (Currency)res, customer, (CashAccount)res), -1);
							}
							else
							{
				...
				using (PXTransactionScope ts = new PXTransactionScope())
				{
					...
					foreach (ARRegister custdoc in custdocs)
					{
						...
						foreach (PXResult<ARInvoice, CurrencyInfo, Terms, Customer, Account> res in ARInvoice_DocType_RefNbr.Select((object)doc.DocType, doc.RefNbr))
						{
							...
							{
								PM.PMRegister pmDoc;
								ReleaseInvoice(je, ref doc, res, out pmDoc);
							}
							doc.Released = true;
						}
						...
						foreach (PXResult<ARInvoice, CurrencyInfo, Terms, Customer, Account> res in ARInvoice_DocType_RefNbr.Select((object)doc.DocType, doc.RefNbr))
						{
							...
							var adjustments = PXSelectJoin<ARAdjust,
								...
								Where<ARAdjust.adjdDocType, Equal<Required<ARAdjust.adjdDocType>>,
									...
									And<ARAdjust.released, Equal<True>>>>>>>.Select(this, invoice.DocType, invoice.RefNbr, invoice.CustomerID);
