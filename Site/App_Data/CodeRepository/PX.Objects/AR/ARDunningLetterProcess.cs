...
namespace PX.Objects.AR
{
	public class ARDunningLetterProcess : PXGraph<ARDunningLetterProcess>
	{
		...
		[PXProjection(typeof(Select5<Standalone.ARInvoice,
                    ...
                    LeftJoin<RUTROT.RUTROT, 
                        On<RUTROT.RUTROT.docType, Equal<ARRegister.docType>,
                        And<RUTROT.RUTROT.refNbr, Equal<ARRegister.refNbr>>>>>>>>,
					Aggregate<
		...
		public partial class ARInvoiceWithDL : IBqlTable
		{
			...
			#endregion
			#region Revoked
			public abstract class revoked : IBqlField
			{
			}
			...
			public abstract class dunningLetterLevel : IBqlField
			{
			}
			...
			public abstract class dunningLetterDate : IBqlField
			{
			}
			...
			#endregion
			#region CuryTotalAmtRUTROT
			public abstract class curyTotalAmtRUTROT : IBqlField
			{
			}
            [PXDBDecimal(BqlField = typeof(RUTROT.RUTROT.curyTotalAmt))]
			public virtual decimal? CuryTotalAmtRUTROT
			{
				get;
				set;
			}
			#endregion
			#region DLReleased
			public abstract class dLReleased : IBqlField
			{
			}
		...
		public partial class ARDunningLetterRecordsParameters : IBqlTable
		{
			...
			public abstract class includeNonOverdueDunning : IBqlField
			{
			}
			[PXDBBool]
			...
			#endregion
			#region IncludeType
			public abstract class includeType : IBqlField
			{
			}
			[PXInt]
			...
			#endregion
			#region LevelFrom
			public abstract class levelFrom : IBqlField
			{
			}
			[PXInt]
			...
			#endregion
			#region LevelTo
			public abstract class levelTo : IBqlField
			{
			}
			[PXInt]
		...
		public partial class ARDunningLetterList : IBqlTable
		{
			...
			[PXDBInt(IsKey = true)]
			[PXDefault]
			[Branch(typeof(GL.Branch.branchID))]
			...
			public abstract class numberOfDocuments : IBqlField
			{
			}
			[PXInt]
			...
			public abstract class numberOfOverdueDocuments : IBqlField
			{
			}
			[PXInt]
			[PXDefault(0, PersistingCheck = PXPersistingCheck.Nothing)]
			...
			public abstract class lastDunningLetterDate : IBqlField
			{
			}
			[PXDBDate(IsKey = true)]
			[PXDefault]
			...
			#endregion
			#region DueDays
			public abstract class dueDays : IBqlField
			{
			}
			[PXDBInt]
			[PXDefault]
		...
		private static ARDunningLetterDetail CreateDunningLetterDetail(DateTime? docDate, bool processByCutomer, ARInvoiceWithDL invoice, List<int> dueDaysByLevel)
		{
			...
			detail.DocBal = invoice.DocBal - (invoice.CuryTotalAmtRUTROT ?? decimal.Zero);
