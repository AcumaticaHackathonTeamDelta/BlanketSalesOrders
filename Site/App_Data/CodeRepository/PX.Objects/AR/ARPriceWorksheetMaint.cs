...
namespace PX.Objects.AR
{
    ...
    public class ARPriceWorksheetMaint : PXGraph<ARPriceWorksheetMaint, ARPriceWorksheet>
	{
		...
		public ARPriceWorksheetMaint()
        {
			...
			PXUIFieldAttribute.SetVisible<ARPriceWorksheetDetail.alternateID>(Details.Cache, null, _loadSalesPricesUsingAlternateID);
		}
        ...
        public static void ReleaseWorksheet(ARPriceWorksheet priceWorksheet)
        {
            ...
            {
                foreach (PXResult<ARPriceWorksheetDetail, InventoryItem> row in salesPriceWorksheetMaint.Details.Select())
				{
					ARPriceWorksheetDetail priceLine = row;
					InventoryItem item = row;

					PXResultset<ARSalesPrice> salesPrices = 
						PXSelect<ARSalesPrice, 
						Where<
							ARSalesPrice.priceType, Equal<Required<ARSalesPrice.priceType>>,
							And2<Where2<
								Where<ARSalesPrice.customerID, Equal<Required<ARSalesPrice.customerID>>, And<ARSalesPrice.priceType, Equal<PriceTypes.customer>>>,
							...
							And2<Where<
								ARSalesPrice.siteID, Equal<Required<ARSalesPrice.siteID>>,
								Or<ARSalesPrice.siteID, IsNull, And<Required<ARSalesPrice.siteID>, IsNull>>>,
						...
						OrderBy<Asc<ARSalesPrice.effectiveDate, Asc<ARSalesPrice.expirationDate>>>>.Select(
							salesPriceWorksheetMaint,
							priceLine.PriceType,
							...
							priceLine.CuryID,
							priceLine.BreakQty,
							salesPriceWorksheetMaint.Document.Current.IsPromotional ?? false);
                    ...
                    if (salesPriceWorksheetMaint.Document.Current.IsPromotional != true || salesPriceWorksheetMaint.Document.Current.ExpirationDate == null)
                    {
                        ...
                        if (salesPrices.Count > 0)
                        {
							...
							if (insertNewPrice)
							{
								...
								{
									salesPriceWorksheetMaint.ARSalesPrices.Insert(
										CreateSalesPrice(
											priceLine,
											false,
											salesPriceWorksheetMaint.Document.Current.EffectiveDate,
											salesPriceWorksheetMaint.Document.Current.ExpirationDate));
								}
								else
								{
									ARSalesPrice minSalesPrice = 
										PXSelect<ARSalesPrice, 
										Where<
											ARSalesPrice.priceType, Equal<Required<ARSalesPrice.priceType>>,
											And2<Where2<
												Where<ARSalesPrice.customerID, Equal<Required<ARSalesPrice.customerID>>, And<ARSalesPrice.custPriceClassID, IsNull>>,
										...
										OrderBy<Asc<ARSalesPrice.effectiveDate>>>.SelectSingleBound(
											salesPriceWorksheetMaint,
											new object[] { },
											priceLine.PriceType, 
											priceLine.CustomerID, 
									...
									salesPriceWorksheetMaint.ARSalesPrices.Insert(
										CreateSalesPrice(
											...
											false,
											salesPriceWorksheetMaint.Document.Current.EffectiveDate,
											minSalesPrice?.EffectiveDate?.AddDays(-1) ?? salesPriceWorksheetMaint.Document.Current.ExpirationDate));
								}
							}
						}
						else
					...
					if (salesPriceWorksheetMaint._loadSalesPricesUsingAlternateID && priceLine.AlternateID.IsNullOrEmpty() == false)
					{
						bool xRefExists = PriceWorksheetAlternateItemAttribute.XRefsExists(salesPriceWorksheetMaint.Details.Cache, priceLine);
						if (xRefExists == false)
						{
							PXCache xRefCache = item.StkItem == true
								? salesPriceWorksheetMaint.StockCrossReferences.Cache
								: salesPriceWorksheetMaint.NonStockCrossReferences.Cache;
            ...
            if ((addItemParameters.Current.PriceType != PriceTypes.BasePrice && addItemParameters.Current.PriceCode != null) || (addItemParameters.Current.PriceType == PriceTypes.BasePrice && addItemParameters.Current.PriceCode == null))
            {
                foreach (ARAddItemSelected line in addItemLookup.Cache.Cached)
                {
                    if (line.Selected == true)
                    {
	                    ...
	                    var newline = new ARPriceWorksheetDetail
									{
                        ...
                        PXCache<ARPriceWorksheetDetail>.CreateCopy(Details.Update(newline));
                    }
                }
		...
		public static void CopyPricesProc(ARPriceWorksheet priceWorksheet, CopyPricesFilter copyFilter)
		{
			...
			PXResultset<ARSalesPrice> salesPrices = 
				...
				Where<
					...
					And2<Where2<
						Where<
							ARSalesPrice.customerID, Equal<Required<ARSalesPrice.customerID>>,
							And<ARSalesPrice.custPriceClassID, IsNull>>,
						Or2<Where<
							ARSalesPrice.custPriceClassID, Equal<Required<ARSalesPrice.custPriceClassID>>,
							And<ARSalesPrice.customerID, IsNull>>,
						Or<Where<ARSalesPrice.custPriceClassID, IsNull, And<ARSalesPrice.customerID, IsNull>>>>>,
					And<ARSalesPrice.curyID, Equal<Required<ARSalesPrice.curyID>>,
					And2<AreSame<ARSalesPrice.siteID, Required<ARSalesPrice.siteID>>,
					And<ARSalesPrice.isPromotionalPrice, Equal<Required<ARSalesPrice.isPromotionalPrice>>,
					And<Where2<
						Where<
				...
				OrderBy<Asc<ARSalesPrice.effectiveDate, Asc<ARSalesPrice.expirationDate>>>>
				.Select(
					salesPriceWorksheetMaint,
		...
		private void CalculatePendingPrices(CalculatePricesFilter settings)
		{
			...
			foreach (ARPriceWorksheetDetail sp in Details.Select())
			{
				...
				INItemSite iis = PXSelect<INItemSite, Where<
					INItemSite.inventoryID, Equal<Required<INItemSite.inventoryID>>, 
					And<INItemSite.siteID, Equal<Required<INItemSite.siteID>>>>>
					.SelectWindowed(this, 0, 1, sp.InventoryID, sp.SiteID);

				decimal? lastCost = iis?.LastCost ?? itemCost.LastCost;
				decimal? avgCost = iis?.AvgCost ?? itemCost.AvgCost;
				decimal? stdCost = iis?.StdCost ?? item.StdCost;
				decimal? recCost = iis?.RecPrice ?? item.RecPrice;

				decimal markup = (iis?.MarkupPct ?? item.MarkupPct ?? 0) * 0.01m;
				...
				Func<decimal, decimal> applyConversion =
					...
					{
						decimal? result;
						return item.BaseUnit != sp.UOM && TryConvertToBase(Caches[typeof(InventoryItem)], item.InventoryID, sp.UOM, amt, out result)
							? result.Value
							: amt;
					};
				...
				switch (settings.PriceBasis)
				{
					...
					case PriceBasisTypes.StdCost:
					{
						...
						{
							skipUpdate = getSkipUpdate(stdCost);
							decimal correctedAmtInBaseUnit = applyMarkUp(stdCost ?? 0);
							correctedAmt = applyConversion(correctedAmtInBaseUnit);
						}
						else
						{
							skipUpdate = getSkipUpdate(avgCost);
							decimal correctedAmtInBaseUnit = applyMarkUp(avgCost ?? 0);
							correctedAmt = applyConversion(correctedAmtInBaseUnit);
						}
					...
					case PriceBasisTypes.PendingPrice:
						skipUpdate = getSkipUpdate(sp.PendingPrice);
						correctedAmt = sp.PendingPrice ?? 0m;
						break;
					case PriceBasisTypes.CurrentPrice:
						skipUpdate = getSkipUpdate(sp.CurrentPrice);
				...
				if (!skipUpdate)
				{
					...
					var u = (ARPriceWorksheetDetail)Details.Cache.CreateCopy(sp);
		...
		private void CheckForEmptyPendingPriceDetails()
		{
			...
			foreach (ARPriceWorksheetDetail emptyPendingPriceDetail in emptyPendingPriceDetails)
			{
				...
				// -
				Details.Cache.SmartSetStatus(emptyPendingPriceDetail, PXEntryStatus.Updated);
			}
		}
