...
namespace PX.Objects.AR
{
	public class ARSalesPriceMaint : PXGraph<ARSalesPriceMaint>
	{
		...
		public virtual IEnumerable records()
		{
			...
			foreach (PXResult<ARSalesPrice> res in qSelect)
			{
					...
					Where<InventoryItem.itemStatus, NotEqual<INItemStatus.inactive>,
						...
						And<Where2<Where<Required<ARSalesPriceFilter.itemClassCD>, IsNull,
							...
							And2<Where<Required<ARSalesPriceFilter.myWorkGroup>, Equal<False>,
										Or<InventoryItem.priceWorkgroupID, InMember<CurrentValue<ARSalesPriceFilter.currentOwnerID>>>>,
							And<Where<Required<ARSalesPriceFilter.workGroupID>, IsNull,
								Or<Required<ARSalesPriceFilter.workGroupID>, Equal<InventoryItem.priceWorkgroupID>>>>>>>>>>>>>>>>>.GetCommand());
		...
		public override object GetStateExt(string viewName, object data, string fieldName)
		{
				...
				|| viewName.Contains("PriceCode")) && ViewNames.ContainsKey(CustomerCode.View) && ViewNames.ContainsKey(CustPriceClassCode.View))
			{
				if (GetPriceType(viewName) == PriceTypeList.Customer)
				{
					...
					if (fieldName == typeof(ARSalesPrice.priceCode).Name)
					{
						return base.GetStateExt(customerCodeViewName, data, typeof(CR.BAccount.acctCD).Name);
					}
					else if (fieldName == typeof(ARSalesPrice.description).Name)
					{
		...
		public virtual decimal? CalculateSalesPriceInt(PXCache sender, string custPriceClass, int? customerID, int? inventoryID, int? siteID, CurrencyInfo currencyinfo, decimal? quantity, string UOM, DateTime date, bool alwaysFromBaseCurrency)
		{
			...
			try
			{
				spItem = FindSalesPrice(sender, custPriceClass, customerID, inventoryID, siteID, currencyinfo.BaseCuryID, alwaysFromBaseCurrency ? currencyinfo.BaseCuryID : currencyinfo.CuryID, Math.Abs(quantity ?? 0m), UOM, date);
			}
			catch (PXUnitConversionException)
			{
				return null;
		...
		public virtual SalesPriceItem FindSalesPrice(PXCache sender, string custPriceClass, int? customerID, int? inventoryID, int? siteID, string baseCuryID, string curyID, decimal? quantity, string UOM, DateTime date)
		{
			var salesPriceSelect =
				new SalesPriceSelect(sender, inventoryID, UOM, (decimal) quantity)
				{
			...
			return priceForCurrentUOM.SelectCustomerPrice()
				...
				?? SelectDefaultItemPrice(sender, inventoryID, baseCuryID)
				?? priceForSalesUOM.SelectCustomerPrice()
				?? priceForSalesUOM.SelectBasePrice();
		}

		private static SalesPriceItem SelectDefaultItemPrice(PXCache sender, int? inventoryID, string baseCuryID)
		{
			InventoryItem inventoryItem =
				PXSelect<InventoryItem, Where<InventoryItem.inventoryID, Equal<Required<InventoryItem.inventoryID>>>>.Select(sender.Graph, inventoryID);
			return inventoryItem?.BasePrice != null && inventoryItem.BasePrice != 0
				? new SalesPriceItem(inventoryItem.BaseUnit, inventoryItem.BasePrice ?? 0m, baseCuryID)
				: null;
		}

		private class SalesPriceSelect
		{
			...
			protected ARSalesPrice SelectCustomerPrice(decimal quantity, params object[] additionalParams)
			{
				return SelectCommand
					.SelectSingle(
						new object[] { InventoryID, CustomerID, CustPriceClass, CustomerID, CustPriceClass, CuryID, SiteID, quantity, Date, Date, Date, Date }
			...
			protected ARSalesPrice SelectBasePrice(decimal quantity, params object[] additionalParams)
			{
				return SelectCommand
					.SelectSingle(
						new object[] { InventoryID, CustomerID, CustPriceClass, null, null, CuryID, SiteID, quantity, Date, Date, Date, Date }
							.Concat(additionalParams)
							.ToArray());
			}
			...
			#region Factories
			public SalesPriceForCurrentUOM ForCurrentUOM() => new SalesPriceForCurrentUOM(Cache, InventoryID, UOM, Qty) { CustomerID = CustomerID, CustPriceClass = CustPriceClass, CuryID = CuryID, SiteID = SiteID, Date = Date };
			public SalesPriceForBaseUOM ForBaseUOM() => new SalesPriceForBaseUOM(Cache, InventoryID, UOM, Qty) { CustomerID = CustomerID, CustPriceClass = CustPriceClass, CuryID = CuryID, SiteID = SiteID, Date = Date };
			public SalesPriceForSalesUOM ForSalesUOM() => new SalesPriceForSalesUOM(Cache, InventoryID, UOM, Qty) { CustomerID = CustomerID, CustPriceClass = CustPriceClass, CuryID = CuryID, SiteID = SiteID, Date = Date }; 
			#endregion
		}

		private class SalesPriceForCurrentUOM : SalesPriceSelect
		{
		...
		private class SalesPriceForBaseUOM : SalesPriceSelect
		{
		...
		private class SalesPriceForSalesUOM : SalesPriceSelect
		{
			...
			{
				InventoryItem inventoryItem = PXSelect<InventoryItem, Where<InventoryItem.inventoryID, Equal<Required<InventoryItem.inventoryID>>>>.Select(Cache.Graph, InventoryID);
		...
		public class SalesPriceItem
		{
			...
			public SalesPriceItem(string uom, decimal price, string curyid, string priceType = null)
			{
				UOM = uom;
				Price = price;
				CuryID = curyid;
			...
			public static SalesPriceItem FromPrice(ARSalesPrice price, string uom = null) 
				=> new SalesPriceItem(uom ?? price.UOM, price.SalesPrice ?? 0, price.CuryID, price.PriceType);
		}
