...
namespace PX.Objects.AR
{
			...
			int? dayofWeek)
		{
			...
			switch (aPrepareOn)
			{
				...
				case ARStatementScheduleType.EndOfPeriod:
					try
					{
						string dateFinancialPeriod = FinPeriodIDAttribute.PeriodFromDate(aLastStmtDate);
						DateTime periodEndDate = FinPeriodIDAttribute.PeriodEndDate(graph, dateFinancialPeriod);
						...
						{
							nextDate = FinPeriodIDAttribute.PeriodEndDate(
								graph,
								FinPeriodIDAttribute.PeriodPlusPeriod(graph, dateFinancialPeriod, 1));
						}
					}
			...
			int? dayOfWeek)
		{
			...
			switch (aPrepareOn)
			{
				...
				case ARStatementScheduleType.EndOfPeriod:
					try
					{
						string dateFinancialPeriod = FinPeriodIDAttribute.PeriodFromDate(aBeforeDate);
						DateTime periodEndDate = FinPeriodIDAttribute.PeriodEndDate(graph, dateFinancialPeriod);
						...
						{
							string previousFinancialPeriod = FinPeriodIDAttribute.PeriodPlusPeriod(
								graph,
								dateFinancialPeriod,
								-1);

							return FinPeriodIDAttribute.PeriodEndDate(
								graph,
	...
	public class StatementCycleProcessBO : PXGraph<StatementCycleProcessBO>
	{
			...
			DateTime previousStatementDate)
		{
			...
			IEnumerable<Customer> previousStatementCustomers = PXSelectJoin<
				Customer,
					// Collect customers from previous statements' ARStatement.statementCustomerID.
					// -
					LeftJoin<ARStatement,
						On<Customer.bAccountID, Equal<ARStatement.customerID>,
						And<ARStatement.statementDate, Equal<Required<ARStatement.statementDate>>,
						And<ARStatement.statementCustomerID, In<Required<ARStatement.statementCustomerID>>>>>,
					...
					// -
					LeftJoin<ARStatementAdjust,
						On<ARStatementAdjust.statementDate, Equal<Required<ARStatementAdjust.statementDate>>,
						And<Where<
							Customer.bAccountID, Equal<ARStatementAdjust.adjgCustomerID>, 
							And<ARStatementAdjust.adjdCustomerID, In<Required<ARStatementAdjust.adjdCustomerID>>,
							Or<
							Customer.bAccountID, Equal<ARStatementAdjust.adjdCustomerID>,
							And<ARStatementAdjust.adjgCustomerID, In<Required<ARStatementAdjust.adjgCustomerID>>>>>>>>>>,
				Where<
					ARStatement.statementDate, IsNotNull, 
				...
				.Except(customerFamily, customerIDComparer);

			return previousStatementCustomers.Concat(customerFamily).ToArray();
		}
		...
		protected virtual void GenerateStatement(ARStatementCycle cycle, DateTime statementDate)
		{
			...
			foreach (IEnumerable<Customer> customerFamily in customerFamilies)
			{
				...
				// If at least one of the customers in the family has statements generated
				// for the specified date or a later date, the whole family will be excluded
				// from the statement processing.
				// -
				bool familyHasExistingStatementsOnDate =
					customerFamily.Any(customer => customer.StatementLastDate >= statementDate);

				if (familyHasExistingStatementsOnDate)
				{
					customersWithExistingStatementsOnDate.Add(
						customerFamily.First().AcctCD.Trim());
		...
		protected virtual void UpdateStatementCycleLastStatementDate(ARStatementCycle cycle, DateTime statementDate, bool isOnDemand = false)
		{
			...
			bool allCustomersProcessed = PXSelectReadonly2<
				...
				Where<
					CustomerMaster.statementCycleId, Equal<Required<Customer.statementCycleId>>,
					And<Where<
						Customer.statementLastDate, IsNull,
						Or<Customer.statementLastDate, Less<Required<Customer.statementLastDate>>>>>>>
				.SelectWindowed(this, 0, 1, cycle.StatementCycleId, statementDate)
				.IsEmpty();
			...
			bool isOnDemand)
		{
			...
			if (isBalanceBroughtForwardStatement)
			{
				foreach (ApplicationWithCurrency applicationWithCurrency in GetUnreportedApplications(customer, statementDate))
				{
					...
					IEnumerable<ARStatement> applicableStatements = new []
					{
			...
			PXSelectBase<ARStatement> selectAllPreviousByCustomer = new PXSelect<
				ARStatement,
				Where<
		...
		private IEnumerable<DocumentWithApplications> GetDocumentsWithApplicationsToDate(
			...
			DateTime statementDate)
		{
			PXSelectBase<ARRegister> documentsWithApplicationsView = new PXSelectJoin<
				ARRegister,
					LeftJoin<ARInvoice,
					...
					LeftJoin<ARAdjust,
						...
						And<ARAdjust.adjgDocDate, LessEqual<Required<ARAdjust.adjgDocDate>>,
						And<Where<
							ARAdjust.adjgDocType, Equal<ARRegister.docType>,
							And<ARAdjust.adjgRefNbr, Equal<ARRegister.refNbr>,
						Or<
							ARAdjust.adjdDocType, Equal<ARRegister.docType>,
			...
			if (customer.StatementType == ARStatementType.OpenItem)
			{
				documentsWithApplicationsView.WhereAnd<Where<
					...
					// -
					ARRegister.closedFinPeriodID, IsNull,
					Or<ARRegister.closedFinPeriodID, GreaterEqual<Required<ARRegister.closedFinPeriodID>>>>>();
			}
			else
			{
				documentsWithApplicationsView.WhereAnd<Where<
			...
			object[] queryParameters = new object[]
			{
				...
				customer.BAccountID,
				statementDate,
				FinPeriodIDAttribute.PeriodFromDate(this, statementDate),
			};
			...
			foreach (PXResult<ARRegister, ARInvoice, ARAdjust> result in 
				documentsWithApplicationsView.Select(queryParameters.ToArray()))
			{
					...
					|| currentDocument.RefNbr != previousDocument?.RefNbr)
				{
					...
					{
						yield return new DocumentWithApplications
						{
				...
				{
					documentApplications.Add(result);
				}
			}
		...
		private IEnumerable<ApplicationWithCurrency> GetUnreportedApplications(
			Customer customer,
			DateTime statementDate)
			=> PXSelectJoin<
				ARAdjust,
			...
			.Cast<PXResult<ARAdjust, CurrencyInfoAlias, CurrencyInfoAlias2>>()
			.Select(result => new ApplicationWithCurrency
			{
				Application = result,
				AdjustingCurrencyID = ((CurrencyInfoAlias)result)?.CuryID,
				AdjustedCurrencyID = ((CurrencyInfoAlias2)result)?.CuryID,
			});
			...
			bool ageCredits)
		{
			...
			if (invoice != null && invoice?.DocType != ARDocType.SmallCreditWO || ageCredits)
			{
				...
				int bucketNumber = statementCycle.UseFinPeriodForAging == true
					? AgingEngine.AgeByPeriods(graph, statementDate, dateForAging, FinancialPeriodProvider.Default, AgingDirection.Backwards, 5)
			...
			bool isOnDemand) 
		{
			...
			if (deletedCount == 0 && !isOnDemand)
			{
				...
				persistGraph.Customer.Update(customer);
			}
	...
	public class StatementCreateBO : PXGraph<StatementCreateBO>
	{
		...
		public virtual void ARStatement_RowDeleted(PXCache cache, PXRowDeletedEventArgs e)
		{
			...
			foreach (Customer customer in Customer.Select(statement.CustomerID))
			{
				if (statement.OnDemand != true
					&& statement.StatementDate == customer.StatementLastDate)
				{
					customer.StatementLastDate = FindLastCstmStatementDate(statement.CustomerID, statement.StatementDate);
