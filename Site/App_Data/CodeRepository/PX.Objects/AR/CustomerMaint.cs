...
namespace PX.Objects.AR
{
	namespace Override
	{
        ...
        public partial class BAccount : IBqlTable
        {
            ...
            #endregion
            #region AcctName
            public abstract class acctaName : IBqlField { }
            [PXDBString(60, IsUnicode = true)]
            public virtual string AcctName { get; set; }
            #endregion
            #region ConsolidateToParent
            public abstract class consolidateToParent : IBqlField { }
            [PXDBBool]
        ...
        public partial class Customer : IBqlTable
        {
            ...
            public abstract class sharedCreditPolicy : IBqlField { }
            [PXDBBool]
            public virtual bool? SharedCreditPolicy { get; set; }
			#endregion
			#region StatementLastDate
			public abstract class statementLastDate : IBqlField { }
			[PXDBDate]
				...
				Or<BAccount.type, Equal<BAccountType.combinedType>>>>
	{
		...
		public partial class CustomerBalanceSummary : IBqlTable
		{
			...
			public abstract class consolidatedbalance : IBqlField { }
		...
		public virtual IEnumerable NewInvoiceMemo(PXAdapter adapter)
		{
			...
			if (customer != null && customer.BAccountID > 0L)
			{
				...
				invEntry.Document.Cache.RaiseFieldUpdated<ARInvoice.customerID>(newDoc, null);	//credit rule will be cleared on CustomerID_Updated()
		...
		public virtual IEnumerable NewSalesOrder(PXAdapter adapter)
		{
			...
			if (customer != null && customer.BAccountID > 0L)
			{
				...
				newDoc.CustomerID = customer.BAccountID;
				soEntry.Document.Cache.RaiseFieldUpdated<ARInvoice.customerID>(newDoc, null);	//credit rule will be cleared on CustomerID_Updated()
			...
			if (customer != null && customer.BAccountID > 0L)
			{
				...
				if (!isInserted)
				{
					...
					IEnumerable<Override.ExtendedCustomer> customerFamily = 
						GetChildAccountsAndSelfStripped<Override.Customer.sharedCreditCustomerID>(this, customer.BAccountID);
					...
					// When converting Business Account (or Vendor, or ..) to Customer
					// we might have no Customer record yet
					// -
					if (!customerFamilyIDs.Any())
					...
					ARBalances bal = GetCustomerBalances(this, customerFamilyIDs);

					decimal? balance = bal.CurrentBal;
					decimal? consolidatedBalance = bal.CurrentBal;

					if(PXAccess.FeatureInstalled<FeaturesSet.parentChildAccount>())
					{
						consolidatedBalance = 0m;
						foreach (ChildCustomerBalanceSummary child in ChildAccounts.Select())
						{
							consolidatedBalance += child.ConsolidateToParent == true || child.IsParent == true ? child.Balance : 0m;
							balance = customer.BAccountID == child.CustomerID ? child.Balance : balance;
						}
					}
					...
					if (customer.CreditRule == CreditRuleTypes.CS_CREDIT_LIMIT || customer.CreditRule == CreditRuleTypes.CS_BOTH)
					{
						...
						{
							IEnumerable<Override.ExtendedCustomer> sharedFamily =
								GetChildAccountsAndSelfStripped<Override.Customer.sharedCreditCustomerID>(this, customer.SharedCreditCustomerID);

							int?[] sharedFamilyIDs = sharedFamily
								.Select(row => row.BusinessAccount.BAccountID)
								.Where(id => id != null)
								.Append(customer.BAccountID)
								.ToArray();

							remBal = GetCustomerBalances(this, sharedFamilyIDs);
						}
		...
		[Serializable]
		public partial class ChildCustomerBalanceSummary : CustomerBalanceSummary
		{
			#region CustomerID
			public new abstract class customerID : IBqlField { }

			[PXDBInt]
			[Customer(Enabled = false, IsKey = true)]
		...
		{
			if (CurrentCustomer.Cache.GetStatus(CurrentCustomer.Current) == PXEntryStatus.Inserted)
				yield break;

			IEnumerable<Override.ExtendedCustomer> customerFamily = 
				GetChildAccountsAndSelfStripped<Override.BAccount.parentBAccountID>(this, BAccount.Current?.BAccountID);

			int?[] customerFamilyIDs = customerFamily
				.Select(customer => customer.BusinessAccount.BAccountID)
				.Where(id => id != null)
				.ToArray();
			...
			var prepaymentBalances = PXSelectJoinGroupBy<CuryARHistory,
							InnerJoin<ARCustomerBalanceEnq.ARLatestHistory, On<ARCustomerBalanceEnq.ARLatestHistory.accountID, Equal<CuryARHistory.accountID>,
								And<ARCustomerBalanceEnq.ARLatestHistory.branchID, Equal<CuryARHistory.branchID>,
								And<ARCustomerBalanceEnq.ARLatestHistory.customerID, Equal<CuryARHistory.customerID>,
								And<ARCustomerBalanceEnq.ARLatestHistory.subID, Equal<CuryARHistory.subID>,
								And<ARCustomerBalanceEnq.ARLatestHistory.curyID, Equal<CuryARHistory.curyID>,
								And<ARCustomerBalanceEnq.ARLatestHistory.lastActivityPeriod, Equal<CuryARHistory.finPeriodID>>>>>>>>,
								Where<CuryARHistory.customerID, In<Required<CuryARHistory.customerID>>>,
								Aggregate<GroupBy<CuryARHistory.customerID,
									Sum<CuryARHistory.finYtdDeposits>>>>.Select(this, customerFamilyIDs)
							.ToDictionary(
								result => ((CuryARHistory)result).CustomerID,
								result => (CuryARHistory)result);


			var balances = PXSelectGroupBy<ARBalances,
					Where<ARBalances.customerID, In<Required<ARBalances.customerID>>>,
					Aggregate<
					...
					.Select(this, customerFamilyIDs)
						.ToDictionary(result => ((ARBalances)result).CustomerID, result => (ARBalances)result);

			foreach(Override.ExtendedCustomer customer in customerFamily)
			{
				ARBalances balance = 
					balances.ContainsKey(customer.BusinessAccount.BAccountID) ? 
					balances[customer.BusinessAccount.BAccountID] : 
					new ARBalances();

				CuryARHistory prepaymentBalance = 
					prepaymentBalances.ContainsKey(customer.BusinessAccount.BAccountID) ? 
					prepaymentBalances[customer.BusinessAccount.BAccountID] : 
					new CuryARHistory();

				yield return new ChildCustomerBalanceSummary
				{
					CustomerID = customer.BusinessAccount.BAccountID,
					CustomerName = customer.BusinessAccount.AcctName,
					StatementCycleId = customer.Customer.StatementCycleId,
					ConsolidateToParent = customer.BusinessAccount.ConsolidateToParent,
					ConsolidateStatements = customer.Customer.ConsolidateStatements,
					SharedCreditPolicy = customer.Customer.SharedCreditPolicy,
					IsParent = customer.BusinessAccount.BAccountID == BAccount.Current.BAccountID,
					Balance = balance.CurrentBal ?? 0m,
					UnreleasedBalance = balance.UnreleasedBal ?? 0m,
					OpenOrdersBalance = balance.TotalOpenOrders ?? 0m,
					DepositsBalance = prepaymentBalance.FinYtdDeposits ?? 0m,
					OldInvoiceDate = balance.OldInvoiceDate
				};
			}
		}
			...
			where ParentField : IBqlField
        {
			Type select = BqlCommand.Compose(
				...
				CreateInstance<PXGraph>().Caches[BqlCommand.GetItemType(typeof(ParentField))].GetBqlField(nameof(CR.BAccount.bAccountID)),
		...
		protected virtual void Customer_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			if (isExistingRecord && row.Status == Customer.status.Active)
			{
				//Actions
			...
			bool hasChildren = 
				...
				GetChildAccountsAndSelfStripped<Override.BAccount.parentBAccountID>(this, row.BAccountID).Any(c => c.BusinessAccount.BAccountID != row.BAccountID);
			...
			if (defLocation != null && this.CustomerClass.Current != null)
			{
				...
				{
					if (this.DefLocation.Cache.GetStatus(defLocation) == PXEntryStatus.Notchanged)
					{
						this.DefLocation.Cache.SetStatus(defLocation, PXEntryStatus.Updated);
					}
				}

				bool isRequiredUsageType = (this.CustomerClass.Current.RequireAvalaraCustomerUsageType ?? false) && PXAccess.FeatureInstalled<FeaturesSet.avalaraTax>();
				PXDefaultAttribute.SetPersistingCheck<LocationExtAddress.cAvalaraCustomerUsageType>(this.DefLocation.Cache, defLocation, isRequiredUsageType ? PXPersistingCheck.Null : PXPersistingCheck.Nothing);
				PXUIFieldAttribute.SetRequired<LocationExtAddress.cAvalaraCustomerUsageType>(this.DefLocation.Cache, isRequiredUsageType);
				if (isRequiredUsageType && string.IsNullOrEmpty(defLocation.CAvalaraCustomerUsageType))
				{
					if (this.DefLocation.Cache.GetStatus(defLocation) == PXEntryStatus.Notchanged)
					{
						this.DefLocation.Cache.SetStatus(defLocation, PXEntryStatus.Updated);
		...
		protected virtual void Customer_RowPersisting(PXCache cache, PXRowPersistingEventArgs e)
		{
			...
			if (e.Operation != PXDBOperation.Delete)
			{
				...
				{
					Contact contact = BillContact.Select();
					PXCache contactCache = BillContact.Cache;
					if (contact != null && String.IsNullOrEmpty(contact.EMail))
					{
						contactCache.SmartSetStatus(contact, PXEntryStatus.Updated);
			...
			if (row.ParentBAccountID == null)
		{
				...
				if ((childs = GetChildAccounts()).Any() &&
					CurrentCustomer.Ask(message, MessageButtons.YesNo) == WebDialogResult.Yes)
			{
		...
		protected virtual void Customer_SharedCreditPolicy_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			if (row.ParentBAccountID == null)
			{
				...
				if ((childs = GetChildAccounts(consolidateToParent: true)).Any() &&
					CurrentCustomer.Ask(message, MessageButtons.YesNo) == WebDialogResult.Yes)
			{
		...
		protected virtual void Customer_CustomerClassID_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
		{
			...
			if (row != null)
			{
				...
				if (this.doCopyClassSettings)
				{
					...
					InitCustomerLocation(this.DefLocation.Current, this.DefLocation.Current.LocType, true);
				}
		...
		protected virtual void Customer_Status_FieldVerifying(PXCache cache, PXFieldVerifyingEventArgs e)
		{
			...
			if (e.ExternalCall && row.SharedCreditChild == true)
			{
				...
				if (parent != null)
				{
					...
					bool isValidStatus = parent.Status == Customer.status.CreditHold
						? newValue == Customer.status.Hold || newValue == Customer.status.Inactive || newValue == Customer.status.CreditHold
						: newValue != Customer.status.CreditHold;
					if (!isValidStatus)
					{
		...
		protected virtual string GetSharedCreditChildStatus(string parentStatus, out Func<Customer, bool> func)
		{
			...
			if (parentStatus == Customer.status.CreditHold)
			{
				childStatus = Customer.status.CreditHold;
				func = child => child.Status != Customer.status.Hold && child.Status != Customer.status.Inactive;
			}
			else if (parentStatus == Customer.status.Active || parentStatus == Customer.status.OneTime)
			{
				childStatus = Customer.status.Active;
				func = child => child.Status == Customer.status.CreditHold;
			}
			else if (parentStatus == Customer.status.Hold || parentStatus == Customer.status.Inactive)
			{
				childStatus = Customer.status.Hold;
				func = child => child.Status == Customer.status.CreditHold;
			}
			else
			{
		...
		[PXUIField(DisplayName = "Entity Usage Type", Required = false)]
		[TX.TXAvalaraCustomerUsageType.List]
		[PXDefault(typeof(Search<AR.CustomerClass.avalaraCustomerUsageType, Where<AR.CustomerClass.customerClassID, Equal<Current<AR.Customer.customerClassID>>>>), PersistingCheck = PXPersistingCheck.Nothing)]
		...
		protected virtual void Contact_EMail_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
		{
			...
			if (owner != null && row != null)
			{
				...
				{
					BAccount.Cache.SmartSetStatus(owner, PXEntryStatus.Updated);
				}
			}
		}
