...
namespace PX.Objects.AR
{
	...
	public partial class CustomerPaymentMethod : PX.Data.IBqlTable
	{
		...
		/// <summary>
		/// The identifier of a <see cref="Customer"/> record to
		...
		/// method requires remittance information. The field defaults to
		...
		/// account in Acumatica ERP and Authorize.Net. The main purposes of
		/// the identifier are to link multiple bank cards to a single customer
		...
		/// <summary>
		/// Indicates (if set to <c>true</c>) that the customer payment
		/// method is the result of another payment method conversion on
	...
	/// <summary>
	/// For tokenized customer payment methods, which allow direct input of the credit 
	/// card data (see <see cref="CCProcessingCenter.AllowDirectInput"/>), an unbound 
	/// record of this type provides a UI switch between the Payment Profile ID 
	/// entry and the direct credit card data entry, affecting the set of 
	/// <see cref="CustomerPaymentMethodDetail">payment method details</see> shown 
	/// on the Customer Payment Methods (AR303010) form, which corresponds to the 
	/// <see cref="CustomerPaymentMethodMaint"/> graph.
	/// </summary>
	[Serializable]
	...
	{
		#region InputMode
		public abstract class inputMode : PX.Data.IBqlField
		{
		}
	...
	public class InputModeType
	{
		...
		public class token : Constant<string>
		{
			public token() : base(Token) { }
		}

		public class details : Constant<string>
		{
