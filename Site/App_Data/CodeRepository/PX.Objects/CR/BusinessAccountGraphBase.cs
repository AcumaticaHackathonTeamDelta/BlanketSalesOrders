...
namespace PX.Objects.CR
{
		...
		where WhereClause : class, IBqlWhere, new()
	{
		...
		public PXSelect<ContactExtAddress, Where<ContactExtAddress.contactBAccountID, Equal<Current<BAccount.bAccountID>>,
								And<ContactExtAddress.contactType, NotEqual<ContactTypesAttribute.bAccountProperty>>>> ExtContacts;
		...
		public virtual IEnumerable ViewMainOnMap(PXAdapter adapter)
		{
			...
			BAccountUtility.ViewOnMap(this.DefAddress.Current);
			return adapter.Get();
		}
		    ...
		    if (row != null)
		    {
		        ...
		        else
		        {
                    if (BAccount.Current.DefAddressID == null)
                    {
                        ...
                        if (Caches[typeof(BAccount)].GetStatus(BAccount.Current) == PXEntryStatus.Notchanged)
                        {
                            Caches[typeof(BAccount)].SetStatus(BAccount.Current, PXEntryStatus.Updated);
                        }
                    }
                    else
                    {
                        ...
                        if (ret == null)
                        {
                            ...
                            if (Caches[typeof(Base)].GetStatus(BAccountAccessor.Current) == PXEntryStatus.Notchanged)
                            {
                                Caches[typeof(Base)].SetStatus(BAccountAccessor.Current, PXEntryStatus.Updated);
                            }
                        }
                    }
		        }
				...
				if (account.DefLocationID != defLocationID)
				{
					...
					foreach (LocationExtAddress other in PXParentAttribute.SelectSiblings(Locations.Cache, row))
					{
						if (!object.Equals(other.LocationID, other.VAPAccountLocationID))
						{
							...
							if (Locations.Cache.GetStatus(other) == PXEntryStatus.Notchanged)
							{
								Locations.Cache.SetStatus(other, PXEntryStatus.Updated);
							}
		...
		protected virtual void SetAsDefault(ContactExtAddress row)
		{
			...
			if (account != null)
			{
				...
				if (account.DefContactID != defContactID)
				{
					...
					if (this.BAccountAccessor.Cache.GetStatus(account) == PXEntryStatus.Notchanged)
					{
						this.BAccountAccessor.Cache.SetStatus(account, PXEntryStatus.Updated);
					}
				}
			}
		}
		...
		protected virtual Location FindLocation(int? aId)
		{
			if (aId.HasValue)
			{
				foreach (Location it in this.IntLocations.Select())
				{
					if (it.LocationID == aId) return it;
				}
			}
			return null;
		}
	...
	public partial class BAccountR : BAccount
	{
        ...
        #endregion
        #region ViewInCrm
        public new abstract class viewInCrm : IBqlField
        {
        }        
        [PXBool]
