	...
	public class CREmailActivityMaint : CRBaseActivityMaint<CREmailActivityMaint, CRSMEmail>
	{
		...
		public partial class NotificationFilter : IBqlTable
		{
			...
			[PXSelector(typeof(Search<CRActivity.noteID,
				 Where<CRActivity.refNoteID, Equal<Current<CRActivity.refNoteID>>>>),
				 typeof(CRActivity.subject),
				 typeof(CRActivity.type),
			...
			#region PageName
			public abstract class pageName : PX.Data.IBqlField
			{
			}
		...
		protected virtual IEnumerable delete(PXAdapter adapter)
		{
			...
			{
				CRSMEmail newMessage = (CRSMEmail)Message.Cache.CreateCopy(record);
				...
				if (newMessage.MPStatus != MailStatusListAttribute.Deleted)
				{
					...
					foreach (Type field in Message.Cache.BqlFields.Where(f => f != typeof(CRSMEmail.mpstatus) 
					...
					{
						this.CommandPreparing.AddHandler(typeof(CRSMEmail), field.Name,
		...
		protected IEnumerable reply(PXAdapter adapter)
		{
			foreach (CRSMEmail oldMessage in adapter.Get())
			{
				...
				PXRedirectHelper.TryRedirect(targetGraph, targetGraph.Message.Current, PXRedirectHelper.WindowMode.Same);
		...
		protected void replyAll()
		{
			...
			PXRedirectHelper.TryRedirect(targetGraph, targetGraph.Message.Current, PXRedirectHelper.WindowMode.NewWindow);
		}
		...
		protected virtual CREmailActivityMaint CreateTargetMail(CRSMEmail oldMessage, string replyTo = null, string cc = null, string bcc = null)
		{
		...
		public virtual void loadEmailSource()
		{
			...
			if (res == WebDialogResult.OK)
			{
				if (NotificationInfo.Current.Type == TemplateSourceType.Notification)
				{
					...
					PXPrimaryGraphAttribute.FindPrimaryGraph(this.Caches[type], false, ref row, out graphType);

					var isRights = graphType != null ? PXAccess.VerifyRights(graphType) : true;
					if(isRights == false)
						throw new PXException(PX.Data.ErrorMessages.NotEnoughRights, graphType);
					...
					Message.Current.MailAccountID = upd.NFrom ?? Message.Current.MailAccountID;
		...
		public virtual IEnumerable create(PXAdapter adapter)
		{
			...
			{
				contact.LastName = names.ToArray()[0].LastName;
            ...
            {
                opp.OpportunityName = mainEmail.Subject;
                opp.Description = mainEmail.Body;
                cache.Update(opp);
		...
		protected virtual IEnumerable archive(PXAdapter adapter)
		{
			...
			{
				if (newMessage.MPStatus != MailStatusListAttribute.Archived)
				{
					newMessage.MPStatus = MailStatusListAttribute.Archived;
		...
		protected virtual IEnumerable restoreArchive(PXAdapter adapter)
		{
			foreach (CRSMEmail newMessage in adapter.Get().Cast<CRSMEmail>())
			{
				if (newMessage.MPStatus == MailStatusListAttribute.Archived)
				{
					newMessage.MPStatus = MailStatusListAttribute.Processed;
					Message.Update(newMessage);
				}
				Actions.PressSave();
				yield return newMessage;
		...
		public override void Persist()
		{
			...
			this.Message.Cache.RaiseRowSelected(this.Message.Cache.Current);			
		}
		...
		[PXRestrictor(typeof(Where<Match<Current<AccessInfo.userName>>>), ErrorMessages.RestrictedEmailAccount, typeof(EMailAccount.description))]
		...
		protected virtual void CRSMEmail_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			var isPmVisible = PM.ProjectAttribute.IsPMVisible( GL.BatchModule.EP);
			...
			PXUIFieldAttribute.SetVisible<PMTimeActivity.trackTime>(tActCache, tAct, row.IsIncome != true);
			PXUIFieldAttribute.SetVisible<PMTimeActivity.approvalStatus>(tActCache, tAct, tAct?.TrackTime == true && row.IsIncome != true);
			...
			PXUIFieldAttribute.SetVisible<PMTimeActivity.overtimeBillable>(tActCache, tAct, tAct?.IsBillable == true && tAct?.TrackTime == true);
			...
			PXUIFieldAttribute.SetVisible<PMTimeActivity.costCodeID>(tActCache, tAct, tAct?.TrackTime == true);
			...
			Delete.SetEnabled(!wasUsed && row.IsLocked != true && row.MPStatus != MailStatusListAttribute.InProcess && row.MPStatus != MailStatusListAttribute.Processed);

			if (origStatus == ActivityStatusListAttribute.Open)
			{
				PXUIFieldAttribute.SetEnabled<CRSMEmail.isExternal>(cache, row, true);

				Delete.SetEnabled(!wasUsed && row.IsLocked != true);
			}
			else
			{
				PXUIFieldAttribute.SetEnabled<CRSMEmail.isExternal>(cache, row, false);
				PXUIFieldAttribute.SetEnabled<PMTimeActivity.trackTime>(tActCache, tAct, row.IsLocked != true);
			}

			PXUIFieldAttribute.SetVisible<CRSMEmail.uistatus>(cache, row, false);
			PXUIFieldAttribute.SetEnabled<CRSMEmail.parentNoteID>(cache, row, false);
			...
			PXUIFieldAttribute.SetEnabled<CRSMEmail.mpstatus>(cache, row, false);			
			PXUIFieldAttribute.SetEnabled<CRSMEmail.startDate>(cache, row, false);
			...
			PXUIFieldAttribute.SetEnabled(tActCache, tAct, 
                row.MPStatus == MailStatusListAttribute.Draft || 
                row.MPStatus == MailStatusListAttribute.Failed);

			if (origTimeStatus == ActivityStatusListAttribute.Open)
			{
				PXUIFieldAttribute.SetEnabled<PMTimeActivity.projectID>(tActCache, tAct, true);
				PXUIFieldAttribute.SetEnabled<PMTimeActivity.projectTaskID>(tActCache, tAct, true);
				PXUIFieldAttribute.SetEnabled<PMTimeActivity.trackTime>(tActCache, tAct, true);
				PXUIFieldAttribute.SetEnabled<PMTimeActivity.timeSpent>(tActCache, tAct, true);
				PXUIFieldAttribute.SetEnabled<PMTimeActivity.isBillable>(tActCache, tAct, true);
				PXUIFieldAttribute.SetEnabled<PMTimeActivity.earningTypeID>(tActCache, tAct, true);
			...
			{
				PXUIFieldAttribute.SetEnabled<PMTimeActivity.projectID>(tActCache, tAct, false);
				PXUIFieldAttribute.SetEnabled<PMTimeActivity.projectTaskID>(tActCache, tAct, false);
				PXUIFieldAttribute.SetEnabled<PMTimeActivity.trackTime>(tActCache, tAct, false);
				PXUIFieldAttribute.SetEnabled<PMTimeActivity.timeSpent>(tActCache, tAct, false);
				PXUIFieldAttribute.SetEnabled<PMTimeActivity.isBillable>(tActCache, tAct, false);
				PXUIFieldAttribute.SetEnabled<PMTimeActivity.earningTypeID>(tActCache, tAct, false);
			}
			
			PXUIFieldAttribute.SetEnabled<PMTimeActivity.approvalStatus>(tActCache, tAct, 
				tAct != null && tAct.TrackTime == true && 
                (row.MPStatus == MailStatusListAttribute.Processed || row.MPStatus == MailStatusListAttribute.Archived));
			...
			Archive.SetEnabled(row.MPStatus == MailStatusListAttribute.Processed);
			Archive.SetVisible(row.MPStatus == MailStatusListAttribute.Processed);
			RestoreArchive.SetEnabled(row.MPStatus == MailStatusListAttribute.Archived);
			RestoreArchive.SetVisible(row.MPStatus == MailStatusListAttribute.Archived);

			Restore.SetEnabled(row.MPStatus == MailStatusListAttribute.Deleted);
			Restore.SetVisible(row.MPStatus == MailStatusListAttribute.Deleted);
		...
		private static string GenerateBackAddress(PXGraph graph, CRSMEmail message, string defaultDisplayName, string defaultAddress, bool allowUseCurrentUser)
		{
				...
				if (message.OwnerID == null || message.ClassID == CRActivityClass.EmailRouting)
			{
				...
				result = string.IsNullOrEmpty(displayName)
					? address
		...
		private static string GetReplyAddress(CRSMEmail oldMessage)
		{
		...
		private string GetMailAccountAddress(CRSMEmail oldMessage)
		{
		...
		private string GetReplyAllCCAddress(CRSMEmail oldMessage, string mailAccountAddress)
		{
			var newAddressList = new List<MailAddress>();
		...
		private string GetReplyAllBCCAddress(CRSMEmail oldMessage, string mailAccountAddress)
		{
			var newAddressList = new List<MailAddress>();
		...
		private string GetReplyAllAddress(CRSMEmail oldMessage, string mailAccountAddress)
		{
			var newAddressList = new List<MailAddress>();
		...
		protected static void AttachFiles(CRSMEmail newEmail, Guid? refNoteId, PXCache cache, IEnumerable<FileInfo> files)
		{
			...
			{
				var format = refNoteId == null ? "[{0}] {2}{3}" : "[{0}] [{2}] \\{3}";
		...
		private void CopyAttachments(PXGraph targetGraph, CRSMEmail message, CRSMEmail newMessage)
		{
