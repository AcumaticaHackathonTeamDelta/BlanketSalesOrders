...
namespace PX.Objects.CR
{
	...
	[PXDimensionSelector(DimensionName, typeof(Search2<BAccountR.bAccountID,
						LeftJoin<Contact,
						...
						Where<Match<Current<AccessInfo.userName>>>>), typeof(BAccountR.acctCD),
	...
	public class StateAttribute : CountryStateSelectorAttribute
	{
		...
		public override void FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			...
			slot.Countries.TryGetValue((string)countryID, out item);
			var country = item as Definition.CountryItem;

			if (country != null &&
				country.StateValidation != Country.stateValidationMethod.No &&
				!Find(country.States, sender, e) &&
				!Find(country.StatesDescription, sender, e) &&
				!FindRegex(country.StatesRegEx, sender, e))
	...
	{		
		protected class Definition : IPrefetchable
		{
            ...
            private void GetCountryList()
            {
                ...
                {
                    string query = "(" + sqlDialect.limitRowsBeingSelected(
                        sqlDialect.quoteTableAndColumn("CountryKvExt", "ValueString") +
                        " FROM " + sqlDialect.quoteDbIdentifier("CountryKvExt") +
                        " WHERE " + sqlDialect.quoteTableAndColumn("CountryKvExt", "RecordID") +
                        " = " + sqlDialect.quoteTableAndColumn("Country", "NoteID") +
                        " AND " + sqlDialect.quoteTableAndColumn("CountryKvExt", "FieldName") +
                        " = " + sqlDialect.enquoteValue(item.Country), 1) + ")";
                    fields.Add(new PXDataField(query));
                }
            ...
            private void GetStateList()
            {
                ...
                {
                    string query = "(" + sqlDialect.limitRowsBeingSelected(
                        sqlDialect.quoteTableAndColumn("StateKvExt", "ValueString") +
                        " FROM " + sqlDialect.quoteDbIdentifier("StateKvExt") +
                        " WHERE " + sqlDialect.quoteTableAndColumn("StateKvExt", "RecordID") +
                        " = " + sqlDialect.quoteTableAndColumn("State", "NoteID") +
                        " AND " + sqlDialect.quoteTableAndColumn("StateKvExt", "FieldName") +
                        " = " + sqlDialect.enquoteValue(item.State), 1) + ")";
                    fields.Add(new PXDataField(query));
                }
                int columnsCount = fields.ToArray().Length;
	...
	public class LeadStatusesAttribute : PXStringListAttribute
	{
		...
		public LeadStatusesAttribute()
			: base(new[] { New, Open, Suspend, Closed, Converted },
					new[] { Messages.LeadNew, Messages.LeadOpen, Messages.LeadSuspend, Messages.Lost, Messages.LeadConverted })
		{}
	...
	#region LeadSelectorAttribute

	public class LeadSelectorAttribute : PXSelectorAttribute
	{
		public LeadSelectorAttribute()
			:base(typeof(Search<Contact.contactID, Where<Contact.contactType, Equal<ContactTypesAttribute.lead>, 
		...
		public override void CacheAttached(PXCache sender)
		{
			...
			{			
				_Select = _Select.WhereNew<Where<Contact.contactType, Equal<ContactTypesAttribute.lead>>>();
				_Select = _Select.OrderByNew<OrderBy<Asc<Contact.majorStatus, Desc<Contact.duplicateStatus, Asc<Contact.contactID>>>>>();
	...
	public class DuplicateStatusAttribute : PXStringListAttribute
	{
		...
		public sealed class validated : Constant<string>
		{
		...
		public override Type GetGraphType(PXCache cache, ref object row, bool checkRights, Type preferedType)
		{
			...
			foreach (PrimaryGraph pg in _items)
			{
				...
				if (typeof(IBqlWhere).IsAssignableFrom(pg.Condition))
				{
					...
					bool? result = null;
					object value = null;
					BqlFormula.Verify(graph.Caches[itemType], row, where, ref result, ref value);
	...
	public class ContactTypesAttribute : PXStringListAttribute
	{
		...
		public class personPriority : Constant<int>
		{
		...
		public class bAccountPriority : Constant<int>
		{
		...
		public class salesPersonPriority : Constant<int>
		{
		...
		public class employeePriority : Constant<int>
		{
		...
		public class leadPriority : Constant<int>
		{
	...
	public sealed class CRCaseSeverityAttribute : PXStringListAttribute
	{
		...
		public sealed class low : Constant<string>
		{
			public low() : base(_LOW) { }
		}

		public sealed class medium : Constant<string>
		{
			public medium() : base(_MEDIUM) { }
		}

		public sealed class high : Constant<string>
		{
	...
	public class MailStatusListAttribute : PXStringListAttribute
	{
		...
		public class archived : Constant<string>
		{
				...
				typeof (CRActivity.noteID),
					...
					typeof (Where<,,>),
						...
						typeof (And<,,>), typeof (CRActivity.noteID), typeof (NotEqual<>), typeof (Current<>), Field,
							typeof (And<>), typeof (Where<,,>), typeof (CRActivityRelation.noteID), typeof (NotEqual<>), typeof (Current<>), Field,
							typeof (Or<,>), typeof (CRActivityRelation.noteID), typeof (IsNull),
							typeof (Aggregate<>),
		...
		private static Type GetQuery(Type searchField, bool showContactsWithNullEmail, Type[] contactTypes)
        {
            ...
            if (contactTypes.Length == 1)
            {
                contactTypeClause = BqlCommand.Compose(typeof(Where<,>), typeof(Contact.contactType),
                    typeof(Equal<>), contactTypes.Single());
            }
            else
            {
                var allContactTypes =
                    typeof(ContactTypesAttribute).GetNestedTypes().Where(x => x.BaseType == typeof(Constant<string>));
            ...
            else
            {
                ...
                command = BqlCommand.Compose(
                    ...
                    typeof(LeftJoin<,>), typeof(BAccount),
                    typeof(On<,>), typeof(BAccount.bAccountID), typeof(Equal<>), typeof(Contact.bAccountID),
                    typeof(Where2<,>), notNullClause, typeof(And<>), contactTypeClause);
            }
    ...
    public class CROpportunityStagesAttribute : PXStringListAttribute, IPXFieldDefaultingSubscriber, IPXRowUpdatedSubscriber, IPXRowSelectedSubscriber
    {
        ...
        public CROpportunityStagesAttribute(Type oppClassID, Type stageChangedDate = null) : base(new string[] { }, new string[] { })
        {
            if (oppClassID == null)
            ...
            List<CROppClassStage> stages = GetClassStages(string.Empty);
        ...
        public override void FieldSelecting(PXCache cache, PXFieldSelectingEventArgs e)
        {
            ...
            // set default list with all stages - it is used in GI
            List<CROppClassStage> allStages = GetClassStages(string.Empty);
            _AllowedValues = allStages.Select(x => x.StageID).ToArray();
            _AllowedLabels = allStages.Select(x => x.Name).ToArray();
            ...
            string[] labels = stages.Select(x => x.Name).ToArray();

            e.ReturnState = (PXStringState)PXStringState.CreateInstance(e.ReturnState, null, null, _FieldName, null, -1, null, values, labels, true, null, labels);
        }
                ...
                if (classID != oldClassID && stageID == null)
	{
        ...
        public virtual void RowSelected(PXCache cache, PXRowSelectedEventArgs e)
        {
            ...
            if (TryGetClassAndStage(cache, e.Row, out classID, out stageID))
            {
                ...
                {
                    var state = (PXFieldState)cache.GetStateExt<CROpportunity.stageID>(e.Row);
        ...
        {
            return (classID != null && Definitions.ClassStages.ContainsKey(classID)) ? Definitions.ClassStages[classID] : Definitions.ClassStages[string.Empty];
		}
        private Definition Definitions
        {
            get
            {
                ...
                if (defs == null)
                {
                    defs = PXContext.SetSlot<Definition>(PXDatabase.GetSlot<Definition>(typeof(Definition).FullName, typeof(CROpportunityProbability), typeof(CROpportunityClassProbability)));
                }
                return defs;
            }
        }
        private class Definition : IPrefetchable
        {
            ...
            public void Prefetch()
            {
                ...
                using (new PXConnectionScope())
                {
                    ...
                    List<CROppClassStage> allStages =
                            ...
                            new PXDataField<CROpportunityProbability.probability>())
                                .Select(x =>
                                    new CROppClassStage()
                                    {
                                        ...
                                        Name = x.GetString(1),
                                        SortOrder = x.GetInt32(2),
                                        Probability = x.GetInt32(3),
                                        IsActive = false
                                    })
                                .OrderBy(x => x.SortOrder).ThenBy(x => x.Probability).ThenBy(x => x.StageID).ToList();

                    // add full list for empty classID
                    ClassStages.Add(string.Empty, CloneStagesList(allStages));
                        ...
                        new PXDataField<CROpportunityClassProbability.stageID>()))
		{
                        ...
                        if (!ClassStages.ContainsKey(classID))
			{
                        ...
                        ClassStages[classID].Single(s => s.StageID == stageID).IsActive = true;
                    }
                }
            }
	...
	public sealed class CRDuplicateContactsSelectorAttribute : PXCustomSelectorAttribute
	{
		...
		public IEnumerable GetRecords()
		{
			...
			if(sourceCache.InternalCurrent == null) yield break;
	...
	public sealed class CRDuplicateBAccountSelectorAttribute : PXCustomSelectorAttribute
	{
		...
		public override void CacheAttached(PXCache sender)
		{
		    ...
		    var select =
		        BqlCommand.CreateInstance(
		            typeof(Search2<CRDuplicateRecord.duplicateContactID,
                        InnerJoin<BAccountR,
                               On<BAccountR.defContactID, Equal<CRDuplicateRecord.duplicateContactID>>,                     
		                InnerJoin<CRLeadContactValidationProcess.Contact2,
		                       On<BAccountR.bAccountID, Equal<CRLeadContactValidationProcess.Contact2.bAccountID>,
		                      And<BAccountR.defContactID, Equal<CRLeadContactValidationProcess.Contact2.contactID>>>>>,		                		                
		                Where<CRDuplicateRecord.contactID, Equal<Current<BAccount.defContactID>>, 
		                ...
		                OrderBy<Asc<BAccountR.acctCD>>>));

            View = new PXView(_Graph, false, select);
		}
		...
		public IEnumerable GetRecords()
		{
			...
			foreach (PXResult<CRDuplicateRecord,BAccountR,CRLeadContactValidationProcess.Contact2> result in View.SelectMulti(currentID))
			{
				BAccountR b = result;
				CRDuplicateRecord d = result;
				d = (CRDuplicateRecord)_Graph.Caches[typeof(CRDuplicateRecord)].Locate(d);
    ...
    public abstract class CRContactAttribute : ContactAttribute, IPXRowUpdatedSubscriber
    {
        ...
        {
            PXView view = null;
            if ((DocumentRow as CROpportunity).ContactID != null)
            {
            ...
            else if((DocumentRow as CROpportunity).LocationID != null)
            {
                BqlCommand Select = BqlCommand.CreateInstance(typeof(
                    Select2<Contact,
      ...
      public OUEntityIDSelectorAttribute(Type typeBqlField, Type contactBqlField, Type baccountBqlField)
		...
		protected override void CreateSelectorView(PXGraph graph, Type itemType, PXNoteAttribute noteAtt, out string viewName, out string[] fieldList, out string[] headerList)
		{
			...
			if (itemType == typeof(CROpportunity))
				search =
					BqlCommand.Compose(typeof(Search<,>), typeof(CROpportunity.opportunityNumber), 
    ...
    public sealed class SendFilterSourcesAttribute : PXStringListAttribute
    {
        ...
        public sealed class neverSent : Constant<string>
        {
	...
	public class CRRoleTypeList
	{
		...
		public sealed class referrer : Constant<string>
		{
		...
		public sealed class supervisor : Constant<string>
		{
		...
		public sealed class businessUser : Constant<string>
		{
		...
		public sealed class decisionMaker : Constant<string>
		{
		...
		public sealed class relatedEntity : Constant<string>
		{
		...
		public sealed class technicalExpert : Constant<string>
		{
		...
		public sealed class supportEngineer : Constant<string>
		{
		...
		public sealed class evaluator : Constant<string>
		{
	...
	public class CRRelationContactSelectorAttribute : PXCustomSelectorAttribute, IPXFieldDefaultingSubscriber
	{
		...
		public IEnumerable GetRecords()
		{
			...
			if (IsBAccountRelation(sender, row) ||
			...
			if (select != null)
			{
				PXView view = _Graph.TypedViews.GetView(select, true);
		...
		protected virtual bool IsBAccountRelation(PXCache sender,CRRelation row)
		{			
			CRRelationTypeListAttribure attr = sender.GetAttributes<CRRelation.targetType>(row).OfType<CRRelationTypeListAttribure>().First();
			return attr.GetTargetID(row.Role) == (int)CRRelationTypeListAttribure.TypeEntityList.BAccount;
		}
		...
		public override void FieldSelecting(PXCache sender, PXFieldSelectingEventArgs e)
		{
			...
			bool isDisabled = (IsBAccountRelation(sender, row) ||
								  (row.Role == CRRoleTypeList.RelatedEntity &&
		...
		public virtual void FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
		{
			...
			if (row.TargetNoteID != null)
			{
				...
				item = helper.GetEntityRow(cachetype, row.TargetNoteID);
			}
	...
	public class CRRelationTypeListAttribure : PXEventSubscriberAttribute, IPXFieldSelectingSubscriber,
		 IPXFieldDefaultingSubscriber
	{
		...
		public CRRelationTypeListAttribure(Type RoleType)
		{
			roleType = RoleType;
			...
			if(role != null)
				switch (role)
				{
					...
					case CRRoleTypeList.SupportEngineer:
						...
						break;
					case CRRoleTypeList.RelatedEntity:
			...
			e.ReturnState =
				 (PXStringState)
					  PXStringState.CreateInstance(e.ReturnState, null, null, _FieldName, null, -1, null, item.fieldNames,
							item.fieldDisplayNames, true, null, item.fieldDisplayNames);
	...
	public class CRRelationAccount : PXEventSubscriberAttribute, IPXFieldSelectingSubscriber, IPXFieldDefaultingSubscriber
	{
		public virtual void FieldSelecting(PXCache sender, PXFieldSelectingEventArgs e)
		{
			CRRelation row = e.Row as CRRelation;
			if (row == null) return;
			((PXFieldState)e.ReturnState).Enabled = row.Role != CRRoleTypeList.RelatedEntity;
		}
		...
		public virtual void FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
		{
			...
			if (row == null || row.Role != CRRoleTypeList.RelatedEntity || row.TargetNoteID == null) // only "RE"
				return;

			var helper = new EntityHelper(sender.Graph);
			Type cachetype = System.Web.Compilation.PXBuildManager.GetType(row.TargetType, false);
			var obj = helper.GetEntityRow(cachetype, row.TargetNoteID);
	...
	public class EntityIDSelectorAttribute : PXEventSubscriberAttribute, IPXFieldSelectingSubscriber, IPXFieldUpdatingSubscriber
	{
		...
		public virtual void FieldUpdating(PXCache sender, PXFieldUpdatingEventArgs e)
		{
			...
			{
				Guid val;
				e.NewValue = Guid.TryParse((string)e.NewValue, out val) ? val as Guid? : null;
			}
		...
		protected virtual string GetDescription(PXCache sender, object row)
		{
			...
			return noteId.HasValue && noteId.Value != Guid.Empty
				 ? EntityHelper.GetEntityDescription(sender.Graph, helper.GetEntityRow(type, noteId))
				 : null;
		}
