	...
	public class RMReportReaderGL : PXGraphExtension<RMReportReader>
    {
        ...
        public virtual void GLEnsureInitialized()
        {
            if (_historyLoaded == null)
            {
                ...
                PXSelectJoin<CR.BAccount, InnerJoin<Branch, On<CR.BAccount.bAccountID, Equal<Branch.bAccountID>>>>.Select(this.Base);
	            // ReSharper disable once InconsistentlySynchronizedField
                _bAccounts = new List<CR.BAccount>(Base.Caches[typeof(CR.BAccount)].Cached.Cast<BAccount>());
                ...
                if (Base.Report.Current.ApplyRestrictionGroups == true)
                {
                    ...
                    PXSelect<Branch, Where2<MatchWithBranch<Branch.branchID>, And<Match<Current<AccessInfo.userName>>>>>.Clear(this.Base);
                    PXSelect<Branch, Where2<MatchWithBranch<Branch.branchID>, And<Match<Current<AccessInfo.userName>>>>>.Select(this.Base);
                }
                else
                {
        ...
        private void LoadHistory(int ledgerID, int year)
        {
            ...
            FinPeriod lastPeriodOfPrevYear = _reportPeriods.FinPeriods.Where(p => String.Compare(p.FinPeriodID, year.ToString()) < 0).LastOrDefault();
        ...
        public virtual object GetHistoryValue(ARmDataSet dataSet, bool drilldown, Func<ARmDataSet, bool, object> del)
        {
            ...
            if (rmType == ARmReport.GL)
            {
                ...
                !string.IsNullOrEmpty(dataSet[Keys.AccountClass] as string ?? "")) && !string.IsNullOrEmpty(dataSet[Keys.StartPeriod] as string ?? "") && (short?)dataSet[Keys.AmountType] != (short?)BalanceType.NotSet)))
                {
                    ...
                    {
                        accounts.ForEach(a => splitret.Add(new object[] { a.AccountCD, a.Description, 0m, null, null }));
                    }
                    else if (ds.Expand == ExpandType.Sub)
                    {
                        subs.ForEach(s => splitret.Add(new object[] { s.SubCD, s.Description, 0m, null, null }));
                    }
	    ...
	    public virtual IEnumerable GetItemsInRange(Type table, ARmDataSet dataSet, Func<Type, ARmDataSet, IEnumerable> del)
	    {
		    ...
		    {
			    return _branchRangeCache.GetItemsInRange(dataSet[Keys.StartBranch] as string, 
					branch => branch.BranchCD, 
					(branch, code) => branch.BranchCD = code);
		    }
        ...
        private void EnsureHistoryLoaded(RMDataSourceGL dsGL)
        {
            ...
            {
                var key = new Tuple<int, int>(dsGL.LedgerID.Value, year);
        ...
        private List<GLHistory> GetPeriodsToCalculate(Branch branch, Account account, Sub sub, RMDataSource ds, RMDataSourceGL dsGL, out bool takeLast)
        {
            ...
            {
                return _reportPeriods.GetPeriodsForBeginningBalanceAmount(dsGL, periodsForKey, isPLAccount || account.AccountID == _ytdNetIncomeAccountID, out takeLast);
            }
            else if (ds.AmountType == BalanceType.EndingBalance || ds.AmountType == BalanceType.CuryEndingBalance)
            {
                return _reportPeriods.GetPeriodsForEndingBalanceAmount(dsGL, periodsForKey, isPLAccount || account.AccountID == _ytdNetIncomeAccountID);
            }
            else
            {
        ...
        private object CalculateAndExpandValue(bool drilldown, RMDataSource ds, RMDataSourceGL dsGL, ARmDataSet dataSet, List<Account> accounts, List<Sub> subs, List<Branch> branchList, List<object[]> splitret)
        {
			...
			Parallel.For(0, accounts.Count, account =>
            {
				...
				Parallel.For(0, subs.Count, sub =>
                {
					...
					Parallel.ForEach(branchList, currentBranch =>
                    {
                        bool takeLast = false;
        ...
        private static decimal GetAmountFromGLHistory(RMDataSource ds, Account account, GLHistory hist, bool takeLast)
        {
            switch (ds.AmountType.Value)
            {
                ...
                case BalanceType.BeginningBalance:
                    if (takeLast)
                    {
                        return (decimal)hist.FinYtdBalance; // There's no history record for this period, so we use the ending balance of the previous one.
                    }
                    else
                    {
                        return (decimal)hist.FinBegBalance;
                    }
                case BalanceType.CuryBeginningBalance:
                    if (takeLast)
                    {
                        return (decimal)hist.CuryFinYtdBalance; // There's no history record for this period, so we use the ending balance of the previous one.
                    }
                    else
                    {
                        return (decimal)hist.CuryFinBegBalance;
                    }
                case BalanceType.EndingBalance:
                    return (decimal)hist.FinYtdBalance;
                case BalanceType.CuryEndingBalance:
                    return (decimal)hist.CuryFinYtdBalance;
                default:
		...
		public ARmReport GetReport(Func<ARmReport> del)
		{
            ...
            if (ar.Type == ARmReport.GL)
            {
                ...
                Base.CreateParameter(Keys.StartBranch, "StartBranch", Messages.GetLocal(Messages.StartBranchTitle), ar.DataSet[Keys.StartBranch] as string, rGL.RequestStartBranch ?? false, colSpan, sViewName, sInputMask, aRp);
                Base.CreateParameter(Keys.EndBranch, "EndBranch", Messages.GetLocal(Messages.EndBranchTitle), ar.DataSet[Keys.EndBranch] as string, rGL.RequestEndBranch ?? false, colSpan, sViewName, sInputMask, aRp);
            }
	...
	public class ArmDATA : PX.Data.Reports.SoapNavigator.DATA
	{
		...
		public object FormatPeriod(object period, object shift)
		{
			try
			{
				...
				int month = int.Parse(per.Substring(0, 2));
				int res = year * k + month + j;
				int resMonth = res % k;
				int resYear;
				if (resMonth == 0)
				{
