    ...
    public class RMReportReaderPM : PXGraphExtension<RMReportReaderGL, RMReportReader>
    {
        ...
        public virtual void PMEnsureInitialized()
        {
            if (_historyLoaded == null)
            {
				...
				var costCodeSelect = new PXSelectJoinGroupBy<PMCostCode, InnerJoin<PMHistory, On<PMCostCode.costCodeID, Equal<PMHistory.costCodeID>>>, Aggregate<GroupBy<PMCostCode.costCodeID>>>(this.Base);
				costCodeSelect.View.Clear();
				costCodeSelect.Cache.Clear();

				_budgetByKey = new Dictionary<BudgetKeyTuple, PMBudget>();

				foreach (PMBudget budget in SelectBudgetRecords())
				{
				...
				foreach (PMCostCode item in costCodeSelect.Select())
				{
					//The PXSelectJoinGroupBy is read-only, and Cost Codes won't be added to the cache. Add them manually.
					costCodeSelect.Cache.SetStatus(item, PXEntryStatus.Notchanged);
				}
		...
		{
			PXSelect<PMBudget> selectBudget = new PXSelect<PMBudget>(this.Base);
			...
			using (new PXFieldScope(selectBudget.View, new Type[] { typeof(PMBudget.projectID), typeof(PMBudget.projectTaskID), typeof(PMBudget.accountGroupID), typeof(PMBudget.inventoryID), typeof(PMBudget.costCodeID),
				typeof(PMBudget.qty), typeof(PMBudget.amount), typeof(PMBudget.revisedQty), typeof(PMBudget.revisedAmount),
				typeof(PMBudget.committedAmount), typeof(PMBudget.committedInvoicedAmount), typeof(PMBudget.committedInvoicedQty), typeof(PMBudget.committedOpenAmount), typeof(PMBudget.committedOpenQty), typeof(PMBudget.committedQty), typeof(PMBudget.committedReceivedQty),
				typeof(PMBudget.actualQty), typeof(PMBudget.actualAmount)}))
			{
		...
		private object CalculateAndExpandValue(bool drilldown, RMDataSource ds, RMDataSourceGL dsGL, RMDataSourcePM dsPM, ARmDataSet dataSet, List<PMAccountGroup> accountGroups, List<PMProject> projects, List<PMTask> tasks, List<InventoryItem> items, List<PMCostCode> costCodes, List<object[]> splitret)
        {
            ...
            Parallel.For(0, accountGroups.Count, accountGroup =>
            {
                ...
                Parallel.For(0, projects.Count, project =>
                {
                    ...
                    Parallel.For(0, tasks.Count, task =>
                    {
						...
						Parallel.For(0, costCodes.Count, costCode =>
						{
							...
							Parallel.For(0, items.Count, item =>
							{
								...
								{                                    
									...
									if (ds.Expand == ExpandType.AccountGroup)
									{
										...
										{
											splitret[accountGroup][2] = (decimal)splitret[accountGroup][2] + hist.Item1;
									...
									else if (ds.Expand == ExpandType.Project)
									{
										lock (currentProject)
										{
											splitret[project][2] = (decimal)splitret[project][2] + hist.Item1;
									...
									else if (ds.Expand == ExpandType.ProjectTask)
									{
										lock (currentItem)
										{
											splitret[task][2] = (decimal)splitret[task][2] + hist.Item1;
									...
									else if (ds.Expand == ExpandType.Inventory)
									{
										lock (currentItem)
										{
											splitret[item][2] = (decimal)splitret[item][2] + hist.Item1;
		...
		private List<PMHistory> GetPeriodsToCalculate(PMProject project, PMTask task, PMAccountGroup accountGroup, InventoryItem item, PMCostCode costCode, RMDataSource ds, RMDataSourceGL dsGL, bool drilldown)
        {
