...
using PX.Objects.CR;
using PX.Objects.IN;
using PX.Objects.IN.Repositories;
using PX.Objects.PM;
using PX.Objects.CS;
...
namespace PX.Objects.CT
{
	public class CTBillEngine : PXGraph<CTBillEngine>
	{
		...
		public virtual string GetDescriptionMessageForInventory(InventoryAction action, InventoryItem inventory)
		{
			string actionMessage;
			switch (action)
			{
				case InventoryAction.Setup:
					actionMessage = PXMessages.LocalizeNoPrefix(Messages.SetupDescription);
					break;
				case InventoryAction.ActivateRenew:
					actionMessage = PXMessages.LocalizeNoPrefix(Messages.ActivateRenew);
					break;
				case InventoryAction.SetupUpgrade:
					actionMessage = PXMessages.LocalizeNoPrefix(Messages.SetupUpgrade);
					break;
				case InventoryAction.UpgradeActivation:
					actionMessage = PXMessages.LocalizeNoPrefix(Messages.UpgradeActivation);
					break;
				default:
					actionMessage = PXMessages.LocalizeNoPrefix(Messages.ActivateRenew);
					break;
			}
			actionMessage = string.Format(PXMessages.LocalizeNoPrefix(actionMessage),
					PXDBLocalizableStringAttribute.GetTranslation<InventoryItem.descr>(Caches[typeof(InventoryItem)], inventory));
		...
		public virtual string GetInvoiceDescription(string action, Contract contract, Customer customer)
		{
			string actionMessage;
			switch (action)
			{
				case ContractAction.Setup:
					actionMessage = PXMessages.LocalizeNoPrefix(Messages.SettingUpContract);
					break;
				case ContractAction.Activate:
					actionMessage = PXMessages.LocalizeNoPrefix(contract.IsPendingUpdate == true ? Messages.UpgradingContract : Messages.ActivatingContract);
					break;
				case ContractAction.Terminate:
					actionMessage = PXMessages.LocalizeNoPrefix(Messages.TerminatingContract);
					break;
				case ContractAction.Bill:
					return GetInvoiceDescriptionBilling(contract, customer);
				case ContractAction.Renew:
					return GetInvoiceDescriptionRenewing(contract, customer);
				default:
					actionMessage = PXMessages.LocalizeNoPrefix(Messages.ActivatingContract);
					break;
			}
			actionMessage = string.Format(actionMessage, contract.ContractCD,
					PXDBLocalizableStringAttribute.GetTranslation<Contract.description>(Caches[typeof(Contract)], contract, customer.LocaleName));
		...
		public virtual string GetTransactionDescriptionWithPrefix(string prefix, string description)
		{
			return string.IsNullOrEmpty(prefix) ? description : prefix + ": " + description;
		}
		...
		public CTBillEngine()
		{
			...
			inventoryItemRepository = new InventoryItemRepository(this);
		}
		...
		private void CreateNewRevision(int? contractID, string action, string newStatus)
		{
			...
			foreach (ContractDetail newdet in ContractDetails.Select(contract.ContractID)
				.RowCast<ContractDetail>()
				.Select(det => ContractDetails.Cache.CreateCopy(det) as ContractDetail))
			{
		...
		{
			ARInvoiceEntry invoiceEntry = CreateInstance<ARInvoiceEntry>();
			...
			foreach (InvoiceData invData in invoices)
			{
				...
				invoice.DocDesc = GetInvoiceDescription(action, contract, customer);
				...
				foreach (UsageData item in invData.UsageData.Where(IsBillable))
				{
					...
					tran.TranDesc = GetTransactionDescriptionWithPrefix(item.Prefix, item.Description);
					tran.UOM = item.UOM;
					tran.SalesPersonID = contract.SalesPersonID;
					tran.CaseID = item.CaseID;
		...
		private void SetDiscountsForTran(ARInvoiceEntry invoiceEntry, ARInvoice invoice, ARTran tran, string discountID)
		{
			...
			if (discountID != null && invoiceEntry.Transactions != null)
			{
				...
				DiscountEngine<ARTran>.UpdateManualLineDiscount<ARInvoiceDiscountDetail>(
					invoiceEntry.Transactions.Cache, invoiceEntry.Transactions, tran, invoiceEntry.ARDiscountDetails, invoice.BranchID, invoice.CustomerLocationID, invoice.DocDate);
			}
		}
		...
		public virtual void Bill(int? contractID, DateTime? date = null)
		{
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				if (data.Count > 0)
				{
					...
					invoice.CustomerID = customer.BAccountID;
					invoice.CustomerLocationID = location.LocationID;
					invoice.DocDesc = GetInvoiceDescription(ContractAction.Bill, contract, customer);
					...
					foreach (UsageData item in data.Where(IsBillable))
					{
			...
			using (PXTransactionScope ts = new PXTransactionScope())
			{
				if (data.Count > 0)
				{
					...
					ARInvoice invoice = invoiceEntry.Document.Insert(new ARInvoice
					{
						...
						DocDesc = GetInvoiceDescription(ContractAction.Renew, contract, customer),
		...
		protected virtual void GetUpgradeSetup(IEnumerable<PXResult<ContractDetail, ContractItem>> details, List<UsageData> usagedata, decimal? prorate)
		{
			foreach (PXResult<ContractDetail, ContractItem> res in details)
			{
				...
				{
					InventoryItem inventory = inventoryItemRepository.FindByID(item.BaseItemID);
					InventoryItem inventoryRenew = inventoryItemRepository.FindByID(item.RenewalItemID);
		...
		protected virtual void GetUpgradeRecurring(IEnumerable<PXResult<ContractDetail, ContractItem>> details, List<UsageData> usagedata, decimal? prorate)
		{
			foreach (PXResult<ContractDetail, ContractItem> res in details)
			{
				...
				{
					InventoryItem inventory = inventoryItemRepository.FindByID(item.RecurringItemID);
		...
		protected virtual void GetUpgradeUsage(Contract contract, IEnumerable<PXResult<ContractDetail, ContractItem>> details, List<UsageData> list, out Dictionary<int, List<TranNotePair>> sourceTran, out List<UsageData> tranData)
		{
			...
			foreach (KeyValuePair<int, List<TranNotePair>> kv in sourceTran)
				...
				PXResult<ContractDetail, ContractItem, InventoryItem> billing = details
					...
					.Select(@t => new { @t, inventory = inventoryItemRepository.FindByID(((ContractItem) @t.item).RecurringItemID) })
		...
		protected virtual List<UsageData> ProcessTransactions(Contract contract, Dictionary<int, List<TranNotePair>> transactions)
		{
			...
			foreach (KeyValuePair<int, List<TranNotePair>> kv in transactions)
			{
				...
				if (billing == null)
				{
					InventoryItem inventoryItem = inventoryItemRepository.FindByID(kv.Key);
					billing = new PXResult<ContractDetail, ContractItem, InventoryItem>(null, null, inventoryItem);
				}

				ContractItem item = billing;

				if (!(IsFirstBillAfterRenewalExpiredContractInGracePeriod(contract, schedule) && item.RecurringType != RecurringOption.Prepay))
				{
		...
		protected virtual void ProcessSingleRecord(Contract contract, TranNotePair tran, PXResult<ContractDetail, ContractItem, InventoryItem> res, List<UsageData> addedData)
		{
			...
			tranData.CaseID = tran.Tran.CaseID;
		...
		protected virtual void ProcessSummaryUsageItems(Contract contract, List<TranNotePair> trans, PXResult<ContractDetail, ContractItem, InventoryItem> res, List<UsageData> addedData)
		{
			...
			if (used > 0)
			{
				...
				tranData.IsTranData = true;
				tranData.CaseID = trans[0].Tran.CaseID;
		...
		protected virtual void UpdateReferencePMTran2ARTran(ARInvoiceEntry graph, Dictionary<int, List<TranNotePair>> sourceTran, List<UsageData> tranData)
		{
