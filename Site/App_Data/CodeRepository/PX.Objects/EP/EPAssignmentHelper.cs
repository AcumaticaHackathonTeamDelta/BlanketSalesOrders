			...
			foreach (EPRule rule in rules)
			{
				...
				switch (rule.RuleType)
				{
					...
					case EPRuleType.Document:
						...
						if (rule.OwnerSource == null || (filteredItems != null && filteredItems.Count == 0)) continue;
					...
					case EPRuleType.Filter:
						...
						if (filteredItems == null || filteredItems.Count == 0) continue;
			...
			try
			{
				List<EPRuleBaseCondition> conditions = 
					PXSelectReadonly<EPRuleCondition, 
						Where<EPRuleCondition.ruleID, Equal<Required<EPRule.ruleID>>>>
					.Select(this, rule.RuleID)
					.Select(_ => (EPRuleBaseCondition)_)
					.ToList();
				...
				Type resultViewType = CreateResultView(_Graph, conditions);
				

				PXView itemView = new PXView(_Graph, false, BqlCommand.CreateInstance(resultViewType),
