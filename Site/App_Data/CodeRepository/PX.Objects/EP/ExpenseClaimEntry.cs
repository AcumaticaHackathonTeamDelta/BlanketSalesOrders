...
namespace PX.Objects.EP
{
	public class ExpenseClaimEntry : PXGraph<ExpenseClaimEntry, EPExpenseClaim>, PXImportAttribute.IPXPrepareItems
	{
        ...
        public class EPExpenseClaimDetailsForSubmit : EPExpenseClaimDetails
        {
			...
			#endregion
			#region CuryTipAmt
			public new abstract class curyTipAmt : IBqlField
			{
			}
		...
		public partial class EPCustomerUpdateAsk : IBqlTable
		{
			...
			public abstract class customerUpdateAnswer : IBqlField
			{
			}
			...
			public abstract class newCustomerID : IBqlField
			{
			}
            ...
            public abstract class oldCustomerID : IBqlField
			{
			}
			...
			if (ExpenseClaim.Current != null)
			{
				foreach (EPExpenseClaimDetails item in ReceiptsForSubmit.Select())
				{
					if (item.Selected == true)
					{
						...
						EPClaimReceiptController.SubmitReceipt(ExpenseClaim.Cache, ExpenseClaimDetails.Cache, ExpenseClaim.Current, details);
					}
				}
			}
			ReceiptsForSubmit.Cache.Clear();
		...
		protected virtual IEnumerable ChangeOk(PXAdapter adapter)
		{
			...
			{
				var query = ExpenseClaimDetails.Select().Where(_ => ((EPExpenseClaimDetails)_).ContractID == null || ProjectDefaultAttribute.IsNonProject(((EPExpenseClaimDetails)_).ContractID));
		...
		public virtual IEnumerable ViewProject(PXAdapter adapter)
		{
			if (ExpenseClaimDetails.Current != null)
			{
				if (ProjectDefaultAttribute.IsProject(this, ExpenseClaimDetails.Current.ContractID))
				{
					...
					graph.Project.Current = graph.Project.Search<PMProject.contractID>(ExpenseClaimDetails.Current.ContractID);
		...
		public virtual IEnumerable ViewInvoice(PXAdapter adapter)
		{
			...
			{
				PXRedirectHelper.TryRedirect(APDocuments.Cache, APDocuments.Current, Messages.ViewInvoice, PXRedirectHelper.WindowMode.NewWindow);
			}
			return adapter.Get();
		}
		#endregion
		...
		public PXSelectReadonly<APInvoice,
			Where<APInvoice.origModule, Equal<BatchModule.moduleEP>,
			And<APInvoice.origRefNbr, Equal<Current<EPExpenseClaim.refNbr>>>>> APDocuments;
		...
		public PXSelect<EPTaxTran, Where<EPTaxTran.claimDetailID, Equal<Optional<EPExpenseClaimDetails.claimDetailID>>>> Tax_Rows;
        ...
        {
            IEnumerable<EPTaxTran> taxTrans = PXSelect<EPTaxTran, 
                Where<EPTaxTran.refNbr, Equal<Required<EPExpenseClaim.refNbr>>>>.Select(this, ExpenseClaim.Current.RefNbr).Select(_=>(EPTaxTran)_);
            var taxGroups = taxTrans.GroupBy(_ => _.TaxID);
            foreach (var items in taxGroups)
            {
                ...
                {
                    result.TaxRate = item.TaxRate;
                    result.TaxID = item.TaxID;
                    result.CuryTaxableAmt += item.ClaimCuryTaxableAmt;
                    result.CuryTaxAmt += item.ClaimCuryTaxAmt;
                    result.CuryExpenseAmt += item.ClaimCuryExpenseAmt;
                    result.TaxableAmt += item.TaxableAmt;
                    result.TaxAmt += item.TaxAmt;
                    result.ExpenseAmt += item.ExpenseAmt;
                    result.NonDeductibleTaxRate = item.NonDeductibleTaxRate;
                }
            ...
            foreach (EPTaxAggregate legacyRow in PXSelectReadonly<EPTaxAggregate, Where<EPTaxAggregate.refNbr, Equal<Required<EPExpenseClaimDetails.refNbr>>>>.Select(this, ExpenseClaim.Current.RefNbr))
            {
                if (Taxes.Locate(legacyRow) == null)
                {
                    Taxes.Cache.SetStatus(legacyRow, PXEntryStatus.Held);
                }
		...
		protected virtual void CurrencyInfo_RowUpdated(PXCache cache, PXRowUpdatedEventArgs e)
		{
			foreach (EPExpenseClaimDetails detail in ExpenseClaimDetails.Select())
			{
				if (detail.ClaimCuryInfoID != detail.CuryInfoID)
				{
					...
					EPClaimReceiptController.RecalcAmountInClaimCury(ExpenseClaimDetails.Cache, detail);
            ...
            if (row.TaxZoneID != oldRow.TaxZoneID)
            {
                ...
                if (e.ExternalCall && !this.IsMobile)
                {
                    if (!string.IsNullOrEmpty(row.TaxZoneID))
                    {
                        ...
                        string taxZoneID = EPClaimReceiptController.GetTaxZoneID(this, employee);
		...
		protected virtual void EPExpenseClaimDetails_RowInserted(PXCache cache, PXRowInsertedEventArgs e)
		{
			...
			if (ExpenseClaim.Current != null && row != null)
			{
				EPClaimReceiptController.SubmitReceipt(ExpenseClaim.Cache, cache, ExpenseClaim.Current, row);
			}
            EPClaimReceiptController.DeleteLegacyTaxRows(this, row.RefNbr);
        }
		...
		protected virtual void EPExpenseClaimDetails_RowDeleting(PXCache cache, PXRowDeletingEventArgs e)
		{
			...
			if (detail != null && cache.GetStatus(detail) != PXEntryStatus.InsertedDeleted)
			{
				EPClaimReceiptController.RemoveReceipt(ExpenseClaimDetails.Cache, detail);
				e.Cancel = true;
		...
		protected virtual void EPExpenseClaimDetails_RowSelected(PXCache cache, PXRowSelectedEventArgs e)
		{
			...
			if (row != null)
			{
				...
				Tax_Rows.Cache.SetAllEditPermissions(enabledEditReceipt);
		...
		{
			EPClaimReceiptController.ExpenseSubID_FieldDefaulting(sender, e, epsetup.Current.ExpenseSubMask);
		}

		protected virtual void EPExpenseClaimDetails_SalesSubID_FieldDefaulting(PXCache sender, PXFieldDefaultingEventArgs e)
		{
			EPClaimReceiptController.SalesSubID_FieldDefaulting(sender, e, epsetup.Current.SalesSubMask);
		}
		...
		protected virtual void EPExpenseClaimDetails_CuryTranAmt_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
		{
			EPClaimReceiptController.AmtFieldUpdated(cache, e);
		}
		protected virtual void EPExpenseClaimDetails_CuryTranAmtWithTaxes_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
		{
			EPClaimReceiptController.AmtFieldUpdated(cache, e);
		}
		protected virtual void EPExpenseClaimDetails_CuryTaxTotal_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
		{
			EPClaimReceiptController.AmtFieldUpdated(cache, e);
		}
		protected virtual void EPExpenseClaimDetails_CuryTaxRoundDiff_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
		{
			EPClaimReceiptController.AmtFieldUpdated(cache, e);
		}
		protected virtual void EPExpenseClaimDetails_CuryVATTaxableTotal_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
		{
			EPClaimReceiptController.AmtFieldUpdated(cache, e);
		}
		protected virtual void EPExpenseClaimDetails_CuryVATExemptTotal_FieldUpdated(PXCache cache, PXFieldUpdatedEventArgs e)
		{
			EPClaimReceiptController.AmtFieldUpdated(cache, e);
		}
        ...
        {
            EPClaimReceiptController.RecalcAmountInClaimCury(ExpenseClaimDetails.Cache, ExpenseClaimDetails.Current);
        }
        ...
        [PXDefault(typeof(EPExpenseClaim.curyID))]
