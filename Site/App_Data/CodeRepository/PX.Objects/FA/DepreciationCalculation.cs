...
namespace PX.Objects.FA
{
	public class DepreciationCalculation : PXGraph<DepreciationCalculation, FADepreciationMethod>
	{
		...
		// TODO: Temporary crutch, must be rmoved during depr. calc. unitesting refactoring
		[Obsolete("Temporary crutch, must be rmoved during depr. calc. unitesting refactoring")]
		private static FABookPeriod GetBookPeriod(PXGraph graph, string PeriodID, int? BookID)
		{
		...
		public virtual void CalculateDepreciationAddition(FixedAsset cls, FABookBalance bookbal, FADepreciationMethod method, FABookHistory next)
		{
			...
			{
				Params.Fill(bookbal, null, method);
			...
			if (bookbal.DeprToDate != null)
			{
				...
				// TODO: use DeprCalcParameters.GetBookPeriod during depr. calc. unitesting refactoring
				FABookPeriod per2 = GetBookPeriod(this, bookbal.DeprToPeriod, bookbal.BookID);
				...
				Params.Fill(bookbal2, null, method, additionEndDate);

				if (DateTime.Compare(Params.RecoveryEndDate, (DateTime)bookbal.DeprToDate) < 0)
				{
					bookbal.DeprToDate = Params.RecoveryEndDate;
					bookbal.DeprToPeriod = FABookPeriodIDAttribute.PeriodFromDate(this, Params.RecoveryEndDate, bookbal.BookID);
				}
			}
		...
		public virtual void CalculateDepreciation(FADepreciationMethod method, FABookBalance assetBalance, DateTime? additionEndDate = null)
		{
			...
			Params.Fill(assetBalance, book, method, additionEndDate);
			...
			if (histdict.TryGetValue(PeriodID, out hist) && hist.YtdSuspended > 0)
			{
				for (int i = 0; i <= hist.YtdSuspended; i++)
				{
					...
					newhist = new FABookHist
					{
						AssetID = assetBalance.AssetID,
						BookID = assetBalance.BookID,
						FinPeriodID = FABookPeriodIDAttribute.PeriodPlusPeriod(this, PeriodID, i, hist.BookID)
					};
		...
		private void SetSLDeprOther(bool writeToAsset, bool yearlyAccountancy, FADepreciationMethodLines line, int year, FABookBalance assetBalance, ref decimal otherDepreciation, ref decimal lastDepreciation, ref decimal rounding, ref DateTime previousEndDate)
		{
			...
			if (Params.AveragingConvention == FAAveragingConvention.FullDay)
			{
				...
				{
					if (i == Params.DepreciationStartPeriod && deprYear == Params.DepreciationStartYear) // first period
					{
		...
		private void SetNL2Depr(bool writeToAsset, bool yearlyAccountancy, FADepreciationMethodLines line, int year, FABookBalance assetBalance, ref decimal rounding)
		{
			...
			decimal yearDepreciation = (Params.DepreciationBasis - Params.AccumulatedDepreciation) * (Params.DepreciationMethod.PercentPerYear ?? 0);
			for (int i = fromPeriod; i <= depreciateToPeriod; i++)
			{
				if (Params.DepreciationBasis == Params.AccumulatedDepreciation) return;
				decimal depreciation = yearlyAccountancy
					? yearDepreciation/Params.DepreciationPeriodsInYear
					: (decimal)((double) (Params.DepreciationBasis - Params.AccumulatedDepreciation)*(1 - Math.Pow(1 - (double) (Params.DepreciationMethod.PercentPerYear ?? 0m), 1.0/Params.DepreciationPeriodsInYear)));
				Params.AccumulatedDepreciation += Round(depreciation);
				if (Round(depreciation) != (Params.DepreciationBasis - Params.AccumulatedDepreciation))
					rounding += depreciation - Round(depreciation);
				WhereToWriteDepreciation(writeToAsset, line, assetBalance, year, i, depreciation, true, ref rounding);
	...
	{
		public DeprCalcParameters Fill(FABookBalance balance, FABook book = null, FADepreciationMethod depreciationMethod = null, DateTime? recoveryEndDate = null)
		{
			if (balance.BookID == null)
	{
				return this;
			}

			return Fill(balance.BookID.Value,
		...
		protected virtual DeprCalcParameters SelectFromDatabase()
				{
			...
			PXSelectBase<FABookPeriod> periodsCmd = new PXSelect<FABookPeriod,
				...
				OrderBy<Asc<FABookPeriod.finPeriodID>>>(graph);
			PXSelectBase<FABookYear> yearsCmd = new PXSelect<FABookYear,
				Where<FABookYear.bookID, Equal<Required<FABookYear.bookID>>>,
				OrderBy<Asc<FABookYear.year>>>(graph);
			List<object> parms = new List<object> { BookID };
			...
			string deprToYear = GetDatePlusYears(DepreciationStartDate, UsefulLife).Year.ToString("0000");
			...
			YearSetup = FABookPeriodIDAttribute.GetBookCalendar(graph, Book);
		...
		protected virtual DeprCalcParameters Calculate()
		{
			...
			switch (AveragingConvention)
			{
				...
				case FAAveragingConvention.NextPeriod:
					...
					DepreciationStartPeriod = int.Parse(FiscalPeriodUtils.PeriodInYear(nextPeriodID));
					DepreciationStartYear = int.Parse(FiscalPeriodUtils.FiscalYear(nextPeriodID));
			...
			WholeRecoveryDays = (RecoveryEndDate - RecoveryStartDate).Days;
		...
		public string GetNextPeriodID(string periodID)
		{
			...
			int periodsInNextYear = GetPeriodsInYear(nextYear);
			if (nextPeriod > periodsInNextYear)
			{
				nextYear++;
				periodsInNextYear = GetPeriodsInYear(nextYear);
				nextPeriod = nextPeriod % periodsInNextYear;
			}

			return $"{nextYear:0000}{nextPeriod:00}";
		}
		...
		{
			return new DeprCalcParameters().Fill(assetBalance).RecoveryEndDate;
		}

		public static string GetRecoveryStartPeriod(PXGraph graph, FABookBalance assetBalance)
		{
			DeprCalcParameters parameters = new DeprCalcParameters().Fill(assetBalance);
			if (parameters.DepreciationStartYear == 0 && parameters.DepreciationStartPeriod == 0)
		{
				return null;
		}
			string startPeriodID = $"{parameters.DepreciationStartYear:0000}{parameters.DepreciationStartPeriod:00}";
			if (parameters.StartDepreciationMidPeriodRatio == 0m && (assetBalance.AveragingConvention == FAAveragingConvention.ModifiedPeriod || assetBalance.AveragingConvention == FAAveragingConvention.ModifiedPeriod2))
		{
