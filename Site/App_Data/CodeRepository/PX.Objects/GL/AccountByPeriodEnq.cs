...
namespace PX.Objects.GL
{
	...
	public partial class AccountByPeriodFilter : IBqlTable
	{
		...
		public abstract class includeReclassified : PX.Data.IBqlField
		{
		}
		...
		public abstract class unsignedBegBal : IBqlField {}
		[PXDBBaseCury(typeof(AccountByPeriodFilter.ledgerID))]
		[PXDefault(TypeCode.Decimal, "0.0")]
		public virtual decimal? UnsignedBegBal { get; set; }
		#endregion
		#region UnsignedCuryBegBal
		public abstract class unsignedCuryBegBal : IBqlField {}
        ...
        public abstract class periodstartDateUI : IBqlField { }
	...
	public partial class GLTranR : GLTran, ISignedBalances
	{
		...
		public new abstract class reclassifiedWithTranDate : IBqlField { }
	...
	public partial class ReclassBatch : Batch
	{
		...
		public new abstract class posted : PX.Data.IBqlField { }
	}
	...
	public class AccountByPeriodEnq : PXGraph<AccountByPeriodEnq>
	{
		...
		public FinPeriod CurrentStartPeriod => PXSelectReadonly<FinPeriod, Where<FinPeriod.finPeriodID, Equal<Current<AccountByPeriodFilter.startPeriodID>>>>.Select(this);
		...
		protected PXSelectBase<GLTranR> Command
		{
			get
			{
				...
				if (filter.ShowSummary == true)
				{
					cmd = new PXSelectJoinGroupBy<GLTranR,
						InnerJoin<Sub, On<GLTranR.subID, Equal<Sub.subID>,
							And<Match<Sub, Current<AccessInfo.userName>>>>,
						InnerJoin<Account, On<GLTranR.accountID, Equal<Account.accountID>,
							And<Match<Account, Current<AccessInfo.userName>>>>,
						LeftJoin<Batch, On<GLTranR.module, Equal<Batch.module>,
							And<GLTranR.batchNbr, Equal<Batch.batchNbr>>>>>>,
				...
				else
				{
					cmd = new PXSelectJoin<GLTranR,
						...
						Where<GLTranR.ledgerID, Equal<Current<AccountByPeriodFilter.ledgerID>>,
							...
							And<GLTranR.finPeriodID, LessEqual<Current<AccountByPeriodFilter.endPeriodID>>,
							And<Batch.voided, NotEqual<True>,
							And<Batch.scheduled, NotEqual<True>>>>>>>>(this);
				}
				...
				{
					cmd.WhereAnd<Where2<
						Where<GLTranR.isReclassReverse, Equal<False>>,
							And<Where<GLTranR.reclassified, Equal<False>,
							Or<Where<GLTranR.reclassified, Equal<True>,
								And<Where<GLTranR.reclassifiedWithTranDate, Less<Current<AccountByPeriodFilter.startDate>>,
								Or<GLTranR.reclassifiedWithTranDate, GreaterEqual<Current<AccountByPeriodFilter.endDate>>>>>>>>>>>();
				}
		...
		public virtual IEnumerable previousperiod(PXAdapter adapter)
		{
			...
			filter.StartPeriodID = nextperiod?.FinPeriodID;
			FinPeriod nextperiod2 = PXSelect<FinPeriod,
				...
				?? PXSelectOrderBy<FinPeriod,
					OrderBy<Desc<FinPeriod.finPeriodID>>>.SelectWindowed(this, 0, 1);
			filter.EndPeriodID = nextperiod2?.FinPeriodID;
		...
		public virtual IEnumerable ReclassifyAll(PXAdapter adapter)
		{
			...
			ReclassifyTransactionsProcess.TryOpenForReclassification<GLTranR>(this,
				trans,
				CurrentLedger,
				tran => tran.BatchType,
				GLTranEnq.View,
		...
		public virtual IEnumerable viewDocument(PXAdapter adapter)
		{
			...
			if (tran != null)
			{
				...
				JournalEntry.OpenDocumentByTran(tran, batch);
			}
		...
		protected virtual IEnumerable glTranEnq()
		{
			...
			var records = Command.View.Select(null, null, PXView.Searches, PXView.SortColumns, PXView.Descendings, TransactionFilters, ref startRow, isSyncPosition ? PXView.MaximumRows : 0, ref totalRows)
				.Cast<PXResult<GLTranR, Sub, Account ,Batch>>()
				.Select(result =>
				{
					GLTranR tran = result;
					Batch batch = result;
		...
		protected virtual IEnumerable filter()
		{
			...
			if (filter.AccountID != null && filter.LedgerID != null && filter.StartPeriodID != null && filter.EndPeriodID != null)
			{
				...
				if (account != null)
				{
					...
					decimal turnover = Command.View.Select(null, null, new object[PXView.SortColumns.Length], PXView.SortColumns, PXView.Descendings, TransactionFilters, ref startRow, 0, ref totalRows)
						.RowCast<GLTranR>()
		...
		[PXRestrictor(typeof(Where<True, Equal<True>>), Messages.BranchInactive, ReplaceInherited = true)]
			...
			if (filter?.AccountID != null && filter.LedgerID != null)
			{
				...
				if (period != null)
				{
					...
					if (filter.StartDate.HasValue)
					{
						...
						if (filter.BranchID != null)
						{
							cmd.WhereAnd<Where<GLTran.branchID, Equal<Current<AccountByPeriodFilter.branchID>>>>();
						}
		...
		public virtual PXGraph Create(string aTranType, string aRefNbr, int? referenceID)
		{
			switch(aTranType)
			{
				...
				case CATranType.CATransferExp:
				{
						...
						destGraph.CAAdjRecords.Current = PXSelect<CAAdj,
															Where<CAAdj.adjTranType, Equal<CATranType.cATransferExp>,
															  And<CAAdj.transferNbr, Equal<Required<CAAdj.adjRefNbr>>>>>.
														 Select(destGraph, aRefNbr);
						return destGraph;
				}
				...
				case CATranType.CAVoidDeposit:
				{
					...
					destGraph.Document.Current = destGraph.Document.Search<CADeposit.refNbr, CADeposit.tranType>(aRefNbr, aTranType);
					return destGraph;
				}
				default:
					return null;
	...
	public class DRDocGraphCreator : IDocGraphCreator
	{
		...
		{
			int scheduleID;
			if (!string.IsNullOrEmpty(aRefNbr) && int.TryParse(aRefNbr, out scheduleID))
			{
				DraftScheduleMaint destGraph = PXGraph.CreateInstance<DraftScheduleMaint>();
				destGraph.Schedule.Current = destGraph.Schedule.Search<DRSchedule.scheduleID>(scheduleID);
				return destGraph;
			}
			return null;
		}
	...
	public class INDocGraphCreator : IDocGraphCreator
	{
		...
		public virtual PXGraph Create(string aTranType, string aRefNbr, int? referenceID)
		{
			switch(aTranType)
			{
				...
				{
					INTransferEntry destGraph = PXGraph.CreateInstance<INTransferEntry>();
					destGraph.transfer.Current = destGraph.transfer.Search<INRegister.refNbr>(aRefNbr);
					return destGraph;
				}
