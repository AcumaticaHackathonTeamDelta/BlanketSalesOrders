...
namespace PX.Objects.GL
{
	public class GLBudgetTreeMaint : PXGraph<GLBudgetTreeMaint>
	{
		...
		private void deleteRecords(Guid? groupID)
		{
			...
			{
				if (item.GroupID != item.ParentGroupID)
        ...
        {        
            ...
            foreach (GLBudgetTree node in allNodes)
            {
                allNodesDict.Add(node, Details.Cache.GetStatus(node));
            }

            foreach (GLBudgetTree row in allNodes)
            {
                foreach (GLBudgetTree node in allNodes)
                {
                    if (row.GroupID != node.GroupID && allNodesDict[node] != PXEntryStatus.Notchanged)
                    {
                        ...
                        //Comparing parent and child masks 
                        if (node.ParentGroupID == row.GroupID && row.AccountMask != null && row.SubMask != null)
                        {
                            if (!MatchMask(node.AccountMask, row.AccountMask ?? string.Empty, false))
                            {
                                Details.Cache.RaiseExceptionHandling<GLBudgetTree.accountMask>(node, node.AccountMask, new PXSetPropertyException(String.Format(PXMessages.LocalizeNoPrefix(Messages.BudgetTreeIncorrectAccountMask), row.AccountMask), PXErrorLevel.Error));
                            }
                            else
                            {
                                if (!MatchMask(node.SubMask, row.SubMask ?? string.Empty, false))
                                {
                                    Details.Cache.RaiseExceptionHandling<GLBudgetTree.subMask>(node, node.SubMask, new PXSetPropertyException(String.Format(PXMessages.LocalizeNoPrefix(Messages.BudgetTreeIncorrectSubMask), row.SubMask), PXErrorLevel.Error));
                                }
                            }
                        }
