...
namespace PX.Objects.IN
{
	...
	public abstract class INItemPlanIDAttribute : INItemPlanIDBaseAttribute, IPXRowInsertedSubscriber, IPXRowUpdatedSubscriber, IPXRowDeletedSubscriber
	{
			...
			where TNode : class, IQtyAllocatedBase
		{
			...
			if (plan.IgnoreOrigPlan != true && !string.IsNullOrEmpty(plan.OrigPlanType))
			{
				...
				{
					INPlanType origPlanType = (INPlanType)PXSelectReadonly<INPlanType, Where<INPlanType.planType, Equal<Required<INPlanType.planType>>>>.Select(graph, plan.OrigPlanType);
		...
		where lotSerClass : IBqlField
	{
		...
		protected virtual void SalesUnit_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			if (salesunit == null && string.IsNullOrEmpty(TopGetValue<salesUnit, string>(e.Row)) == false)
			{
				...
				salesunit.PriceAdjustmentMultiplier = 1m;
				salesunit.UnitMultDiv = MultDiv.Multiply;
		...
		protected virtual void PurchaseUnit_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			if (purchaseunit == null && string.IsNullOrEmpty(TopGetValue<purchaseUnit, string>(e.Row)) == false)
			{
				...
				purchaseunit.PriceAdjustmentMultiplier = 1m;
				purchaseunit.UnitMultDiv = MultDiv.Multiply;
		...
		protected virtual void BaseUnit_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			if (string.Equals((string)e.OldValue, TopGetValue<baseUnit, string>(e.Row)) == false)
			{
				if (string.IsNullOrEmpty((string)e.OldValue) == false)
				{
					...
					baseunit.ItemClassID = TopGetValue<itemClassID, int?>(e.Row);
					baseunit.InventoryID = 0;
					baseunit.FromUnit = (string)e.OldValue;
					baseunit.ToUnit = (string)e.OldValue;
					baseunit.UnitRate = 1m;
					baseunit.PriceAdjustmentMultiplier = 1m;
					baseunit.UnitMultDiv = MultDiv.Multiply;
		...
		where lotSerClass : IBqlField
	{
		...
		protected virtual void SalesUnit_FieldUpdated(PXCache sender, PXFieldUpdatedEventArgs e)
		{
                ...
                && string.Equals((string)e.OldValue, TopGetValue<baseUnit, string>(e.Row)) == false)
            {
                ...
                oldunit.UnitRate = 1m;
                oldunit.PriceAdjustmentMultiplier = 1m;
                oldunit.UnitMultDiv = MultDiv.Multiply;
        ...
        protected virtual void INUnit_RowInserted(PXCache sender, PXRowInsertedEventArgs e)
		{
			...
			if (unit.UnitType == INUnitType.InventoryItem)
			{
				...
				if (global == null)
				{
					...
					global.ItemClassID = 0;
					global.UnitRate = 1m;
					global.PriceAdjustmentMultiplier = 1m;
					global.UnitMultDiv = MultDiv.Multiply;
		...
		where Where : IBqlWhere, new()
	{
        ...
        public virtual void Master_InventoryID_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
        {
            if (sender.Graph.UnattendedMode || sender.Graph.IsImport)
                return;
            var item = (InventoryItem)PXSelect<InventoryItem, Where<InventoryItem.inventoryID, Equal<Required<InventoryItem.inventoryID>>>>.SelectWindowed(sender.Graph, 0, 1, e.NewValue);
		...
		protected void IssueNumbersInt(PXCache sender, TLSMaster Row, PXResult<InventoryItem, INLotSerClass> item, decimal BaseQty)
		{
			...
			if ((GetTranTrackMode(Row, item) & INLotSerTrack.Mode.Issue) > 0)
			{
				...
				{
					int lotSerialIndex = lotSerialStatuses.FindIndex(x => ((INLotSerialStatus)((PXResult)x)[typeof(INLotSerialStatus)]).LotSerialNbr == Row.LotSerialNbr);
					if (lotSerialIndex >= 0)
					{
		...
		And<INTranSplit.refNbr, Equal<Current<INRegister.refNbr>>>>>
	{
        ...
        private void INTran_RowSelected(PXCache sender, PXRowSelectedEventArgs e)
        {
            ...
            InventoryItem ii = (InventoryItem)PXSelectorAttribute.Select<INTran.inventoryID>(MasterCache, MasterCache.Current);
        ...
        protected override void SetEditMode()
        {
            if (!Initialized || PrevCorrectionMode != CorrectionMode || PrevFullMode != FullMode)
            {
                ...
                if (PrevCorrectionMode = CorrectionMode)
                {
                    ...
                    PXUIFieldAttribute.SetEnabled(DetailCache, null, "SubItemID", true);
                ...
                if (PrevFullMode = FullMode)
                {
					...
					PXUIFieldAttribute.SetEnabled(MasterCache, null, "POReceiptNbr", false);
					PXUIFieldAttribute.SetEnabled(MasterCache, null, "POReceiptType", false);
					PXUIFieldAttribute.SetEnabled(MasterCache, null, "SOOrderNbr", false);
					PXUIFieldAttribute.SetEnabled(MasterCache, null, "SOShipmentNbr", false);

                    PXUIFieldAttribute.SetEnabled(DetailCache, null, true);
                    PXUIFieldAttribute.SetEnabled(DetailCache, null, "UOM", false);
					PXUIFieldAttribute.SetEnabled(DetailCache, null, "UnitCost", false);
                }
	...
	public class INUnitAttribute : AcctSub2Attribute, IPXFieldVerifyingSubscriber, IPXRowSelectedSubscriber, IPXRowPersistingSubscriber
	{
		...
		private PXSelectorAttribute _selector;

		public bool VerifyOnCopyPaste { get; set; } = true;

		private void InitSelector(Type searchType)
		{
			_selector = new PXSelectorAttribute(searchType);
		...
		{
			Type searchType = typeof(Search4<INUnit.fromUnit, Where<INUnit.unitType, Equal<INUnitType.global>>, Aggregate<GroupBy<INUnit.fromUnit>>>);
			InitSelector(searchType);
		}

		public INUnitAttribute(Type InventoryType, Type BaseUnitType)
		{
			_BaseUnitType = BaseUnitType;
			Type searchType = GenericCall.Of(() => BuildSearchByBaseUnit<IBqlField>()).ButWith(BaseUnitType);
			InitSelector(searchType);
		}

		private static Type BuildSearchByBaseUnit<TBaseUnit>() where TBaseUnit : IBqlField
		{
			return typeof(
				Search4<INUnit.fromUnit,
					Where<INUnit.unitType, Equal<INUnitType.global>,
						And<INUnit.toUnit, Equal<Optional<TBaseUnit>>>>,
		...
		{
			_InventoryType = InventoryType;
			Type searchType = GenericCall.Of(() => BuildSearchByInventory<IBqlField>()).ButWith(InventoryType);
			InitSelector(searchType);
		}

		private static Type BuildSearchByInventory<TInventoryID>() where TInventoryID : IBqlField
		{
			return typeof(
				Search4<INUnit.fromUnit,
					Where<INUnit.unitType, Equal<INUnitType.inventoryItem>,
						And<INUnit.inventoryID, Equal<Optional<TInventoryID>>,
							Or<INUnit.unitType, Equal<INUnitType.global>,
							And<Optional<TInventoryID>, IsNull>>>>,
					Aggregate<GroupBy<INUnit.fromUnit>>>);
		}
		...
		public virtual void FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			if (_FieldName != "FromUnit" && _FieldName != "ToUnit")
			{
				...
				{
					if (sender.Graph.IsCopyPasteContext == false || VerifyOnCopyPaste)
					{
					((IPXFieldVerifyingSubscriber)_Attributes[_SelAttrIndex]).FieldVerifying(sender, e);
				}
			}
			...
			where InventoryIDField : IBqlField
		{
			...
			if (item != null)
			{
				unit = PXSelect<INUnit, 
					...
					.Select(sender.Graph, item.InventoryID, item.BaseUnit, FromUnit);
			}
			else
			{
		...
		private static decimal Convert(PXCache sender, Int32? InventoryID, string FromUnit, decimal value, decimal? baseValue, INPrecision prec, bool ViceVersa)
		{
				...
				if (item != null)
				{
					unit = PXSelect<INUnit, 
						Where<INUnit.inventoryID, Equal<Required<INUnit.inventoryID>>, 
							And<INUnit.toUnit, Equal<Required<INUnit.toUnit>>, 
							And<INUnit.fromUnit, Equal<Required<INUnit.fromUnit>>>>>>
						.Select(sender.Graph, InventoryID, item.BaseUnit, FromUnit);
			}
			else
			{
			...
			{
				unit = PXSelectReadonly2<INUnit, 
					InnerJoin<InventoryItem, On<InventoryItem.inventoryID, Equal<INUnit.inventoryID>, 
						And<InventoryItem.baseUnit, Equal<INUnit.toUnit>>>>, 
					Where<INUnit.unitType, Equal<INUnitType.inventoryItem>, 
						And<InventoryItem.inventoryID, Equal<Required<InventoryItem.inventoryID>>, 
						And<INUnit.fromUnit, Equal<Required<INUnit.fromUnit>>>>>>
					.Select(sender.Graph, InventoryID, FromUnit);
			}
		...
		public static bool TryConvertGlobalUnits(PXGraph graph, string from, string to, decimal value, INPrecision prec, out decimal result)
		{
			...
			//result = ConvertValue(value, unit, prec, false, UsePriceAdjustmentMultiplier(graph));
			//return true;
		...
		private static decimal ConvertValue(decimal value, INUnit unit, INPrecision prec, bool viceVersa = false, bool usePriceAdjustmentMultiplier = false)
		{
	...
	public class CrossItemAttribute : InventoryAttribute, IPXFieldVerifyingSubscriber
	{
		...
		[Obsolete("Use FindAlternate(sender, alternateID)")]
		...
		{
			FoundInventory foundInventory = FindAlternate(sender, e.NewValue as string);
			e.NewValue = foundInventory?.InventoryCD ?? e.NewValue;
			AlternateID = foundInventory?.AlternateID;
			SubItemID = foundInventory?.SubItemCD;
		}
			...
			if (String.IsNullOrEmpty(alternateID) == false)
			{
				...
				PXSelectBase<INItemXRef> cmd =
					new PXSelectJoin<INItemXRef,
						...
						LeftJoin<INSubItem, On<INSubItem.subItemID, Equal<INItemXRef.subItemID>>>>,
	...
	public class PriceWorksheetAlternateItemAttribute : PXAggregateAttribute
	{
		...
		{
			var inventory = PXSelectorAttribute.Select(sender, priceWrapper.PriceWorksheetDetail, priceWrapper.InventoryIDField.Name) as InventoryItem;
	...
	public class NonStockItemAttribute : InventoryAttribute
	{
		...
		public static PXRestrictorAttribute CreateRestrictor()
			=> new PXRestrictorAttribute(typeof(Where<InventoryItem.stkItem, Equal<boolFalse>, And<InventoryItem.itemStatus, NotEqual<InventoryItemStatus.unknown>>>), Messages.InventoryItemIsAStock);
	...
	[PXDBInt]
	[PXUIField(DisplayName = "Inventory ID", Visibility = PXUIVisibility.Visible)]
	public class AnyInventoryAttribute : AcctSubAttribute
	{
		#region State
		public const string DimensionName = "INVENTORY";
		...
		public AnyInventoryAttribute()
			: this(typeof(Search<InventoryItem.inventoryID, Where2<Match<Current<AccessInfo.userName>>, And<InventoryItem.itemStatus, NotEqual<InventoryItemStatus.unknown>>>>), typeof(InventoryItem.inventoryCD), typeof(InventoryItem.descr))
		{
		}
	...
	[PXRestrictor(typeof(Where<InventoryItem.itemStatus, NotEqual<InventoryItemStatus.inactive>,
								And<InventoryItem.itemStatus, NotEqual<InventoryItemStatus.markedForDeletion>>>), Messages.InventoryItemIsInStatus, typeof(InventoryItem.itemStatus), ShowWarning = true)]
	public class InventoryAttribute : AnyInventoryAttribute
	{
		...
		[Obsolete]
		public InventoryAttribute(Type WhereType) : this(
			GenericCall.Of(() => BuildSearch<BqlNone>()).ButWith(WhereType),
			typeof(InventoryItem.inventoryCD),
			typeof(InventoryItem.descr)) {}

		private static Type BuildSearch<TWhere>() where TWhere : IBqlWhere, new()
			=> typeof(
		...
		[Obsolete]
		public InventoryAttribute(Type JoinType, Type WhereType) : this(
			GenericCall.Of(() => BuildSearch<BqlNone, BqlNone>()).ButWith(JoinType, WhereType),
			typeof(InventoryItem.inventoryCD),
			typeof(InventoryItem.descr)) {}

		private static Type BuildSearch<TJoin, TWhere>() 
	...
	public class PriceWorksheetInventoryAttribute : InventoryAttribute
	{
		...
		private static Type BuildSearch<TBAccount, TAlternateID, TAlternateType, TRestrictInventoryByAlternateID>()
			...
			Where2<
				...
				>>
			>,
			Aggregate<GroupBy<InventoryItem.inventoryID, GroupBy<INItemXRefExt.uOM>>>>);
	}
	#endregion
	...
	public class SiteAttribute : AcctSubAttribute
	{
        ...
        public class transitSiteID : Constant<Int32?>
        {
    ...
    public class ToSiteAttribute : SiteAttribute
	{
		...
		public ToSiteAttribute(Type transferTypeField)
		{
            ...
            SearchTypes.Add(typeof(Where<,,>));
            SearchTypes.Add(typeof(INSite.siteID));
            SearchTypes.Add(typeof(NotEqual<SiteAttribute.transitSiteID>));
            SearchTypes.Add(typeof(And<Match<Current<AccessInfo.userName>>>));
	...
	public class PXBaseQuantityAttribute : PXQuantityAttribute
	{
		internal override object Select(PXCache cache, object data)
		{
			...
			if (ret != null)
			{
				...
				ret.UnitMultDiv = (ret.UnitMultDiv == MultDiv.Multiply) ? MultDiv.Divide : MultDiv.Multiply;
			}
			return ret;
		}
					...
					if (!handled)
					{
						...
						if (conv != null && conv.UnitRate != 0m)
						{
							_ensurePrecision(sender, e.Row);
								...
								{
									decimal revValue = Math.Round((resultFieldCurrentValue ?? 0m) * (conv.UnitMultDiv == MultDiv.Multiply ? 1 / (decimal)conv.UnitRate : (decimal)conv.UnitRate), (int)_Precision, MidpointRounding.AwayFromZero);
		...
		public override void IsValidPeriod(PXCache sender, object Row, object NewValue)
		{
			...
			if (NewValue != null && _ValidatePeriod != PeriodValidation.Nothing)
			{
				...
				if (p.INClosed == true)
				{
					...
					{
                        sender.RaiseExceptionHandling(_FieldName, Row, null, new FiscalPeriodClosedException(p.FinPeriodID, PXErrorLevel.Warning, Messages.FinancialPeriodClosedInIN));
						return;
					}
					else
					{
                        throw new FiscalPeriodClosedException(p.FinPeriodID, Messages.FinancialPeriodClosedInIN);
					}
				}
			}
			return;
