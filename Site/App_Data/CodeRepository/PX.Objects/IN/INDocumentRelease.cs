...
namespace PX.Objects.IN
{
    ...
    public class INReleaseProcess : PXGraph<INReleaseProcess>
    {
        ...
        public PXSelectJoin<INItemPlan,
        ...
        public virtual void UpdateLocationStatus(INTran tran, INTranSplit split)
        {
            INPIStatus pistatus_rec =
                ...
                .Select(this, split.SiteID, split.InventoryID, split.LocationID);
        ...
        public virtual INTransitLine GetTransitLine(INTran tran)
        {
            ...
            if (ntlloc == null)
            {
                ...
                PXResult<INSite, INItemSite> res = (PXResult<INSite, INItemSite>)PXSelectJoin<INSite,
                    LeftJoinSingleTable<INItemSite,
                        On<INItemSite.inventoryID, Equal<Current<INTran.inventoryID>>,
                        And<INItemSite.siteID, Equal<INSite.siteID>>>>,
                    Where<INSite.siteID, Equal<Current<INTran.toSiteID>>>>.SelectSingleBound(this, new object[] { tran });
        ...
        {
            if (tran.InvtMult == (short)1 && tran.TranType != INTranType.Transfer && (split.InvtMult == (short)1 || item.ValMethod != INValMethod.Standard && !correctImbalance || item.ValMethod == INValMethod.Standard && correctImbalance) || (tran.InvtMult == (short)0 && !IsUnmanagedTran(tran)))
            {
                ...
                if (tran.TranType == INTranType.Receipt || !(item.ValMethod == INValMethod.Specific ||
                ...
                Dictionary<INCostStatus, INCostStatus> unmodifiedCostStatuses = new Dictionary<INCostStatus, INCostStatus>();

				//verify imbalance and accumulate changes for possible strategy change (receipt->issue)
                foreach (INCostStatus costStatus in costStatuses)
                {
                    if (costStatus.AccountID != layer.AccountID || costStatus.SubID != layer.SubID)
                        continue;

                        unmodifiedCostStatuses[costStatus] = (PXCache<INCostStatus>.CreateCopy(costStatus));
                        ...
                        if (split.BaseQty != 0m && tran.BaseQty != 0m && costStatus.QtyOnHand < 0m)
                        {
                            PXCache<INCostStatus>.RestoreCopy(layer, unmodifiedLayer);
                            foreach(var costPair in unmodifiedCostStatuses)
                            {
                                PXCache<INCostStatus>.RestoreCopy(costPair.Key, costPair.Value);
                            }
				...
				foreach (INCostStatus costStatus in costStatuses)
				{
					if (item.ValMethod == INValMethod.Specific ||
						item.ValMethod == INValMethod.Average || item.ValMethod == INValMethod.FIFO)
					{
						costStatus.TotalCost += (decimal)costtran.InvtMult * (costtran.TranCost - pretotal);
					}
				}
			}
        }
        ...
        protected virtual bool SameWarehouseTransfer(INTran tran, INTranSplit split)
        {
            ...
            if (split.FromSiteID == null && tran.OrigTranType != null && tran.OrigRefNbr != null && tran.OrigLineNbr != null)
            {
        ...
        public virtual void IssueCost(INTran tran, INTranSplit split, InventoryItem item, bool correctImbalance)
        {
            ...
            if (tran.InvtMult == (short)-1 || tran.InvtMult == (short)1 && split.InvtMult == (short)-1 && (item.ValMethod == INValMethod.Standard && !correctImbalance || item.ValMethod != INValMethod.Standard && correctImbalance) || (tran.TranType == INTranType.Transfer && !(IsOneStepTransfer() && split.InvtMult != (short)-1)))
            {
                object[] parameters;
                    ...
                    if (tran.InvtMult == (short)1 && QtyUnCosted > 0m)
                    {
                        ...
                        if (IsIngoingTransfer(tran))
                        {
                            if (split.ValMethod == INValMethod.Specific)
                                throw new PXException(Messages.StatusCheck_QtyTransitLotSerialOnHandNegative, intranselect.Cache.GetValueExt<INTran.inventoryID>(tran), costSubItemID, split.LotSerialNbr);
                            throw new PXException(Messages.StatusCheck_QtyTransitOnHandNegative, intranselect.Cache.GetValueExt<INTran.inventoryID>(tran), costSubItemID);
                        }
        ...
        public virtual void TransferCost(INTran tran, INTranSplit split, InventoryItem item, INCostStatus issueCost, INTranCost issueTranCost, decimal issuedQty, decimal issuedCost)
        {
            ...
            if (IsOneStepTransfer())
            {
                ...
                {
                    if (positivetran.OrigLineNbr == tran.LineNbr && positivetran.OrigRefNbr == tran.RefNbr && positivetran.OrigTranType == tran.TranType)
                    {
        ...
        public virtual void INItemPlan_RowPersisting(PXCache sender, PXRowPersistingEventArgs e)
        {
            ...
            {
                if(supplyplans.SelectSingle((e.Row as INItemPlan).PlanID)!=null)
                {
                    e.Cancel = true;
                } 
            }
        ...
        {
            foreach (PXResult<INItemPlan, INPlanType> demand_res in supplyplans.Select(oldPlanID))
            {
        ...
        public virtual void UpdateTransitPlans()
        {
            ...
            foreach (TransitLotSerialStatus status in transitlotnumberedstatus.Cache.Inserted)
            {
                ...
                foreach (INItemPlan oldplan in PXSelect<INItemPlan,
                    Where<INItemPlan.refNoteID, Equal<Current<INTransitLine.noteID>>,
                        ...
                        And<INItemPlan.lotSerialNbr, Equal<Current<TransitLotSerialStatus.lotSerialNbr>>>>>>>.SelectMultiBound(this, new object[] { tl, status }))
                {
                    //avoid ReadItem()
            ...
            foreach (TransitLocationStatus status in transitlocationstatus.Cache.Inserted)
            {
                ...
                foreach (INItemPlan oldplan in PXSelect<INItemPlan,
                    Where<INItemPlan.refNoteID, Equal<Current<INTransitLine.noteID>>,
                        ...
                        And<INItemPlan.subItemID, Equal<Current<TransitLocationStatus.subItemID>>>>>>.SelectMultiBound(this, new object[] { tl, status }))
                {
                    if (newplan == null)
                    {
                        newplan = (INItemPlan)initemplan.Cache.CreateCopy(oldplan);
        ...
        public virtual void UpdateARTranCost(INTran tran, decimal? TranCost)
        {
            if (tran.ARRefNbr != null)
            {
				...
				if (!(sosetup != null && sosetup.SalesProfitabilityForNSKits == SalesProfitabilityNSKitMethod.NSKitStandardCostOnly))
				{
        ...
        public virtual void UpdateSplitDestinationLocation(INTranSplit split, int? value)
        {
            ...
            split.SkipCostUpdate = IsOneStepTransferWithinSite();
            ...
            if (split.SkipCostUpdate ?? false)
            {
                ...
                INLocation whseToLoc = PXSelect<INLocation, Where<INLocation.locationID, Equal<Required<INLocation.locationID>>>>.SelectSingleBound(this, null, split.ToLocationID);
            ...
            INItemPlanIDAttribute.SetReleaseMode<INTranSplit.planID>(intransplit.Cache, true);
                ...
                using (PXTransactionScope ts = new PXTransactionScope())
                {
					var unattachedLCtran = (INTran)PXSelectJoin<INTran,
						...
						Where<INTran.docType, Equal<Current<INTran.docType>>,
							And<INTran.refNbr, Equal<Current<INTran.refNbr>>,
				...
				if (unattachedLCtran != null)
					{
						var ortran = PO.LandedCostHelper.GetOriginalInTran(this, unattachedLCtran.POReceiptNbr, unattachedLCtran.POReceiptLineNbr);

						throw new PXException(inregister.Current.IsPPVTran != true ? PO.Messages.INReceiptMustBeReleasedBeforeLCProcessing 
						: AP.Messages.INReceiptMustBeReleasedBeforePPV, ortran == null ? String.Empty : ortran.RefNbr, unattachedLCtran.POReceiptNbr);
					}
                    ...
                    foreach (INTransitLine res in
                        PXSelectJoin<INTransitLine,
                                InnerJoin<INTran, On<INTran.origRefNbr, Equal<INTransitLine.transferNbr>, And<INTran.origLineNbr, Equal<INTransitLine.transferLineNbr>>>>,
                                Where<INTran.docType, Equal<Current<INRegister.docType>>, And<INTran.refNbr, Equal<Current<INRegister.refNbr>>>>>.SelectMultiBound(this, new object[] { doc }))
                    {
                    }
                    ...
                    Where<INTranSplit.docType, Equal<Required<INTranSplit.docType>>, And<INTranSplit.refNbr, Equal<Required<INTranSplit.refNbr>>>>>.Select(this, doc.DocType, doc.RefNbr))
                    {
						...
						bool replanToTransit = (plantype.DeleteOnEvent != true && plantype.ReplanOnEvent == INPlanConstants.Plan42)
							|| (plantype.PlanType == INPlanConstants.Plan62 && doc.OrigModule == BatchModule.SO && doc.DocType == INDocType.Transfer);
						if (replanToTransit)
						{
					...
					.Select(this, doc.DocType, doc.RefNbr))
					{
						...
						{
							initemplan.Delete(plan);
							//used for 2-step transfer, so it's released and already has costsiteid and costsubitemid
							INTranSplitUpdate upd = new INTranSplitUpdate
							{
								TranType = split.TranType,
								RefNbr = split.RefNbr,
								LineNbr = split.LineNbr,
								CostSiteID = split.CostSiteID,
								CostSubItemID = split.CostSubItemID,
							};

							upd = intransplitupdate.Insert(upd);
							upd.ResetPlanID = true;
						}
					}
                    ...
                    var mainselect = new PXSelectJoin<INTran,
                        ...
                        OrderBy<Asc<INTran.tranType, Asc<INTran.refNbr, Asc<INTran.invtMult, Asc<INTran.lineNbr>>>>>>(this);

                    //For transfer its required to set up all the acceptor costsites and related info first, meanwhile ascending order is required for assembly
                    if (doc.DocType == INDocType.Transfer)
                        mainselect.OrderByNew<OrderBy<Asc<INTran.tranType, Asc<INTran.refNbr, Desc<INTran.invtMult, Asc<INTran.lineNbr>>>>>>();

                    foreach (PXResult<INTran, InventoryItem, INSite, INPostClass, INLotSerClass, INTranSplit, ReasonCode, INLocation> res in mainselect.Select(doc.DocType, doc.RefNbr))
                    {
                        INTran tran = (INTran)res;
                    ...
                    {
                        INSite insite = PXSelect<INSite, Where<INSite.siteID, Equal<Current<ItemCostHist.costSiteID>>>>.SelectSingleBound(this, new object[] { hist }); // probably should be replaced by equal<current<itemcosthist.siteid
...
namespace PX.Objects.IN.Overrides.INDocumentRelease
{
    ...
    public class INTranCostUpdateAccum : PXAccumulatorAttribute
    {
        ...
        public override bool PersistInserted(PXCache sender, object row)
        {
            ...
            catch (PXLockViolationException)
            {
                ...
                var it = (INTran)PXSelectReadonly<INTran, Where<INTran.tranType, Equal<Current<INTranCost.tranType>>, And<INTran.refNbr, Equal<Current<INTranCost.refNbr>>, And<INTran.lineNbr, Equal<Current<INTranCost.lineNbr>>>>>>
    ...
    public partial class INTranCostUpdate : IBqlTable
    {
        ...
        public abstract class resetOversoldFlag : IBqlField { }
        [PXBool()]
        [PXDefault(false)]
    ...
    public partial class INTranSplitUpdate : IBqlTable
    {
		...
		#region ResetPlanID
		public abstract class resetPlanID : PX.Data.IBqlField
		{
		}
	...
	public partial class SiteStatus : INSiteStatus, IQtyAllocated
	{
		...
		public abstract class initSiteStatus : PX.Data.IBqlField
		{
		}
    ...
    public partial class TransitSiteStatus : INSiteStatus, IQtyAllocated
    {
        ...
        [PXInt]
        [PXDBScalar(typeof(Search<InventoryItem.itemClassID, Where<InventoryItem.inventoryID, Equal<INSiteStatus.inventoryID>>>))]
        [PXDefault(typeof(Search<InventoryItem.itemClassID, Where<InventoryItem.inventoryID, Equal<Current<TransitSiteStatus.inventoryID>>>>), CacheGlobal = true, PersistingCheck = PXPersistingCheck.Nothing)]
        public virtual int? ItemClassID
        {
            get
            {
    ...
    public partial class TransitLocationStatus : INLocationStatus, IQtyAllocated, ICostStatus
    {
        ...
        [PXIdentitySubscriber(typeof(INTransitLine.costSiteID))]
        [PXDefault()]
        ...
        [PXDefault(typeof(Search<InventoryItem.itemClassID, Where<InventoryItem.inventoryID, Equal<Current<TransitLocationStatus.inventoryID>>>>), CacheGlobal = true, PersistingCheck = PXPersistingCheck.Nothing)]
        public virtual int? ItemClassID
        {
            get
            {
        ...
        protected bool? _NegQty;
        [PXBool()]
        [PXDefault(typeof(Select<INItemClass, Where<INItemClass.itemClassID, Equal<Current<TransitLocationStatus.itemClassID>>>>), CacheGlobal = true, SourceField = typeof(INItemClass.negQty), PersistingCheck = PXPersistingCheck.Nothing)]
        public virtual bool? NegQty
        {
            get
            {
        ...
        protected Boolean? _InclQtyAvail;
        [PXBool()]
        [PXDefault(typeof(Search<INLocation.inclQtyAvail, Where<INLocation.locationID, Equal<Current<TransitLocationStatus.locationID>>>>), PersistingCheck = PXPersistingCheck.Nothing)]
        public virtual Boolean? InclQtyAvail
        {
            get
            {
    ...
    public partial class ItemLotSerial : INItemLotSerial, IQtyAllocatedSeparateReceipts
    {
        ...
        protected bool? _NegQty;
        [PXBool()]
        [PXDefault(typeof(Select<INItemClass, Where<INItemClass.itemClassID, Equal<Current<ItemLotSerial.itemClassID>>>>), CacheGlobal = true, SourceField = typeof(INItemClass.negQty), PersistingCheck = PXPersistingCheck.Nothing)]
    ...
    public partial class SiteLotSerial : INSiteLotSerial, IQtyAllocatedBase
    {
        ...
        protected bool? _NegQty;
        [PXBool()]
        [PXDefault(typeof(Select<INItemClass, Where<INItemClass.itemClassID, Equal<Current<SiteLotSerial.itemClassID>>>>), SourceField = typeof(INItemClass.negQty), PersistingCheck = PXPersistingCheck.Nothing)]
    ...
    [CostStatusAccumulator(typeof(OversoldCostStatus.qtyOnHand), typeof(OversoldCostStatus.totalCost), typeof(OversoldCostStatus.inventoryID), typeof(OversoldCostStatus.costSubItemID), typeof(OversoldCostStatus.costSiteID), typeof(OversoldCostStatus.layerType), typeof(OversoldCostStatus.receiptNbr))]
    [Serializable]
    ...
    public class TransitSiteStatusAccumulatorAttribute : StatusAccumulatorAttribute
    {
        ...
        public override bool PersistInserted(PXCache sender, object row)
        {
            ...
            catch (PXLockViolationException)
            {
                ...
                TransitSiteStatus item = PXSelectReadonly<TransitSiteStatus, Where<TransitSiteStatus.inventoryID, Equal<Required<TransitSiteStatus.inventoryID>>, And<TransitSiteStatus.subItemID, Equal<Required<TransitSiteStatus.subItemID>>, And<TransitSiteStatus.siteID, Equal<Required<TransitSiteStatus.siteID>>>>>>.Select(sender.Graph, inventoryID, subItemID, siteID);
    ...
    public class TransitLocationStatusAccumulatorAttribute : StatusAccumulatorAttribute
    {
        ...
        public override bool PersistInserted(PXCache sender, object row)
        {
            ...
            catch (PXLockViolationException)
            {
                ...
                TransitLocationStatus item = PXSelectReadonly<TransitLocationStatus, Where<TransitLocationStatus.inventoryID, Equal<Required<TransitLocationStatus.inventoryID>>, And<TransitLocationStatus.subItemID, Equal<Required<TransitLocationStatus.subItemID>>, And<TransitLocationStatus.siteID, Equal<Required<TransitLocationStatus.siteID>>, And<TransitLocationStatus.locationID, Equal<Required<TransitLocationStatus.locationID>>>>>>>.Select(sender.Graph, inventoryID, subItemID, siteID, locationID);
					...
					PXSelectReadonly<INPIStatus,
						Where<INPIStatus.siteID, Equal<Required<INPIStatus.siteID>>,
						 ...
						 And2<Where<INPIStatus.inventoryID, IsNull,
							...
							And<Where<INPIStatus.locationID, IsNull,
										 Or<INPIStatus.locationID, Equal<Required<INPIStatus.locationID>>>>>>>>>
					.Select(sender.Graph, bal.SiteID, bal.InventoryID, bal.LocationID);

				if ((pistatus_rec != null))
				{
					throw new PXException(Messages.PICountInProgressDuringRelease,
    ...
    public class TransitLotSerialStatusAccumulatorAttribute : StatusAccumulatorAttribute
    {
        ...
        public override bool PersistInserted(PXCache sender, object row)
        {
            ...
            catch (PXLockViolationException)
            {
                ...
                TransitLotSerialStatus item = PXSelectReadonly<TransitLotSerialStatus, Where<TransitLotSerialStatus.inventoryID, Equal<Required<TransitLotSerialStatus.inventoryID>>, And<TransitLotSerialStatus.subItemID, Equal<Required<TransitLotSerialStatus.subItemID>>, And<TransitLotSerialStatus.siteID, Equal<Required<TransitLotSerialStatus.siteID>>, And<TransitLotSerialStatus.locationID, Equal<Required<TransitLotSerialStatus.locationID>>, And<TransitLotSerialStatus.lotSerialNbr, Equal<Required<TransitLotSerialStatus.lotSerialNbr>>>>>>>>.Select(sender.Graph, inventoryID, subItemID, siteID, locationID, lotSerialNbr);
    ...
    public class CostSiteIDAttribute : PXForeignSelectorAttribute
    {
        ...
        protected override object GetValueExt(PXCache sender, object item)
        {
            ...
            if (loc == null)
            {
                ...
                {
                    site = (INSite)PXSelect<INSite, Where<INSite.siteID, Equal<Required<INSite.siteID>>>>.SelectWindowed(sender.Graph, 0, 1, val);
                    return site.SiteCD;
                }
            }
