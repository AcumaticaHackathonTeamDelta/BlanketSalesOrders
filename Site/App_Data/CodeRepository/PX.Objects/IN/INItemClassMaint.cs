...
namespace PX.Objects.IN
{
	public class INItemClassMaint : PXGraph<INItemClassMaint, INItemClass>
	{
		public class ImmediatelyChangeID : PXImmediatelyChangeID<INItemClass, INItemClass.itemClassCD>
		{
			...
			[PXButton, PXUIField(DisplayName = "Change ID", MapEnableRights = PXCacheRights.Update, MapViewRights = PXCacheRights.Update)]
			protected override IEnumerable Handler(PXAdapter adapter)
			{
				try
				{
					var answer = Graph.Views[ChangeIdDialogView].Answer;
					if (answer == WebDialogResult.None)
					{
						Graph.Actions.PressSave();
					}
					else if (answer == WebDialogResult.Yes || answer == WebDialogResult.OK)
					{
						String newCD = GetNewCD(adapter);
						String oldCD = GetOldCD(adapter);
						if (oldCD != newCD &&
							PXSelectReadonly<INItemClass, Where<INItemClass.itemClassCD, Equal<Required<INItemClass.itemClassCD>>>>.Select(Graph, newCD).Count != 0)
							throw new PXException(Messages.DuplicateItemClassID);
					}
					base.Handler(adapter);
				}
				catch (Exception e) when (e is PXDialogRequiredException == false)
				{
				...
				Graph.Actions.PressCancel();
				yield return Graph.Caches[typeof(INItemClass)].Current;
			}
		}
		...
		public PXSelectReadonly<ItemClassTree.INItemClass> ItemClassNodes;
		...
		protected virtual void MergeAttributes(INItemClass child, IEnumerable<CSAttributeGroup> attributesTemplate)
		{
			...
			foreach (CSAttributeGroup attr in attributesTemplate)
			{
				var crudParamsTemplate = new PXDataFieldAssign[]
				{
					new PXDataFieldAssign<CSAttributeGroup.sortOrder>(attr.SortOrder),
					new PXDataFieldAssign<CSAttributeGroup.required>(attr.Required),
					new PXDataFieldAssign<CSAttributeGroup.defaultValue>(attr.DefaultValue),
					new PXDataFieldAssign<CSAttributeGroup.createdByID>(attr.CreatedByID),
					new PXDataFieldAssign<CSAttributeGroup.createdByScreenID>(attr.CreatedByScreenID),
					new PXDataFieldAssign<CSAttributeGroup.createdDateTime>(attr.CreatedDateTime),
					new PXDataFieldAssign<CSAttributeGroup.lastModifiedByID>(attr.LastModifiedByID),
					new PXDataFieldAssign<CSAttributeGroup.lastModifiedByScreenID>(attr.LastModifiedByScreenID),
					new PXDataFieldAssign<CSAttributeGroup.lastModifiedDateTime>(attr.LastModifiedDateTime),
				};

				if (attributesExisting.Any(r => r.AttributeID == attr.AttributeID && r.EntityType == attr.EntityType))
				{
					var updateParams = new List<PXDataFieldParam>(crudParamsTemplate);
					updateParams.Add(new PXDataFieldRestrict<CSAttributeGroup.entityClassID>(child.ItemClassStrID));
					updateParams.Add(new PXDataFieldRestrict<CSAttributeGroup.attributeID>(attr.AttributeID));
					updateParams.Add(new PXDataFieldRestrict<CSAttributeGroup.entityType>(attr.EntityType));
				...
				else
				{
					...
					insertParams.Add(new PXDataFieldAssign<CSAttributeGroup.attributeID>(attr.AttributeID));
					insertParams.Add(new PXDataFieldAssign<CSAttributeGroup.entityType>(attr.EntityType));
		...
		private IEnumerable<CSAttributeGroup> SelectCSAttributeGroupRecords(string itemClassID)
		{
			var resultSet = new PXSelectJoin<CSAttributeGroup,
				...
				.Select(itemClassID, typeof(InventoryItem).FullName);
			...
			var segs = PXSelectJoin<Segment,
				LeftJoin<SegmentValue,
					On<SegmentValue.dimensionID, Equal<Segment.dimensionID>,
						And<SegmentValue.segmentID, Equal<Segment.segmentID>, And<SegmentValue.isConsolidatedValue, Equal<boolTrue>>>>,
			...
			ItemClassTree.INItemClass.itemClassID>
	{
		...
		public class INItemClass : IN.INItemClass
		{
			...
			public new abstract class itemClassCD : IBqlField { }
			public new abstract class stkItem : IBqlField { }
