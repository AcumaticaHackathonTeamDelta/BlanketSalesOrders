...
namespace PX.Objects.PM
{
	...
	public class AllocationProcessByProject : PXGraph<AllocationProcessByProject>
	{
		...
		[PXHidden]
		[System.Diagnostics.CodeAnalysis.ExcludeFromCodeCoverage]
		public partial class Customer : PX.Data.IBqlTable
		{
			#region BAccountID
		...
		[PXProjection(typeof(
			...
			, InnerJoin<PMTask, On<PMProject.contractID, Equal<PMTask.projectID>, And<Where<PMTask.status, Equal<ProjectTaskStatus.active>, Or<PMTask.status, Equal<ProjectTaskStatus.completed>>>>>
				, InnerJoin<PMAllocation, On<PMAllocation.allocationID, Equal<PMTask.allocationID>, And<PMAllocation.isActive, Equal<True>>>
					, InnerJoin<PMAllocationStep, On<PMAllocationStep.allocationID, Equal<PMAllocation.allocationID>, And<PMAllocationStep.selectOption, Equal<PMSelectOption.transaction>>>
						, LeftJoin<PMAccountGroupFrom, On<PMAccountGroupFrom.groupID, Equal<PMAllocationStep.accountGroupFrom>>
							, LeftJoin<PMAccountGroupTo, On<PMAccountGroupTo.groupID, Equal<PMAllocationStep.accountGroupTo>>
								, LeftJoin<PMAccountGroup, On<PMAccountGroup.groupCD, Equal<PMAccountGroupFrom.groupCD>, Or<PMAccountGroup.groupCD, Equal<PMAccountGroupTo.groupCD>, Or<PMAccountGroup.groupCD, Between<PMAccountGroupFrom.groupCD, PMAccountGroupTo.groupCD>>>>
									, LeftJoin<PMTran, On<PMTran.projectID, Equal<PMProject.contractID>, And<PMTran.taskID, Equal<PMTask.taskID>, And<PMTran.allocated, Equal<False>, And<PMTran.released, Equal<True>, And<PMTran.accountGroupID, Equal<PMAccountGroup.groupID>>>>>>
					...
					, And<PMProject.isActive, Equal<True>
						, And<PMProject.nonProject, Equal<False>
							, And2<Where<PMProject.status, Equal<ProjectStatus.active>, Or<PMProject.status, Equal<ProjectStatus.completed>>>
								, And<PMAllocationStep.method, Equal<PMMethod.budget>, Or<PMTran.tranID, IsNotNull>>
								>
							>
						>
					>
		...
		[System.Diagnostics.CodeAnalysis.ExcludeFromCodeCoverage]
		public partial class PMProjectForAllocation : PX.Data.IBqlTable
		{
			#region Selected
