...
namespace PX.Objects.PM
{
	...
	public class PMAllocator : PXGraph<PMAllocator>, IRateTable
	{
		...
		public virtual void Execute(PMTask task, bool preselectTransactions)
		{
			...
			foreach (PMAllocationStep step in PXSelect<PMAllocationStep, Where<PMAllocationStep.allocationID, Equal<Required<PMAllocationStep.allocationID>>>, OrderBy<Asc<PMAllocationStep.stepID>>>.Select(this, task.AllocationID))
			{
				try
				{
		...
		public virtual void PreSelectTaskTransactions(PMTask task)
		{
			...
			{
				List<PMAllocationStep> steps = new List<PMAllocationStep>();
				info = new AllocationInfo(distinctAccountGroups, distinctRateTypes, steps);
				allocationInfo.Add(task.AllocationID, info);

				foreach (PMAllocationStep step in PXSelect<PMAllocationStep, Where<PMAllocationStep.allocationID, Equal<Required<PMAllocationStep.allocationID>>>,
						OrderBy<Asc<PMAllocationStep.stepID>>>.Select(this, task.AllocationID))
				{
		...
		public virtual bool PreSelectTasksTransactions(List<PMTask> tasks)
		{
			...
			foreach(string allocationID in distinctAllocationRules)
			{
				...
				if (!allocationInfo.TryGetValue(allocationID, out info))
				{
					...
					foreach (PMAllocationStep step in PXSelect<PMAllocationStep, Where<PMAllocationStep.allocationID, Equal<Required<PMAllocationStep.allocationID>>>,
							OrderBy<Asc<PMAllocationStep.stepID>>>.Select(this, allocationID))
					{
						if (step.Method == PMMethod.Budget)
							info.ContainsBudgetStep = true;
						foreach (int ag in GetAccountGroups(step))
		...
		/// <returns></returns>
		public virtual HashSet<int> GetAccountGroups(PMAllocationStep step)
		{
			HashSet<int> set = new HashSet<int>();
			if (step.SelectOption == PMSelectOption.Step)
			{
				foreach (PMAllocationStep innerStep in GetSteps(step.AllocationID, step.RangeStart, step.RangeEnd))
				{
		...
		/// Method do not query database.
		/// </summary>
		public virtual IList<int> GetAccountGroupsRange(PMAllocationStep step)
		{
		...
		/// Method do not query database.
		/// </summary>
		public virtual IList<PMAllocationStep> GetSteps(string allocationID, int? rangeStart, int? rangeEnd)
		{
			List<PMAllocationStep> list = new List<PMAllocationStep>();
			foreach (PMAllocationStep innerStep in allocationInfo[allocationID].Steps)
			{
		...
		public virtual void NonGlBillLaterAndReverse(string refNbr, List<PXResult<ARTran, PMTran>> billLater, List<PXResult<ARTran, PMTran>> reverseList, List<Tuple<PMProformaTransactLine, PMTran>> billLaterNotAllocated, List<PMTran> reverseToNonbillable)
		{
			if (billLater == null) throw new ArgumentNullException("billLater");
			...
			//Bill later Remainder if any:
			foreach (PXResult<ARTran, PMTran> item in billLater)
			{
				ARTran ar = (ARTran)item;
				PMTran pm = (PMTran)item;
		...
		public virtual bool ProcessStep(PMTask task, PMAllocationStep step)
		{
			if (step.Post == true)
			{
				...
				{
					List<PMTranWithTrace> allocated = ProcessBudgetStep(step, task);
		...
		public virtual void Post(PMAllocationStep step, PMTask task, List<PMTran> sourceList, List<PMTran> allocatedList)
		{
		...
		public virtual List<PMTranWithTrace> PostFullDetail(PMAllocationStep step, PMTask task, List<PMTran> list, List<PMTran> allocatedList, AllocatedService allocService)
		{
		...
		public bool IsWipStep(PMAllocationStep step, PMTask task)
		{
		...
		public virtual List<PMTranWithTrace> PostSummary(PMAllocationStep step, PMTask task, List<PMTran> fullList, List<PMTran> allocatedList, AllocatedService allocService)
		{
			...
			foreach (Group group in groups)
			{
				...
				foreach (PMTran tr in group.List)
				{
					...
					if(tr.InventoryID != PMInventorySelectorAttribute.EmptyInventoryID && !string.IsNullOrEmpty(tr.UOM))
					{
	   ...
	   public virtual void PostAllocatedTrans(PMAllocationStep step, List<PMTranWithTrace> allocated)
		{
		...
		public List<PMTranWithTrace> ProcessBudgetStep(PMAllocationStep step, PMTask task)
		{
					...
					foreach (PMBudget item in ps)
					{
						...
						foreach (PMBudgetAccum psa in this.Caches[typeof(PMBudgetAccum)].Inserted)
						{
									...
									psa.InventoryID == item.InventoryID)
							{
								...
								{
									recordCount++;
									actualAmount += psa.ActualAmount.GetValueOrDefault(0);
									revisedAmount += psa.RevisedAmount.GetValueOrDefault(0);
								}
							}
						}
					}
			...
			foreach (AllocData ad in data)
			{
				...
				if (unallocatedAmt != 0)
				{
					tranAdded.AddRange(AllocateBudget(task, step, ad.AccountGroupID, ad.InventoryID, ad.UOM, unallocatedAmt, quantity));
				}
			}
		...
		public virtual List<PMTranWithTrace> AllocateBudget(PMTask task, PMAllocationStep step, int origAccountGroupID, int inventoryID, string UOM, decimal amount, decimal quantity)
		{
			List<PMTranWithTrace> tranAdded = new List<PMTranWithTrace>();
			/*
				preconditions:
			...
			*/
			...
			if (tran.Qty != null && tran.Qty != 0)
			{
				...
				decimal q = PXDBQuantityAttribute.Round(tran.Qty.Value);
				decimal unitRate = PXDBPriceCostAttribute.Round(a / q);
			...
			tran.Allocated = step.MarkAsNotAllocated != true;
			...
			catch (PXFieldProcessingException ex)
			{
				...
				{
					throw new PXException(PXMessages.LocalizeNoPrefix(Messages.LocationNotFound) + " " + ex.Message);
				}
				else
				{
					throw new PXException(PXMessages.LocalizeNoPrefix(Messages.GenericFieldErrorOnAllocation) + " " + ex.Message);
				}
			}
			...
			if (additionalNonGLCreditTranIsRequired)
			{
				...
				if (creditTran.BillableQty != null && creditTran.BillableQty != 0)
				{
					...
					decimal q = PXDBQuantityAttribute.Round(creditTran.BillableQty.Value);
					decimal unitRate = PXDBPriceCostAttribute.Round(a / q);
		...
		public virtual void AddSourceTrans(PMAllocationStep step, List<PMTran> sourceTrans)
		{
		...
		public virtual void AddAllocationDocument()
		{
			Document.Cache.Insert();
			Document.Current.OrigDocType = PMOrigDocType.Allocation;
			Document.Current.Description = PXMessages.LocalizeNoPrefix(Messages.Allocation);
			Document.Current.IsAllocation = true;
		}
		
		public virtual string GetConcatenatedDescription(PMAllocationStep step, PMTran tran, PMTask task)
		{
			string result = "";
		...
		public virtual List<Group> BreakIntoGroups(PMAllocationStep step, List<PMTran> fullList)
		{
			List<PMTran> list;
		...
		public virtual Grouping CreateGrouping(PMAllocationStep step)
		{
		...
		public virtual bool CanAllocate(PMAllocationStep step, PMTran tran)
		{
			bool result = false;
		...
		public virtual IList<PMTran> Transform(PMAllocationStep step, PMTask task, PMTran original, bool calculateFormulas, bool isWipStep)
		{
			...
			if (doubleEntry)
			{
				...
				debitTran.Allocated = step.MarkAsNotAllocated != true;
				...
				if (calculateFormulas)
				{
					...
					if (billableQty != null && billableQty != 0)
					{
						...
						decimal q = PXDBQuantityAttribute.Round(billableQty.Value);
						decimal unitRate = PXDBPriceCostAttribute.Round(a / q);
				...
				if (creditAccountGroup != null)
				{
					...
					creditTran.Allocated = step.MarkAsNotAllocated != true;
			...
			else
			{
				...
				if (calculateFormulas)
				{
					...
					if (billableQty != null && billableQty != 0)
					{
						...
						decimal unitRate = PXDBPriceCostAttribute.Round(a / q);
						tran.UnitRate = unitRate;
						tran.Billable = original.Billable == true;
						tran.UseBillableQty = original.Billable == true;
					}
					tran.Description = desc;
				}
				list.Add(tran);
		...
		public virtual PMTran CreateFromTemplate(PMAllocationStep step, PMTran original, bool isWipStep)
		{
			PMTran tran = new PMTran();
		...
		public virtual void CalculateFormulas(PMDataNavigator navigator, PMAllocationStep step, PMTran tran, out decimal? qty, out decimal? billableQty, out decimal? amt, out string description)
		{
			qty = null;
			billableQty = null;
			amt = null;
		...
		/// Method do not query database.
		/// </summary>
		public virtual List<PMTran> Select(PMAllocationStep step, int? projectID, int? taskID)
		{
			List<PMTran> list;
		...
		/// Method do not query database.
		/// </summary>
		public virtual List<PMTran> GetTranByStep(PMAllocationStep step, int? projectID, int? taskID)
		{
		...
		public virtual decimal? GetRate(PMAllocationStep step, PMTran tran, string rateTableID)
		{
		...
		public class AllocationInfo
		{
			...
			public List<PMAllocationStep> Steps  { get; private set; }
			public bool ContainsBudgetStep { get; set; }

			public AllocationInfo(HashSet<int> accountGroups, HashSet<string> rateTypes, List<PMAllocationStep> steps)
			{
