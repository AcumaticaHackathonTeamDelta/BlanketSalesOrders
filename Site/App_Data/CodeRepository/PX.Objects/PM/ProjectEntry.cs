...
{
	[Serializable]
	public class ProjectEntry : PXGraph<ProjectEntry, PMProject>, PXImportAttribute.IPXPrepareItems
	{
		#region Inner DACs
		...
		[PXMergeAttributes(Method = MergeMethod.Merge)]
		[PXUIField(DisplayName = "AR Doc. Amount")]
		protected virtual void ARInvoice_OrigDocAmt_CacheAttached(PXCache sender) { }
		...
		public PXSelect<PMCostBudget, Where<PMCostBudget.projectID, Equal<Current<PMProject.contractID>>>,
			OrderBy<Asc<PMCostBudget.sortOrder, Asc<PMCostBudget.projectID, Asc<PMCostBudget.projectTaskID, Asc<PMCostBudget.inventoryID, Asc<PMCostBudget.costCodeID, Asc<PMCostBudget.accountGroupID>>>>>>>> CostBudget;
		...
		public IEnumerable costBudget()
		{
			var selectCostBudget = new PXSelect<PMCostBudget, Where<PMCostBudget.projectID, Equal<Current<PMProject.contractID>>, And<PMCostBudget.type, Equal<GL.AccountType.expense>,
			  And<Where<Current<CostBudgetFilter.projectTaskID>, IsNull, Or<Current<CostBudgetFilter.projectTaskID>, Equal<PMCostBudget.projectTaskID>>>>>>>(this);
			...
			{
				return selectCostBudget.Select();
			}
		}
		...
		[PXImport(typeof(PMProject))]
		[PXFilterable]
		public PXSelect<PMRevenueBudget, Where<PMRevenueBudget.projectID, Equal<Current<PMProject.contractID>>>,
			OrderBy<Asc<PMRevenueBudget.sortOrder, Asc<PMRevenueBudget.projectID, Asc<PMRevenueBudget.projectTaskID, Asc<PMRevenueBudget.inventoryID, Asc<PMRevenueBudget.costCodeID, Asc<PMRevenueBudget.accountGroupID>>>>>>>> RevenueBudget;
		...
		public IEnumerable revenueBudget()
		{
			var selectRevenueBudget = new PXSelect<PMRevenueBudget, Where<PMRevenueBudget.projectID, Equal<Current<PMProject.contractID>>, And<PMRevenueBudget.type, Equal<GL.AccountType.income>,
			  And<Where<Current<RevenueBudgetFilter.projectTaskID>, IsNull, Or<Current<RevenueBudgetFilter.projectTaskID>, Equal<PMRevenueBudget.projectTaskID>>>>>>>(this);

			if (IsRevenueGroupByTask())
			{
				var list = new List<PMRevenueBudget>(selectRevenueBudget.Select().RowCast<PMRevenueBudget>());
				return AggregateBudget<PMRevenueBudget>(list);
			}
			else
			{
				return selectRevenueBudget.Select();
			}
		}
		...
		public PXSelect<CSAnswers, Where<CSAnswers.refNoteID, Equal<Required<PMTask.noteID>>>> Answer;
		...
		public virtual IEnumerable ValidateAddresses(PXAdapter adapter)
		{
			foreach (PMProject current in adapter.Get<PMProject>())
			{
				if (current != null)
				{
					...
					{
						if (PXAddressValidator.Validate<PMAddress>(this, address, true))
							needSave = true;
					}
		...
		public PXChangeID<PMProject, PMProject.contractCD> ChangeID;
		...
		public virtual IEnumerable AddTasks(PXAdapter adapter)
		{
			foreach (SelectedTask task in TasksForAddition.Cache.Updated)
			{
				...
				{
					CopyTask(task, (int)ProjectProperties.Current.ContractID);
					task.Selected = false;
				}
			}
			return adapter.Get();
		...
		public virtual IEnumerable CreateTemplate(PXAdapter adapter)
		{
			...
			Dimension dm = PXSelect<Dimension, Where<Dimension.dimensionID, Equal<Required<Dimension.dimensionID>>>>.Select(this, ProjectAttribute.DimensionNameTemplate);
			...
			else
			{
				...
				foreach (Segment segment in PXSelect<Segment, Where<Segment.dimensionID, Equal<Required<Segment.dimensionID>>>>.Select(this, ProjectAttribute.DimensionNameTemplate))
				{
			...
			foreach (PMTask task in PXSelect<PMTask, Where<PMTask.projectID, Equal<Current<PMProject.contractID>>>>.SelectMultiBound(this, new object[] { project }))
			{
				...
				foreach (PMCostBudget budget in selectCostBudget.Select(task.ProjectID, task.TaskID))
				{
					...
					budget.ActualQty = 0;
					budget.AmountToInvoice = 0;
					budget.ChangeOrderAmount = 0;
					budget.ChangeOrderQty = 0;
					budget.CommittedAmount = 0;
					budget.CommittedInvoicedAmount = 0;
					budget.CommittedOpenAmount = 0;
				...
				foreach (PMDetail detail in PXSelect<PMDetail, Where<PMDetail.contractID, Equal<Required<PMTask.projectID>>, And<PMDetail.taskID, Equal<Required<PMTask.taskID>>>>>.Select(this, task.ProjectID, task.TaskID))
				{
					PMDetail newDetail = new PMDetail();
					newDetail.ContractID = dst.ProjectID;
					...
					newDetail.ItemFee = detail.ItemFee;
					newDetail.CuryItemFee = detail.CuryItemFee;
					newDetail.CuryInfoID = detail.CuryInfoID;
		...
		public IEnumerable ViewCostTransactions(PXAdapter adapter)
		{
			if (CostBudget.Current != null)
			{
				...
				if (CostBudget.Current.InventoryID != null && !ProjectDefaultAttribute.IsNonProject(CostBudget.Current.InventoryID))
		...
		public IEnumerable ViewRevenueTransactions(PXAdapter adapter)
		{
			if (RevenueBudget.Current != null)
			{
				...
				if (RevenueBudget.Current.InventoryID != null && !ProjectDefaultAttribute.IsNonProject(RevenueBudget.Current.InventoryID))
		...
		public ProjectEntry()
		{
			...
			action.AddMenuAction(lockBudget);
			action.AddMenuAction(unlockBudget);
		...
		protected virtual void _(Events.RowDeleting<PMTask> e)
		{
			...
			if (e.Row.IsActive == true && e.Row.IsCancelled == false)
			{
			...
			PMTran tran = PXSelect<PMTran, Where<PMTran.projectID, Equal<Required<PMTask.projectID>>, And<PMTran.taskID, Equal<Required<PMTask.taskID>>>>>.SelectWindowed(this, 0, 1, e.Row.ProjectID, e.Row.TaskID);
			if (tran != null)
			{
				throw new PXException(Messages.HasTranData);
			}

			PMTimeActivity activity = PXSelect<PMTimeActivity, Where<PMTimeActivity.projectID, Equal<Required<PMTask.projectID>>, And<PMTimeActivity.projectTaskID, Equal<Required<PMTask.taskID>>>>>.SelectWindowed(this, 0, 1, e.Row.ProjectID, e.Row.TaskID);
			if (activity != null)
			{
				throw new PXException(Messages.HasActivityData);
			}

			EP.EPTimeCardItem timeCardItem = PXSelect<EP.EPTimeCardItem, Where<EP.EPTimeCardItem.projectID, Equal<Required<PMTask.projectID>>, And<EP.EPTimeCardItem.taskID, Equal<Required<PMTask.taskID>>>>>.SelectWindowed(this, 0, 1, e.Row.ProjectID, e.Row.TaskID);
		...
		protected virtual void _(Events.FieldSelecting<PMTask, PMTask.completedPercent> e)
		{
			if (e.Row != null && e.Row.CompletedPctMethod != PMCompletedPctMethod.Manual)
			{
				...
				PXResultset<PMCostBudget> ps = select.Select(e.Row.ProjectID, e.Row.TaskID);
				...
				if (ps != null)
				{
					...
					{
						if (e.Row.CompletedPctMethod == PMCompletedPctMethod.ByQuantity && item.RevisedQty > 0)
						{
							recordCount++;
							percentSum += Convert.ToDouble(100 * item.ActualQty / item.RevisedQty);
						}
						else if (e.Row.CompletedPctMethod == PMCompletedPctMethod.ByAmount)
						{
							recordCount++;
							actualAmount += item.ActualAmount.GetValueOrDefault(0);
							budgetedAmount += item.RevisedAmount.GetValueOrDefault();
						}
					}
					if (e.Row.CompletedPctMethod == PMCompletedPctMethod.ByAmount)
						e.ReturnValue = budgetedAmount == 0 ? 0 : Convert.ToDecimal(100 * actualAmount / budgetedAmount);
					else
		...
		protected virtual void _(Events.RowSelected<PMProject> e)
		{
			...
			if (e.Row != null)
			{
				...
				viewCostCommitments.SetVisible(Setup.Current.CommitmentTracking == true);
				viewRevenueCommitments.SetVisible(Setup.Current.CommitmentTracking == true);
				viewCommitments.SetVisible(Setup.Current.CommitmentTracking == true);
				PXUIFieldAttribute.SetVisible<PMCostBudget.committedAmount>(CostBudget.Cache, null, Setup.Current.CommitmentTracking == true);
				PXUIFieldAttribute.SetVisible<PMCostBudget.committedQty>(CostBudget.Cache, null, Setup.Current.CommitmentTracking == true && CostQuantityVisible());
				PXUIFieldAttribute.SetVisible<PMCostBudget.committedInvoicedAmount>(CostBudget.Cache, null, Setup.Current.CommitmentTracking == true);
				PXUIFieldAttribute.SetVisible<PMCostBudget.committedInvoicedQty>(CostBudget.Cache, null, Setup.Current.CommitmentTracking == true && CostQuantityVisible());
				PXUIFieldAttribute.SetVisible<PMCostBudget.committedOpenAmount>(CostBudget.Cache, null, Setup.Current.CommitmentTracking == true);
				PXUIFieldAttribute.SetVisible<PMCostBudget.committedOpenQty>(CostBudget.Cache, null, Setup.Current.CommitmentTracking == true && CostQuantityVisible());
				PXUIFieldAttribute.SetVisible<PMCostBudget.committedReceivedQty>(CostBudget.Cache, null, Setup.Current.CommitmentTracking == true && CostQuantityVisible());
				PXUIFieldAttribute.SetVisible<PMCostBudget.actualPlusOpenCommittedAmount>(CostBudget.Cache, null, Setup.Current.CommitmentTracking == true);
				PXUIFieldAttribute.SetVisible<PMCostBudget.varianceAmount>(CostBudget.Cache, null, Setup.Current.CommitmentTracking == true);
				PXUIFieldAttribute.SetVisible<PMCostBudget.inventoryID>(CostBudget.Cache, null, !CostCodeAttribute.UseCostCode() && CostBudgetIsEditable());
				PXUIFieldAttribute.SetVisible<PMCostBudget.costCodeID>(CostBudget.Cache, null, CostCodeAttribute.UseCostCode() && CostBudgetIsEditable());
				PXUIFieldAttribute.SetVisible<PMCostBudget.revenueInventoryID>(CostBudget.Cache, null, e.Row.BudgetLevel == BudgetLevels.Item && CostBudgetIsEditable());
				PXUIFieldAttribute.SetVisible<PMCostBudget.revenueTaskID>(CostBudget.Cache, null, CostBudgetIsEditable());
				PXUIFieldAttribute.SetVisible<PMCostBudget.rate>(CostBudget.Cache, null, CostQuantityVisible());
				...
				PXUIFieldAttribute.SetEnabled<PMCostBudget.rate>(CostBudget.Cache, null, BudgetEditable());
				PXUIFieldAttribute.SetEnabled<PMCostBudget.qty>(CostBudget.Cache, null,  BudgetEditable());
				PXUIFieldAttribute.SetEnabled<PMCostBudget.amount>(CostBudget.Cache, null, BudgetEditable());
				PXUIFieldAttribute.SetEnabled<PMCostBudget.revisedQty>(CostBudget.Cache, null, RevisedEditable());
				PXUIFieldAttribute.SetEnabled<PMCostBudget.revisedAmount>(CostBudget.Cache, null, RevisedEditable());
				PXUIFieldAttribute.SetVisible<PMCostBudget.changeOrderQty>(CostBudget.Cache, null, ChangeOrderVisible() && CostQuantityVisible());
				PXUIFieldAttribute.SetVisible<PMCostBudget.changeOrderAmount>(CostBudget.Cache, null, ChangeOrderVisible());
				PXUIFieldAttribute.SetVisible<PMCostBudget.lastCostAtCompletion>(CostBudget.Cache, null, ProductivityVisible());
				PXUIFieldAttribute.SetVisible<PMCostBudget.costAtCompletion>(CostBudget.Cache, null, ProductivityVisible());
				PXUIFieldAttribute.SetVisible<PMCostBudget.lastCostToComplete>(CostBudget.Cache, null, ProductivityVisible());
				PXUIFieldAttribute.SetVisible<PMCostBudget.costToComplete>(CostBudget.Cache, null, ProductivityVisible());
				...
				PXUIFieldAttribute.SetEnabled<PMCostBudget.costAtCompletion>(CostBudget.Cache, null, ProductivityVisible());//to hide import from excel columns
				PXUIFieldAttribute.SetEnabled<PMCostBudget.costToComplete>(CostBudget.Cache, null, ProductivityVisible());//to hide import from excel columns
				...
				PXUIFieldAttribute.SetVisibility<PMCostBudget.committedAmount>(CostBudget.Cache, null, Setup.Current.CommitmentTracking == true ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				PXUIFieldAttribute.SetVisibility<PMCostBudget.committedQty>(CostBudget.Cache, null, Setup.Current.CommitmentTracking == true  ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				PXUIFieldAttribute.SetVisibility<PMCostBudget.committedInvoicedAmount>(CostBudget.Cache, null, Setup.Current.CommitmentTracking == true ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				PXUIFieldAttribute.SetVisibility<PMCostBudget.committedInvoicedQty>(CostBudget.Cache, null, Setup.Current.CommitmentTracking == true  ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				PXUIFieldAttribute.SetVisibility<PMCostBudget.committedOpenAmount>(CostBudget.Cache, null, Setup.Current.CommitmentTracking == true ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				PXUIFieldAttribute.SetVisibility<PMCostBudget.committedOpenQty>(CostBudget.Cache, null, Setup.Current.CommitmentTracking == true  ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				PXUIFieldAttribute.SetVisibility<PMCostBudget.committedReceivedQty>(CostBudget.Cache, null, Setup.Current.CommitmentTracking == true  ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				PXUIFieldAttribute.SetVisibility<PMCostBudget.actualPlusOpenCommittedAmount>(CostBudget.Cache, null, Setup.Current.CommitmentTracking == true ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				PXUIFieldAttribute.SetVisibility<PMCostBudget.varianceAmount>(CostBudget.Cache, null, Setup.Current.CommitmentTracking == true ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				PXUIFieldAttribute.SetVisibility<PMCostBudget.inventoryID>(CostBudget.Cache, null, !CostCodeAttribute.UseCostCode() ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				...
				PXUIFieldAttribute.SetVisibility<PMCostBudget.changeOrderAmount>(CostBudget.Cache, null, ChangeOrderVisible() ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				PXUIFieldAttribute.SetVisibility<PMCostBudget.lastCostAtCompletion>(CostBudget.Cache, null, CostCodeAttribute.UseCostCode() ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				PXUIFieldAttribute.SetVisibility<PMCostBudget.costAtCompletion>(CostBudget.Cache, null, CostCodeAttribute.UseCostCode() ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				PXUIFieldAttribute.SetVisibility<PMCostBudget.lastCostToComplete>(CostBudget.Cache, null, CostCodeAttribute.UseCostCode() ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				PXUIFieldAttribute.SetVisibility<PMCostBudget.costToComplete>(CostBudget.Cache, null, CostCodeAttribute.UseCostCode() ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				...
				PXUIFieldAttribute.SetVisible<PMRevenueBudget.committedAmount>(RevenueBudget.Cache, null, Setup.Current.CommitmentTracking == true);
				PXUIFieldAttribute.SetVisible<PMRevenueBudget.committedQty>(RevenueBudget.Cache, null, Setup.Current.CommitmentTracking == true && RevenueQuantityVisible());
				PXUIFieldAttribute.SetVisible<PMRevenueBudget.committedInvoicedAmount>(RevenueBudget.Cache, null, Setup.Current.CommitmentTracking == true);
				PXUIFieldAttribute.SetVisible<PMRevenueBudget.committedInvoicedQty>(RevenueBudget.Cache, null, Setup.Current.CommitmentTracking == true && RevenueQuantityVisible()) ;
				PXUIFieldAttribute.SetVisible<PMRevenueBudget.committedOpenAmount>(RevenueBudget.Cache, null, Setup.Current.CommitmentTracking == true);
				PXUIFieldAttribute.SetVisible<PMRevenueBudget.committedOpenQty>(RevenueBudget.Cache, null, Setup.Current.CommitmentTracking == true && RevenueQuantityVisible());
				PXUIFieldAttribute.SetVisible<PMRevenueBudget.committedReceivedQty>(RevenueBudget.Cache, null, Setup.Current.CommitmentTracking == true && RevenueQuantityVisible());
				PXUIFieldAttribute.SetVisible<PMRevenueBudget.actualPlusOpenCommittedAmount>(RevenueBudget.Cache, null, Setup.Current.CommitmentTracking == true);
				PXUIFieldAttribute.SetVisible<PMRevenueBudget.varianceAmount>(RevenueBudget.Cache, null, Setup.Current.CommitmentTracking == true);
				PXUIFieldAttribute.SetVisible<PMRevenueBudget.inventoryID>(RevenueBudget.Cache, null, RevenueBudgetIsEditable() && !CostCodeAttribute.UseCostCode() && e.Row.BudgetLevel == BudgetLevels.Item || IsCopyPasteContext || IsExport || IsImport || enforceVisibility);
				PXUIFieldAttribute.SetVisible<PMRevenueBudget.costCodeID>(RevenueBudget.Cache, null, RevenueBudgetIsEditable() && CostCodeAttribute.UseCostCode() && e.Row.BudgetLevel == BudgetLevels.CostCode || IsCopyPasteContext || IsExport || IsImport || enforceVisibility);
				PXUIFieldAttribute.SetRequired<PMRevenueBudget.inventoryID>(RevenueBudget.Cache, e.Row.BudgetLevel == BudgetLevels.Item);
				PXUIFieldAttribute.SetEnabled<PMRevenueBudget.rate>(RevenueBudget.Cache, null, BudgetEditable());
				PXUIFieldAttribute.SetEnabled<PMRevenueBudget.qty>(RevenueBudget.Cache, null, BudgetEditable());
				PXUIFieldAttribute.SetEnabled<PMRevenueBudget.amount>(RevenueBudget.Cache, null, BudgetEditable());
				PXUIFieldAttribute.SetEnabled<PMRevenueBudget.revisedQty>(RevenueBudget.Cache, null, RevisedEditable());
				PXUIFieldAttribute.SetEnabled<PMRevenueBudget.revisedAmount>(RevenueBudget.Cache, null, RevisedEditable());
				...
				PXUIFieldAttribute.SetVisible<PMRevenueBudget.changeOrderAmount>(RevenueBudget.Cache, null, ChangeOrderVisible());
				PXUIFieldAttribute.SetVisible<PMRevenueBudget.prepaymentAmount>(RevenueBudget.Cache, null, PrepaymentVisible());
				PXUIFieldAttribute.SetVisible<PMRevenueBudget.prepaymentInvoiced>(RevenueBudget.Cache, null, PrepaymentVisible());
				PXUIFieldAttribute.SetVisible<PMRevenueBudget.prepaymentAvailable>(RevenueBudget.Cache, null, PrepaymentVisible());
				...
				PXUIFieldAttribute.SetVisible<RevenueBudgetFilter.groupByTask>(RevenueFilter.Cache, null, e.Row.BudgetLevel != BudgetLevels.Task);
				PXUIFieldAttribute.SetVisible<PMRevenueBudget.rate>(RevenueBudget.Cache, null, RevenueQuantityVisible());
				...
				PXUIFieldAttribute.SetVisibility<PMRevenueBudget.committedAmount>(RevenueBudget.Cache, null, Setup.Current.CommitmentTracking == true ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				PXUIFieldAttribute.SetVisibility<PMRevenueBudget.committedQty>(RevenueBudget.Cache, null, Setup.Current.CommitmentTracking == true ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				PXUIFieldAttribute.SetVisibility<PMRevenueBudget.committedInvoicedAmount>(RevenueBudget.Cache, null, Setup.Current.CommitmentTracking == true ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				PXUIFieldAttribute.SetVisibility<PMRevenueBudget.committedInvoicedQty>(RevenueBudget.Cache, null, Setup.Current.CommitmentTracking == true ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				PXUIFieldAttribute.SetVisibility<PMRevenueBudget.committedOpenAmount>(RevenueBudget.Cache, null, Setup.Current.CommitmentTracking == true ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				PXUIFieldAttribute.SetVisibility<PMRevenueBudget.committedOpenQty>(RevenueBudget.Cache, null, Setup.Current.CommitmentTracking == true ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				PXUIFieldAttribute.SetVisibility<PMRevenueBudget.committedReceivedQty>(RevenueBudget.Cache, null, Setup.Current.CommitmentTracking == true ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				PXUIFieldAttribute.SetVisibility<PMRevenueBudget.actualPlusOpenCommittedAmount>(RevenueBudget.Cache, null, Setup.Current.CommitmentTracking == true ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				PXUIFieldAttribute.SetVisibility<PMRevenueBudget.varianceAmount>(RevenueBudget.Cache, null, Setup.Current.CommitmentTracking == true ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				...
				PXUIFieldAttribute.SetVisibility<PMRevenueBudget.changeOrderAmount>(RevenueBudget.Cache, null, ChangeOrderVisible() ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				...
				PXUIFieldAttribute.SetVisibility<PMRevenueBudget.prepaymentInvoiced>(RevenueBudget.Cache, null, PrepaymentVisible() ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				PXUIFieldAttribute.SetVisibility<PMRevenueBudget.prepaymentAvailable>(RevenueBudget.Cache, null, PrepaymentVisible() ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				...
				PXUIFieldAttribute.SetEnabled<PMOtherBudget.rate>(OtherBudget.Cache, null, BudgetEditable());
				PXUIFieldAttribute.SetEnabled<PMOtherBudget.qty>(OtherBudget.Cache, null, BudgetEditable());
				PXUIFieldAttribute.SetEnabled<PMOtherBudget.amount>(OtherBudget.Cache, null, BudgetEditable());

				PXUIFieldAttribute.SetVisible<PMProjectBalanceRecord.committedAmount>(BalanceRecords.Cache, null, Setup.Current.CommitmentTracking == true);
				PXUIFieldAttribute.SetVisible<PMProjectBalanceRecord.committedInvoicedAmount>(BalanceRecords.Cache, null, Setup.Current.CommitmentTracking == true);
				PXUIFieldAttribute.SetVisible<PMProjectBalanceRecord.committedOpenAmount>(BalanceRecords.Cache, null, Setup.Current.CommitmentTracking == true);
				PXUIFieldAttribute.SetVisible<PMProjectBalanceRecord.actualPlusOpenCommittedAmount>(BalanceRecords.Cache, null, Setup.Current.CommitmentTracking == true);
				PXUIFieldAttribute.SetVisible<PMProjectBalanceRecord.varianceAmount>(BalanceRecords.Cache, null, Setup.Current.CommitmentTracking == true);

				PXUIFieldAttribute.SetVisibility<PMProjectBalanceRecord.committedAmount>(BalanceRecords.Cache, null, Setup.Current.CommitmentTracking == true ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				PXUIFieldAttribute.SetVisibility<PMProjectBalanceRecord.committedInvoicedAmount>(BalanceRecords.Cache, null, Setup.Current.CommitmentTracking == true ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				PXUIFieldAttribute.SetVisibility<PMProjectBalanceRecord.committedOpenAmount>(BalanceRecords.Cache, null, Setup.Current.CommitmentTracking == true ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				PXUIFieldAttribute.SetVisibility<PMProjectBalanceRecord.actualPlusOpenCommittedAmount>(BalanceRecords.Cache, null, Setup.Current.CommitmentTracking == true ? PXUIVisibility.Visible : PXUIVisibility.Invisible);
				PXUIFieldAttribute.SetVisibility<PMProjectBalanceRecord.varianceAmount>(BalanceRecords.Cache, null, Setup.Current.CommitmentTracking == true ? PXUIVisibility.Visible : PXUIVisibility.Invisible);


				PMTaskTotal tt = select.Select();
				if (tt != null)
				{
					e.Row.Asset = tt.Asset;
					e.Row.Liability = tt.Liability;
					e.Row.Income = tt.Income;
					e.Row.Expense = tt.Expense;
				}
				
				if (e.Row.CustomerID != null)
				{
					PMProject doc = e.Row as PMProject;
					PMAddress address = this.Billing_Address.Select();

					bool enableAddressValidation = doc.Status == Contract.status.Completed && address != null && address.IsDefaultAddress == false && address.IsValidated == false;
		...
		protected virtual void _(Events.FieldUpdated<PMProject, PMProject.isActive> e)
		{
			if (e.Row.IsActive == true && (bool?)e.OldValue != true)
			{
				ActivateProject(e.Row);
			}
		}
		...
		protected virtual void _(Events.FieldUpdated<PMProject, PMProject.customerID> e)
		{
			if (e.Row != null)
			{
				if (e.Cache.GetStatus(e.Row) == PXEntryStatus.Inserted)
				{
					...
					Customer customer = cs.Select(e.Row.CustomerID);
					...
					if (!string.IsNullOrEmpty(defaultCuryID))
					{
						e.Row.CuryID = defaultCuryID;
					}

					if (!string.IsNullOrEmpty(defaultCuryRateType))
					{
						e.Row.RateTypeID = defaultCuryRateType;
					}
		...
		protected virtual void _(Events.FieldUpdated<PMProject, PMProject.templateID> e)
		{
			...
			if (ProjectProperties.Cache.GetStatus(e.Row) == PXEntryStatus.Inserted || this.IsMobile)
			{
				DefaultFromTemplate(e.Row, e.Cache);
			}
		}
		...
		protected virtual void _(Events.FieldDefaulting<PMProject, PMProject.curyID> e)
		{
			...
			{
				e.Row.CuryID = Company.Current.BaseCuryID;
			}
		}
		#endregion
		...
		#region Mobile related crap added by Andrew Vasilyev that has to be reviewed and removed
		...
		protected virtual void _(Events.FieldUpdated<PMRevenueBudget, PMRevenueBudget.amount> e)
		{
			if (e.Row != null)
			{
		...
		{
			if (e.Row != null && AutoCompleteRevenueBudget())
			{
				var select = new PXSelect<PMCostBudget, Where<PMCostBudget.revenueTaskID, Equal<Required<PMRevenueBudget.projectTaskID>>,
					And<PMCostBudget.isProduction, Equal<True>,
					And<PMCostBudget.type, Equal<GL.AccountType.expense>>>>>(this);
				var resultset = select.Select(e.Row.ProjectTaskID);
				if (resultset.Count > 0)
				{
					object value = e.ReturnValue;
					decimal revisedTotal = 0;
					decimal actualTotal = 0;

					foreach (PMCostBudget cost in resultset)
					{
						revisedTotal += cost.RevisedAmount.GetValueOrDefault();
						actualTotal += cost.ActualAmount.GetValueOrDefault();
					}

					if (resultset.Count > 0 && revisedTotal != 0)
					{
						value = Decimal.Round(100m * actualTotal / revisedTotal, 2);
					}
					PXFieldState fieldState = PXDecimalState.CreateInstance(value, (short)2, nameof(PMRevenueBudget.CompletedPct), false, 0, Decimal.MinValue, Decimal.MaxValue);
		...
		protected virtual void _(Events.FieldUpdated<PMRevenueBudget, PMRevenueBudget.revisedAmount> e)
		{
			RecalculateAmountToInvoice(e.Row);
		}

		protected virtual void _(Events.FieldUpdated<PMRevenueBudget, PMRevenueBudget.amountToInvoice> e)
		{
			if (!recalculatingAmountToInvoice) //The value is manually edited by the user on the screen
			{
				//Sync the Completed %
				decimal budgetedAmount = e.Row.RevisedAmount.GetValueOrDefault();
				if (budgetedAmount != 0)
				{
					decimal invoicedOrPendingPrepayment = e.Row.AmountToInvoice.GetValueOrDefault() + e.Row.ActualAmount.GetValueOrDefault() + e.Row.InvoicedAmount.GetValueOrDefault() + (e.Row.PrepaymentAmount.GetValueOrDefault() - e.Row.PrepaymentInvoiced.GetValueOrDefault());
		...
		protected virtual void _(Events.FieldVerifying<PMRevenueBudget, PMRevenueBudget.prepaymentPct> e)
		{
			...
			if (prepayment < e.Row.PrepaymentInvoiced)
			{
		...
		protected virtual void _(Events.FieldVerifying<PMRevenueBudget, PMRevenueBudget.prepaymentAmount> e)
		{
			...
			if (prepayment.GetValueOrDefault() < e.Row.PrepaymentInvoiced)
			{
				throw new PXSetPropertyException<PMRevenueBudget.prepaymentAmount>(Messages.PrepaimentLessThanInvoiced, PXErrorLevel.Error);
			}

			decimal invoicedAmount = e.Row.ActualAmount.GetValueOrDefault() + e.Row.InvoicedAmount.GetValueOrDefault();
		...
		protected virtual void _(Events.FieldSelecting<PMRevenueBudget, PMRevenueBudget.amountToInvoice> e)
		{
			if (e.Row != null)
			{
				var select = new PXSelect<PMCostBudget, Where<PMCostBudget.revenueTaskID, Equal<Required<PMRevenueBudget.projectTaskID>>,
				...
				{
					decimal budgetedAmount = e.Row.RevisedAmount.GetValueOrDefault();
					decimal result = Math.Max(0, (budgetedAmount * e.Row.CompletedPct.GetValueOrDefault() / 100m) - e.Row.InvoicedAmount.GetValueOrDefault());

					PXFieldState fieldState = PXDecimalState.CreateInstance(result, (short)2, nameof(PMRevenueBudget.AmountToInvoice), false, 0, Decimal.MinValue, Decimal.MaxValue);
					fieldState.Enabled = false;
		...
		protected virtual void _(Events.FieldVerifying<PMRevenueBudget, PMRevenueBudget.amountToInvoice> e)
		{
			...
			if (newValue.GetValueOrDefault() != 0)
			{
				...
				{
					e.Cache.RaiseExceptionHandling<PMRevenueBudget.amountToInvoice>(e.Row, e.NewValue, new PXSetPropertyException<PMRevenueBudget.amountToInvoice>(Messages.NoProgressiveRule, PXErrorLevel.Warning));
				}
			}
		}
		...
		protected virtual void _(Events.FieldUpdated<PMCostBudget, PMCostBudget.amount> e)
		{
			if (e.Row != null)
			{
				e.Row.RevisedAmount = e.Row.Amount;
		...
		protected virtual void _(Events.FieldUpdated<PMCostBudget, PMCostBudget.inventoryID> e)
		{
			if (!CostCodeAttribute.UseCostCode())
			{
				...
				if (pendingRate == null)
				{
					object pendingAmount = CostBudget.Cache.GetValuePending<PMCostBudget.amount>(e.Row);
					e.Cache.SetDefaultExt<PMCostBudget.rate>(e.Row);//formula will recalculate the amount.

					if (pendingAmount != null)//reset the amount.
					{
						e.Cache.SetValueExt<PMCostBudget.amount>(e.Row, pendingAmount);
					}
				}
		...
		protected virtual void _(Events.RowPersisting<PMCostBudget> e)
		{
			...
			{
				if (Project.Current != null && Project.Current.BudgetMetricsEnabled == true && UpdateLastXXXFields(e.Row))
				{
					RecordProductionRecord(e.Row);
				}
			}
		}
		#endregion
		...
		protected virtual void _(Events.FieldUpdated<PMOtherBudget, PMOtherBudget.inventoryID> e)
		{
			if (!CostCodeAttribute.UseCostCode())
			{
				...
				object pendingRate = RevenueBudget.Cache.GetValuePending<PMOtherBudget.rate>(e.Row);
				...
				if (pendingRate == null)
					e.Cache.SetDefaultExt<PMOtherBudget.rate>(e.Row);
			}
		}
		...
		protected virtual void _(Events.FieldUpdated<PMOtherBudget, PMOtherBudget.amount> e)
		{
			if (e.Row != null)
			{
				e.Row.RevisedAmount = e.Row.Amount;
		...
		{
			return CostCodeAttribute.UseCostCode();
		}
		...
		public virtual bool AutoCompleteRevenueBudget()
		{
			return false;
		}

		public virtual PMTask CopyTask(PMTask task, int ProjectID)
		{
			...
			foreach (CSAnswers answer in Answer.Select(task.NoteID))
			{
				CSAnswers dstanswer =
					Answer.Insert(new CSAnswers()
					{
						RefNoteID = dst.NoteID,
			...
			else
			{
				...
				foreach (PMRevenueBudget budget in selectRevenueBudget.Select(task.ProjectID, task.TaskID))
				{
					...
					else
					{
						...
						summary.Amount += budget.Amount.GetValueOrDefault();
						summary.RevisedAmount += budget.Amount.GetValueOrDefault();
					}
				}
			...
			foreach (PMDetail detail in PXSelect<PMDetail, Where<PMDetail.contractID, Equal<Current<PMTask.projectID>>, And<PMDetail.taskID, Equal<Current<PMTask.taskID>>>>>.SelectMultiBound(this, new object[] { task }))
			{
				PMDetail newDetail = new PMDetail();
				newDetail.ContractID = ProjectID;
				newDetail.TaskID = dst.TaskID;
				newDetail.InventoryID = detail.InventoryID;
				newDetail.UOM = detail.UOM;
				newDetail.Description = detail.Description;
		...
		public virtual void DefaultFromTemplate(PMProject prj, PXCache sender)
		{
			PMProject templ = PXSelect<PMProject, Where<PMProject.contractID, Equal<Required<PMProject.contractID>>>>.Select(this, prj.TemplateID);
			...
			foreach (PMTask task in PXSelect<PMTask, Where<PMTask.projectID, Equal<Current<PMProject.contractID>>>>.SelectMultiBound(this, new object[] { templ }))
			{
				...
				{
					PMTask dest = CopyTask(task, (int)prj.ContractID);
		...
		public virtual void RecalculateAmountToInvoice(PMRevenueBudget row)
		{
			if (row != null)
			{
				decimal budgetedAmount = row.RevisedAmount.GetValueOrDefault();
				decimal invoicedOrPendingPrepayment = row.ActualAmount.GetValueOrDefault() + row.InvoicedAmount.GetValueOrDefault() + (row.PrepaymentAmount.GetValueOrDefault() - row.PrepaymentInvoiced.GetValueOrDefault());
				decimal amountToInvoice = Math.Max(0, (budgetedAmount * row.CompletedPct.GetValueOrDefault() / 100m) - invoicedOrPendingPrepayment);
				...
				if (ContainsProgressiveBillingRule(projectTaskID))
				{
					try
					{
						...
						RevenueBudget.SetValueExt<PMRevenueBudget.amountToInvoice>(row, amountToInvoice);
					}
					finally
					{
		...
		public virtual bool UpdateLastXXXFields(PMCostBudget budget)
		{
			...
			bool updated = false;
			if (budget.CostToComplete != originalCostToComplete)
			{
				budget.LastCostToComplete = originalCostToComplete;
				updated = true;
			}

			if (budget.PercentCompleted != originalPercentCompleted)
			{
				budget.LastPercentCompleted = originalPercentCompleted;
				updated = true;
			}

			if (budget.CostAtCompletion != originalCostAtCompletion)
			{
		...
		public virtual PMBudgetProduction RecordProductionRecord(PMBudget budget)
		{
			...
			item.CostToComplete = budget.LastCostToComplete;
			item.PercentCompleted = budget.LastPercentCompleted;
			item.CostAtCompletion = budget.LastCostAtCompletion;
		...
		public virtual void AddToSummary(PMBudget summary, PMBudget record)
		{
			...
			summary.RevisedAmount = summary.RevisedAmount.GetValueOrDefault() + record.RevisedAmount.GetValueOrDefault();
			...
			summary.CommittedOpenAmount = summary.CommittedOpenAmount.GetValueOrDefault() + record.CommittedOpenAmount.GetValueOrDefault();
			summary.CommittedInvoicedAmount = summary.CommittedInvoicedAmount.GetValueOrDefault() + record.CommittedInvoicedAmount.GetValueOrDefault();
			summary.ChangeOrderAmount = summary.ChangeOrderAmount.GetValueOrDefault() + record.ChangeOrderAmount.GetValueOrDefault();
		...
		public virtual IEnumerable AggregateBudget<T>(IList<T> list)
			where T: PMBudget, new()
		{
			...
			foreach (var result in aggregates.Values)
				yield return result;
		...
		[Serializable]
		[System.Diagnostics.CodeAnalysis.ExcludeFromCodeCoverage]
		public class PMProjectBalanceRecord : PX.Data.IBqlTable, IComparable<PMProjectBalanceRecord>
		{
			...
			[PXBaseCury]
			[PXUIField(DisplayName = "Current Budgeted Amount")]
		...
		public class CostBudgetFilter : IBqlTable
		{
			...
			#region GroupByTask
			public abstract class groupByTask : PX.Data.IBqlField
			{
			}
			[PXDBBool()]
			[PXDefault(false)]
		...
		public virtual IEnumerable GetBalanceLines(string accountType, List<PMProjectBalanceRecord> records)
		{
			if (records.Count > 0)
			{
				...
				decimal totalComAmt = 0;
				decimal totalComOpenAmt = 0;
				decimal totalComInvoicedAmt = 0;
				...
				foreach (PMProjectBalanceRecord record in records)
				{
					if (!IsMobile)
					{
						...
						totalComAmt += record.CommittedAmount ?? 0;
						totalComOpenAmt += record.CommittedOpenAmount ?? 0;
						totalComInvoicedAmt += record.CommittedInvoicedAmount ?? 0;
					}
				...
				if (!IsMobile)
				{
					yield return CreateTotal(accountType, totalAmt, totalRevAmt, totalActAmt, totalComAmt, totalComOpenAmt, totalComInvoicedAmt);
				}
			}
		}
		...
		public virtual PMProjectBalanceRecord CreateTotal(string accountType, decimal amount, decimal revisedAmt, decimal actualAmt, decimal committedAmt, decimal committedOpenAmt, decimal committedInvoicedAmt)
		{
			PMProjectBalanceRecord record = new PMProjectBalanceRecord();
			...
			switch (accountType)
			{
				case AccountType.Asset:
					record.RecordID = -11;
					record.Description = Messages.GetLocal(GL.Messages.Asset) + " " + Messages.GetLocal(GL.Messages.Totals);
					break;

				case AccountType.Liability:
					record.RecordID = -21;
					record.Description = Messages.GetLocal(GL.Messages.Liability) + " " + Messages.GetLocal(GL.Messages.Totals);
					break;

				case AccountType.Income:
					record.RecordID = -31;
					record.Description = Messages.GetLocal(GL.Messages.Income) + " " + Messages.GetLocal(GL.Messages.Totals);
					break;

				case AccountType.Expense:
					record.RecordID = -41;
					record.Description = Messages.GetLocal(GL.Messages.Expense) + " " + Messages.GetLocal(GL.Messages.Totals);
					break;

				case PMAccountType.OffBalance:
					record.RecordID = -51;
					record.Description = Messages.GetLocal(Messages.OffBalance) + " " + Messages.GetLocal(GL.Messages.Totals);
					break;
			}
			...
			record.CommittedAmount = committedAmt;
			record.CommittedOpenAmount = committedOpenAmt;
			record.CommittedInvoicedAmount = committedInvoicedAmt;
