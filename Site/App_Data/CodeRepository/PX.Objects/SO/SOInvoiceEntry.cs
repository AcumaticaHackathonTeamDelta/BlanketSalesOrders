...
namespace PX.Objects.SO
{
	public class SOInvoiceEntry : ARInvoiceEntry
	{
		...
		private void CuryUnitPriceDefaulting(PXCache cache, ARTran row)
		{
			if (row != null && row.InventoryID != null && row.UOM != null && row.IsFree != true && row.ManualPrice != true)
			{
			...
			decimal? price = ARSalesPriceMaint.CalculateSalesPrice(
				...
				cache.SetValueExt<ARTran.curyUnitPrice>(row, price);
				ARSalesPriceMaint.CheckNewUnitPrice<ARTran, ARTran.curyUnitPrice>(cache, row, price);
			}
			else
			{
				cache.SetValueExt<ARTran.curyUnitPrice>(row, row.CuryUnitPrice ?? 0m);
			}
		}
		...
		public virtual void InvoiceOrder(DateTime invoiceDate, PXResult<SOOrderShipment, SOOrder, CurrencyInfo, SOAddress, SOContact, SOOrderType> order, PXResultset<SOShipLine, SOLine> details, Customer customer, DocumentList<ARInvoice, SOInvoice> list)
		{
			...
			{
				bool iscc = false;
				DateTime? orderInvoiceDate = (sosetup.Current.UseShipDateForInvoiceDate == true && soOrder.InvoiceDate == null ? orderShipment.ShipDate : soOrder.InvoiceDate);
				...
				if (soOrder.PaymentCntr == 0 && soOrder.BillSeparately == false && iscc == false)
				{
					if (soOrder.PaymentMethodID == null && soOrder.CashAccountID == null)
						newdoc = list.Find<ARInvoice.docType, ARInvoice.docDate, ARInvoice.branchID, ARInvoice.customerID, ARInvoice.customerLocationID, SOInvoice.billAddressID, SOInvoice.billContactID, SOInvoice.extRefNbr, ARInvoice.curyID, ARInvoice.termsID, ARInvoice.hidden>(soOrderType.ARDocType, orderInvoiceDate ?? invoiceDate, soOrder.BranchID, soOrder.CustomerID, soOrder.CustomerLocationID, soOrder.BillAddressID, soOrder.BillContactID, soOrder.ExtRefNbr, soOrder.CuryID, soOrder.TermsID, false) ?? new ARInvoice();
					else if (soOrder.CashAccountID == null)
						newdoc = list.Find<ARInvoice.docType, ARInvoice.docDate, ARInvoice.branchID, ARInvoice.customerID, ARInvoice.customerLocationID, SOInvoice.billAddressID, SOInvoice.billContactID, SOInvoice.pMInstanceID, SOInvoice.extRefNbr, ARInvoice.curyID, ARInvoice.termsID, ARInvoice.hidden>(soOrderType.ARDocType, orderInvoiceDate ?? invoiceDate, soOrder.BranchID, soOrder.CustomerID, soOrder.CustomerLocationID, soOrder.BillAddressID, soOrder.BillContactID, soOrder.PMInstanceID, soOrder.ExtRefNbr, soOrder.CuryID, soOrder.TermsID, false) ?? new ARInvoice();
					else if (AvalaraMaint.IsExternalTax(this, soOrder.TaxZoneID))
						newdoc = list.Find<ARInvoice.docType, ARInvoice.docDate, ARInvoice.branchID, ARInvoice.customerID, ARInvoice.customerLocationID, SOInvoice.billAddressID, SOInvoice.shipAddressID, SOInvoice.billContactID, SOInvoice.pMInstanceID, SOInvoice.cashAccountID, SOInvoice.extRefNbr, ARInvoice.curyID, ARInvoice.termsID, ARInvoice.hidden>(soOrderType.ARDocType, orderInvoiceDate ?? invoiceDate, soOrder.BranchID, soOrder.CustomerID, soOrder.CustomerLocationID, soOrder.BillAddressID, orderShipment.ShipAddressID, soOrder.BillContactID, soOrder.PMInstanceID, soOrder.CashAccountID, soOrder.ExtRefNbr, soOrder.CuryID, soOrder.TermsID, false) ?? new ARInvoice();
					else
						newdoc = list.Find<ARInvoice.docType, ARInvoice.docDate, ARInvoice.branchID, ARInvoice.customerID, ARInvoice.customerLocationID, SOInvoice.billAddressID, SOInvoice.billContactID, SOInvoice.pMInstanceID, SOInvoice.cashAccountID, SOInvoice.extRefNbr, ARInvoice.curyID, ARInvoice.termsID, ARInvoice.hidden>(soOrderType.ARDocType, orderInvoiceDate ?? invoiceDate, soOrder.BranchID, soOrder.CustomerID, soOrder.CustomerLocationID, soOrder.BillAddressID, soOrder.BillContactID, soOrder.PMInstanceID, soOrder.CashAccountID, soOrder.ExtRefNbr, soOrder.CuryID, soOrder.TermsID, false) ?? new ARInvoice();
				}
				else
				{
                ...
                foreach (SOOrderDiscountDetail docGroupDisc in selectOrderDocGroupDiscounts.Select(orderShipment.OrderType, orderShipment.OrderNbr))
                {
	                ...
	                if (docGroupDisc.Type == DiscountType.Group)
                    {
                        ...
                        foreach (KeyValuePair<DiscountSequenceKey, DiscountEngine<SOLine>.DiscountDetailToLineCorrelation<SOOrderDiscountDetail, SOLine>> dsGroup in grLinesOrderCorrelation)
                        {
                            ...
                            {
	                            var dsKey = new DiscountSequenceKey
											{
												DiscountID = dsGroup.Key.DiscountID,
												DiscountSequenceID = dsGroup.Key.DiscountSequenceID
											};
		...
		public virtual void UpdateDocState(SOInvoice doc, CCTranType lastOperation)
		{
            ...
            CCProcTran voidedTran = PXSelect<CCProcTran,
                    Where<CCProcTran.refNbr, Equal<Required<SOInvoice.refNbr>>,
                        And<CCProcTran.docType, Equal<Required<SOInvoice.docType>>,
                        And<CCProcTran.tranType, Equal<CCTranTypeCode.voidTran>,
                        And<CCProcTran.pCTranNumber, Equal<Required<SOInvoice.extRefNbr>>>>>>>.Select(this, doc.RefNbr, doc.DocType, doc.ExtRefNbr);
            
            if ((string.IsNullOrEmpty(doc.ExtRefNbr) || voidedTran != null) && doc.IsCCCaptured == true)
              {
