		...
		if (string.IsNullOrEmpty(lastUrl) || !string.IsNullOrEmpty(screen))
		{
			...
			if (string.IsNullOrEmpty(url) || !url.Replace(".aspx", "").ToUpper().EndsWith(screen.ToUpper()))
			{
				...
				string query = Page.Request.Url.Query
					...
					.Replace("ScreenID=" + screen, String.Empty)
					.Replace(PXUrl.CompanyID + "=" + HttpUtility.UrlEncode(company), String.Empty)
					.Replace(PXUrl.CompanyID + "=" + company, String.Empty)
				...
				//if (node != null && PXList.Provider.HasList(node.NodeID))
				//	lastUrl = lastUrl.AppendUrlParameter("PopupPanel", "Inline");
			}
		}
		return lastUrl;
	}
