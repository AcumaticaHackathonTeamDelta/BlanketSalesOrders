...
public partial class Page_FS300000 : System.Web.UI.Page
{
    ...
    /// </summary>
    /// <param name="scheduledDateStart">
    /// Double
    /// </param>
    /// <param name="scheduledDateEnd">
    /// Double
    /// </param>
    /// <returns>object[{StartDate,EndDate,WorkDay}]</returns>
    [WebMethod]
    [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Xml)]
    public static Object GetFromToCSCalendarExceptions(DateTime scheduledDateStart, DateTime scheduledDateEnd)
    {
        var graphExternalControls = PXGraph.CreateInstance<ExternalControls>();
        List<object> retList = new List<object>();
        DateHandler requestDateStart = new DateHandler(scheduledDateStart);
        DateHandler requestDateEnd = new DateHandler(scheduledDateEnd);
    ...
    [WebMethod]
    [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Xml)]
    public static Object GetDayRules(DateTime scheduledDateStart, DateTime? scheduledDateEnd)
    {
        var graphExternalControls = PXGraph.CreateInstance<ExternalControls>();
        List<object> retList = new List<object>();
        DateHandler requestDateStart = new DateHandler(scheduledDateStart);
        DateHandler requestDateEnd = scheduledDateEnd == null ? new DateHandler(scheduledDateStart) : new DateHandler((DateTime)scheduledDateEnd);
        ...
        if (fsSetupRow != null && fsSetupRow.CalendarID != null)
        {
            ...
            if (csCalendarExceptionsRow != null)
            {
                ...
                if (showCalendarException)
                {
                    retList.Add(
                        new
                        {
                            StartDate = requestDateStart.setHours(startExDate),
                            EndDate = requestDateStart.setHours(endExDate),
            ...
            if (dayChecked == true || workExceptionFlag)
            {
                retList.Add(
                    new
                    {
                        StartDate = requestDateStart.setHours(startDate),
                        EndDate = requestDateEnd.setHours(endDate),
    ...
    public static Object GetBranches()
    {
        ...
        List<object> retList = new List<object>();
        RequestParams request = new RequestParams(HttpContext.Current.Request);
        var branches = PXAccess.GetBranches();

        int totalRows = branches.Length;
        int total = (request.Limit != 0 && request.Limit < totalRows) ? request.Limit : totalRows;
        int start = (request.Start != 0) ? request.Start : 0;
        ...
        for (int i = 0; i < total && (start + i < totalRows); i++)
        {
            PXAccess.BranchCollection.Info branch = branches[start + i];
    ...
    [WebMethod]
    [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Xml)]
    public static Object GetEmployees(int? branchID, int? branchLocationID, bool? ignoreActiveFlag, bool? ignoreAvailabilityFlag, DateTime? scheduledDate, bool? loadVendor)
    {
        var graphExternalControls = PXGraph.CreateInstance<ExternalControls>();
        RequestParams request = new RequestParams(HttpContext.Current.Request);
        List<object> retList = new List<object>();
        ...
        PXResultset<Vendor> vendorRows = graphExternalControls.VendorRecords(branchID, branchLocationID, ignoreActiveFlag, ignoreAvailabilityFlag, date, request.Filters);
    ...
    [WebMethod]
    [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Xml)]
    public static Object GetEmployeeWorkingSchedule(DateTime scheduledDateStart, DateTime? scheduledDateEnd, bool compressSlot, int[] employeeIDList, int? branchID, int? branchLocationID)
    {
        var graphExternalControls = PXGraph.CreateInstance<ExternalControls>();
        List<object> retList = new List<object>();
        RequestParams request = new RequestParams(HttpContext.Current.Request);
    ...
    [WebMethod]
    [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Xml)]
    public static Object GetDispatchBoardAppointments(DateTime scheduledDate, int[] employeeIDList, int? branchID, int? branchLocationID)
    {
        var graphExternalControls = PXGraph.CreateInstance<ExternalControls>();
    ...
    [WebMethod]
    [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Xml)]
    public static Object GetRoomAppointments(DateTime scheduledDate, int[] roomIDList, int? branchID, int? branchLocationID)
    {
        var graphExternalControls = PXGraph.CreateInstance<ExternalControls>();
    ...
    [WebMethod]
    [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Xml)]
    public static Object GetUserBoardAppointments(DateTime scheduledDateStart, DateTime? scheduledDateEnd, int? employeeID, int? branchID, int? branchLocationID)
    {
        var graphExternalControls = PXGraph.CreateInstance<ExternalControls>();
    ...
    public static Object GetAppointmentToolTip(int appointmentID)
    {
        ...
        Contact contactRow = (Contact)graphExternalControls.Contact
                            .Select(fsServiceOrderRow.ContactID);

        if (contactRow != null)
        {
            fsServiceOrderRow.ContactEmail = contactRow.EMail;
            fsServiceOrderRow.ContactPhone = contactRow.Phone1;
            fsServiceOrderRow.ContactName = contactRow.DisplayName;
        }
    ...
    public static Object PutAppointments(FSAppointmentScheduleBoard appointment)
    {
        ...
        ExternalControls.DispatchBoardAppointmentMessages messages = graphExternalControls.DBPutAppointments(appointment);
    ...
    [WebMethod]
    [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Xml)]
    public static Object GetServiceOrders(int branchID, int? branchLocationID, DateTime? scheduledDateStart = null, DateTime? scheduledDateEnd = null, bool? isRoomCalendar = false)
    {
        var graphExternalControls = PXGraph.CreateInstance<ExternalControls>();
        RequestParams request = new RequestParams(HttpContext.Current.Request);
        List<object> retList = new List<object>();

        PXResultset<FSServiceOrder> fsServiceOrderRows = ExternalControls.ServiceOrderRecords(branchID, branchLocationID, request.Filters, scheduledDateStart, scheduledDateEnd, isRoomCalendar);
        ...
        foreach (FSServiceOrder fsServiceOrderRow in fsServiceOrderRows)
        {
            ...
            foreach (FSSODet fsSODetRow in fsSODetRows)
            {
                ...
                if (fsAppointmentDetRow == null)
                {
                    serviceClassesList.Add(fsSODetRow.ServiceID);
                    if (!rowAppDetFound)
                    {
    ...
    [WebMethod]
    [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Xml)]
    public static Object GetServiceOrdersTree(int? branchID, int? node, int? branchLocationID, DateTime? scheduledDateStart = null, DateTime? scheduledDateEnd = null, bool? isRoomCalendar = false)
    {
        var graphExternalControls = PXGraph.CreateInstance<ExternalControls>();
        RequestParams request = new RequestParams(HttpContext.Current.Request);
        List<object> retList = new List<object>();
            ...
            for (int i = 0; i < total && (start + i < fsServiceOrderRows.Count); i++)
            {
                ...
                foreach (FSSODet fsSODetRow in fsSODetRows) //It can be ignored and treegrid request node by node
                {
                    ...
                    tmpSODet.Add(new
                    {
                        ...
                        ServiceID = fsSODetRow.ServiceID,
                ...
                fsServiceOrderRow.Leaf = (fsSODetRows.Count == 0);
    ...
    [WebMethod]
    [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Xml)]
    public static Object GetUnassignedAppointments(DateTime scheduledDateStart, DateTime? scheduledDateEnd, int? branchID, int? branchLocationID, bool? unassignedAppointmentByRoom)
    {
        var graphExternalControls = PXGraph.CreateInstance<ExternalControls>();
        ...
        foreach (FSCustomFieldAppointment fsCustomFieldAppointmentRow in results )
        {
            if (fsSetupRow != null)
            {
                if ((fsCustomFieldAppointmentRow.FieldName != "Room" || fsSetupRow.ManageRooms == true)
                        && (fsCustomFieldAppointmentRow.FieldName != "AttendeeCount" || fsSetupRow.ManageAttendees == true))
                {
    ...
    */
    [WebMethod]
    [ScriptMethod(UseHttpGet = true, ResponseFormat = ResponseFormat.Xml)]
    public static Object GetRoutes(double scheduledDate, int? node, int branchID)
    {
        var graphExternalControls = PXGraph.CreateInstance<ExternalControls>();
        RequestParams request = new RequestParams(HttpContext.Current.Request);
        List<object> retList = new List<object>();
    ...
    public static Object GetStaffAppointments(DateTime scheduledDate, int? node, int branchID, int? branchLocationID, bool unassignedAppointmentFlag)
    {
        ...
        if (node == 0)
        {
            ...
            for (int i = 0; i < total && (start + i < results.Count); i++)
            {
                PXResult<EPEmployee, FSAppointmentEmployee, FSAppointment, FSServiceOrder, BAccount> result = (PXResult<EPEmployee, FSAppointmentEmployee, FSAppointment, FSServiceOrder, BAccount>)results[i];
                EPEmployee epEmployee = result;
                BAccount bAccount = result;
    ...
    public static Object GetResourceAppointments(double appointmentsDate)
    {
        ...
        foreach (PXResult<FSAppointment, FSServiceOrder, FSAppointmentEmployee, EPEmployee, Location, Contact, Customer> result in results)
        {
            ...
            tmpAppointment.Add(new
            {
                ...
                //Tooltip fields
                ContactName = contactRow.DisplayName,
                ContactEmail = contactRow.EMail,
                ContactPhone = contactRow.Phone1,
    ...
    public static Object GetRouteDevices(String[] RouteDevice)
    {
        ...
        {
            var results = graphExternalControls.FSGPSTrackingHistoryByTrackingID.Select(Guid.Parse(trackingID));
            if (results != null && results.Count > 0)
            {
                FSGPSTrackingHistory row = results[0];
                retList.Add(new
                {
    ...
    public static Object NewAppointmentBridge(FSAppointmentScheduleBoard appointment, List<int> sODetIDList, List<int> employeeIDList)
    {
        ...
        if(success)
        {
            int? processID = graphExternalControls.DBCreateAppointmentBridge(appointment, sODetIDList, employeeIDList);
